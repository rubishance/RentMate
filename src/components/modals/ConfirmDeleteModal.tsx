...
export function ConfirmDeleteModal({
    ...
    return createPortal(
        <AnimatePresence>
            <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-6 bg-black/50 backdrop-blur-sm" dir={lang === 'he' ? 'rtl' : 'ltr'}>
                <motion.div
                    initial={{ opacity: 0, scale: 0.95, y: 20 }}
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0.95, y: 20 }}
                    className="bg-window w-full shadow-2xl overflow-hidden flex flex-col max-h-[90dvh] relative rounded-t-3xl border-0 mt-auto sm:max-h-[85vh] sm:rounded-2xl sm:border sm:border-border/50 sm:mt-0 max-w-lg"
                >
                    <div className="p-6 overflow-y-auto flex-1">
                        ...
                        {step === 1 ? (
                            <div className="space-y-4">
                                ...
                                {verificationText && (
                                    <div className="space-y-4 pt-2">
                                        ...
                                        <input
                                            ...
                                            className="w-full p-2 sm:p-6 border border-border rounded-xl bg-background outline-none focus:ring-2 focus:ring-red-500/50 transition-all font-mono text-sm"
                                            autoComplete="off"
                                ...
                                <div className="p-2 sm:p-6 text-sm text-muted-foreground bg-secondary dark:bg-gray-700/50 rounded-xl">
                                    {lang === 'he'
                        ...
                        <div className="flex gap-2 sm:gap-4 mt-8">
                            <button
                                onClick={onClose}
                                disabled={isDeleting}
                                className="flex-1 px-6 py-2.5 text-gray-700 bg-muted hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 rounded-xl font-medium transition-colors"
                            >
                                {t('cancel')}
                            </button>
                            <button
                                ...
                                className={`flex-1 px-6 py-2.5 text-white rounded-xl font-medium transition-colors shadow-lg flex items-center justify-center gap-2 ${isDeleting || !canConfirm
