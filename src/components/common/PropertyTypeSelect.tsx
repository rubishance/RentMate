...
const PROPERTY_TYPES: { value: NonNullable<Property['property_type']>; label: string }[] = [
    { value: 'apartment', label: 'Apartment' },
    { value: 'penthouse', label: 'Penthouse' },
    { value: 'garden', label: 'Garden Apartment' },
    ...
    return (
        <div className="relative">
            <div className="relative w-full">
                ...
                <CustomDropdown value={value} onChange={onChange} disabled={disabled} />
            </div>
        </div>
    );
}
...
function CustomDropdown({ value, onChange, disabled }: PropertyTypeSelectProps) {
    const [isOpen, setIsOpen] = useState(false);
    const containerRef = useRef<HTMLDivElement>(null);

    const selectedOption = PROPERTY_TYPES.find(p => p.value === value) || PROPERTY_TYPES[0];
    ...
    return (
        <div className="relative" ref={containerRef}>
            <button
                ...
                disabled={disabled}
                className={`w-full flex items-center justify-between px-4 py-2 border rounded-lg bg-white dark:bg-gray-900 transition-all ${isOpen ? 'border-primary ring-2 ring-primary/20' : 'border-gray-200 dark:border-gray-700'
            ...
            {isOpen && (
                <div className="absolute top-full left-0 right-0 mt-1.5 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-xl z-50 overflow-hidden py-1 max-h-[400px] overflow-y-auto animate-in fade-in zoom-in-95 duration-100">
                    {PROPERTY_TYPES.map((type) => (
                        ...
                        >
                            ...
                            {value === type.value && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-blue-500" />}
                        </button>
                    ))}
                </div>
            )}
