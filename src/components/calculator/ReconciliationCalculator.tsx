    ...
    const handleLoadContract = async (contractId: string) => {
        ...
        try {
            ...
            if (contract) {
                ...
                const end = contract.end_date || format(new Date(), 'yyyy-MM-dd');
                setPeriodEnd(end);
                ...
                let tempDate = parseISO(contract.start_date);
                const lastDate = parseISO(end);

                while (tempDate <= lastDate) {
                    months.push(format(tempDate, 'yyyy-MM'));
    ...
    const handleCalculate = async () => {
        ...
        try {
            ...
            const res = await calculateReconciliation({
                ...
                linkageType: recLinkageType as 'cpi' | 'housing' | 'construction',
                                            ...
                                            <div key={payment.id} className="grid grid-cols-12 gap-4 items-center py-3 px-4 text-sm hover:bg-white dark:hover:bg-neutral-800/80 transition-colors group">
                                                ...
                                                <div className="col-span-4 relative">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full p-2 pl-6 bg-transparent border-transparent hover:bg-white dark:hover:bg-neutral-800 focus:bg-white dark:focus:bg-neutral-700 border-2 rounded-xl text-center font-black text-foreground outline-none transition-all"
                                                    />
                                                </div>
