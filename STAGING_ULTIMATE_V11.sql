-- RENTMATE GOLDEN SNAPSHOT V11.0 (ZEN BLOCK PARSED)
...
AS $$
DECLARE
-- [HEADER]     property_address text;
-- [HEADER]     notification_title text;
-- [HEADER]     notification_body text;
BEGIN
...
-- [HEADER]     notification_body := format('Contract for %s is now %s.', property_address, NEW.status);
    ...
    VALUES (
-- [HEADER]         NEW.user_id,
-- [HEADER]         'info', -- Status change is informational/important but not necessarily a warning
-- [HEADER]         notification_title,
-- [HEADER]         notification_body,
-- [HEADER]         json_build_object(
-- [HEADER]             'contract_id', NEW.id,
-- [HEADER]             'event', 'status_change',
-- [HEADER]             'old_status', OLD.status,
-- [HEADER]             'new_status', NEW.status
-- [HEADER]         )::jsonb
-- [HEADER]     );
...
DECLARE
-- [HEADER]     r RECORD;
-- [HEADER]     extension_days int := 60; -- Default extension notice period
BEGIN
    -------------------------------------------------------
...
-- [HEADER]         JOIN public.properties p ON p.id = c.property_id
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.end_date = CURRENT_DATE + INTERVAL '30 days'
-- [HEADER]     LOOP
        -- Check if we already sent this notification (idempotency)
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
-- [HEADER]             AND metadata->>'contract_id' = r.id::text 
-- [HEADER]             AND metadata->>'event' = 'ending_soon'
-- [HEADER]         ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [HEADER]                 r.user_id,
-- [HEADER]                 'warning',
-- [HEADER]                 'Contract Ending Soon',
-- [HEADER]                 format('Contract for %s, %s ends in 30 days (%s).', r.city, r.address, r.end_date),
-- [HEADER]                 json_build_object('contract_id', r.id, 'event', 'ending_soon')::jsonb
-- [HEADER]             );
        END IF;
    END LOOP;
...
-- [HEADER]         JOIN public.properties p ON p.id = c.property_id
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.extension_option = TRUE
        -- Assuming deadline IS the end_date if not specified otherwise, or checking user preference
-- [HEADER]         AND c.end_date = CURRENT_DATE + (extension_days || ' days')::INTERVAL
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
-- [HEADER]             AND metadata->>'contract_id' = r.id::text 
-- [HEADER]             AND metadata->>'event' = 'extension_deadline'
-- [HEADER]         ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [HEADER]                 r.user_id,
-- [HEADER]                 'action', -- Custom type 'action' or 'info'
-- [HEADER]                 'Extension Deadline Approaching',
-- [HEADER]                 format('Extension option for %s, %s ends in %s days.', r.city, r.address, extension_days),
-- [HEADER]                 json_build_object('contract_id', r.id, 'event', 'extension_deadline')::jsonb
-- [HEADER]             );
        END IF;
    END LOOP;
...
-- [HEADER]         JOIN public.properties p ON p.id = c.property_id
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.linkage_type != 'none' -- Only if linked
-- [HEADER]         AND (
-- [HEADER]             c.start_date + INTERVAL '1 year' = CURRENT_DATE OR
-- [HEADER]             c.start_date + INTERVAL '2 years' = CURRENT_DATE OR
-- [HEADER]             c.start_date + INTERVAL '3 years' = CURRENT_DATE
-- [HEADER]         )
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
-- [HEADER]             AND metadata->>'contract_id' = r.id::text 
-- [HEADER]             AND metadata->>'event' = 'index_update'
-- [HEADER]             AND metadata->>'date' = CURRENT_DATE::text
-- [HEADER]         ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [HEADER]                 r.user_id,
-- [HEADER]                 'urgent',
-- [HEADER]                 'Annual Index Update',
-- [HEADER]                 format('Annual index update required for %s, %s.', r.city, r.address),
-- [HEADER]                 json_build_object('contract_id', r.id, 'event', 'index_update', 'date', CURRENT_DATE)::jsonb
-- [HEADER]             );
        END IF;
    END LOOP;
...
-- [HEADER]         FROM public.payments py
-- [HEADER]         JOIN public.contracts c ON c.id = py.contract_id
-- [HEADER]         JOIN public.properties p ON p.id = c.property_id
-- [HEADER]         WHERE py.status = 'pending'
-- [HEADER]         AND py.date = CURRENT_DATE
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
-- [HEADER]             AND metadata->>'payment_id' = r.id::text 
-- [HEADER]             AND metadata->>'event' = 'payment_due'
-- [HEADER]         ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [HEADER]                 r.user_id,
-- [HEADER]                 'warning',
-- [HEADER]                 'Payment Due Today',
-- [HEADER]                 format('Payment of ג‚×%s for %s, %s is due today.', r.amount, r.city, r.address),
-- [HEADER]                 json_build_object('payment_id', r.id, 'event', 'payment_due')::jsonb
-- [HEADER]             );
        END IF;
    END LOOP;
...
DO $$
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'option_periods') THEN
...
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'index_base') THEN
...
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'linkage_rate') THEN
        ALTER TABLE contracts DROP COLUMN linkage_rate;
    END IF;

    -- Drop 'index_linkage_rate' if it exists on contracts (it belongs on payments)
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'index_linkage_rate') THEN
        ALTER TABLE contracts DROP COLUMN index_linkage_rate;
    END IF;

     -- Drop 'user_confirmed' if it exists on properties (not used)
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'user_confirmed') THEN
...
-- [HEADER] ON storage.objects FOR DELETE
-- [HEADER] TO authenticated
USING (bucket_id = 'contracts');
-- CREATE TABLE IF NOT EXISTS for storing index base periods and chaining factors
CREATE TABLE IF NOT EXISTS index_bases (
-- [HEADER]     id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
-- [HEADER]     index_type TEXT NOT NULL, -- e.g., 'cpi', 'construction', 'housing'
-- [HEADER]     base_period_start DATE NOT NULL, -- The start date of this base period (e.g., '2023-01-01')
-- [HEADER]     base_value NUMERIC NOT NULL DEFAULT 100.0, -- The value of the base index (usually 100.0)
-- [HEADER]     previous_base_period_start DATE, -- The start date of the *previous* base period
-- [HEADER]     chain_factor NUMERIC, -- The factor to multiply when moving FROM this base TO the previous base (or vice versa depending on logic)
...
-- [HEADER]   date TEXT NOT NULL, -- Format: 'YYYY-MM'
-- [HEADER]   value DECIMAL(10, 4) NOT NULL,
-- [HEADER]   source TEXT DEFAULT 'cbs' CHECK (source IN ('cbs', 'exchange-api', 'manual')),
-- [HEADER]   created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
DO $$
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'on_new_notification') THEN
...
-- [HEADER]     WHERE last_request_at < (now() - INTERVAL '1 hour');
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    -------------------------------------------------------
-- [HEADER]     IF (rule_ending_soon->>'is_enabled')::boolean IS TRUE THEN
-- [HEADER]         FOR r IN
            SELECT c.id, c.user_id, c.end_date, p.city, p.address
-- [HEADER]             FROM public.contracts c
-- [HEADER]             JOIN public.properties p ON p.id = c.property_id
-- [HEADER]             WHERE c.status = 'active'
-- [HEADER]             AND c.end_date = CURRENT_DATE + ((rule_ending_soon->>'days_offset')::int || ' days')::INTERVAL
-- [HEADER]         LOOP
-- [HEADER]             IF NOT EXISTS (SELECT 1 FROM public.notifications WHERE user_id = r.user_id AND metadata->>'contract_id' = r.id::text AND metadata->>'event' = 'ending_soon') THEN
                INSERT INTO public.notifications (user_id, type, title, message, metadata)
                VALUES (
-- [HEADER]                     r.user_id, 
-- [HEADER]                     'warning', 
                    (rule_ending_soon->>'name')::text, 
-- [HEADER]                     format((rule_ending_soon->>'message_template')::text, r.city, r.address, (rule_ending_soon->>'days_offset')::text), 
-- [HEADER]                     json_build_object('contract_id', r.id, 'event', 'ending_soon')::jsonb
-- [HEADER]                 );
            END IF;
        END LOOP;
    ...
    -------------------------------------------------------
-- [HEADER]     IF (rule_extension->>'is_enabled')::boolean IS TRUE THEN
-- [HEADER]         FOR r IN
            SELECT c.id, c.user_id, c.end_date, p.city, p.address
-- [HEADER]             FROM public.contracts c
-- [HEADER]             JOIN public.properties p ON p.id = c.property_id
-- [HEADER]             WHERE c.status = 'active'
-- [HEADER]             AND c.extension_option = TRUE
-- [HEADER]             AND c.end_date = CURRENT_DATE + ((rule_extension->>'days_offset')::int || ' days')::INTERVAL
-- [HEADER]         LOOP
-- [HEADER]             IF NOT EXISTS (SELECT 1 FROM public.notifications WHERE user_id = r.user_id AND metadata->>'contract_id' = r.id::text AND metadata->>'event' = 'extension_deadline') THEN
                INSERT INTO public.notifications (user_id, type, title, message, metadata)
                VALUES (
-- [HEADER]                     r.user_id, 
-- [HEADER]                     'action', 
                    (rule_extension->>'name')::text, 
-- [HEADER]                     format((rule_extension->>'message_template')::text, r.city, r.address, (rule_extension->>'days_offset')::text), 
-- [HEADER]                     json_build_object('contract_id', r.id, 'event', 'extension_deadline')::jsonb
-- [HEADER]                 );
            END IF;
        END LOOP;
    ...
    -- 3. ANNUAL INDEX UPDATE (1 Year after Start)
    -------------------------------------------------------
-- [HEADER]     IF (rule_index->>'is_enabled')::boolean IS TRUE THEN
-- [HEADER]         FOR r IN
            SELECT c.id, c.user_id, c.start_date, p.city, p.address
-- [HEADER]             FROM public.contracts c
-- [HEADER]             JOIN public.properties p ON p.id = c.property_id
-- [HEADER]             WHERE c.status = 'active'
-- [HEADER]             AND c.linkage_type != 'none'
-- [HEADER]             AND (
-- [HEADER]                 c.start_date + INTERVAL '1 year' = CURRENT_DATE OR
-- [HEADER]                 c.start_date + INTERVAL '2 years' = CURRENT_DATE OR
-- [HEADER]                 c.start_date + INTERVAL '3 years' = CURRENT_DATE
-- [HEADER]             )
-- [HEADER]         LOOP
-- [HEADER]             IF NOT EXISTS (SELECT 1 FROM public.notifications WHERE user_id = r.user_id AND metadata->>'contract_id' = r.id::text AND metadata->>'event' = 'index_update' AND metadata->>'date' = CURRENT_DATE::text) THEN
                INSERT INTO public.notifications (user_id, type, title, message, metadata)
                VALUES (
-- [HEADER]                     r.user_id, 
-- [HEADER]                     'urgent', 
                    (rule_index->>'name')::text, 
-- [HEADER]                     format((rule_index->>'message_template')::text, r.city, r.address), 
-- [HEADER]                     json_build_object('contract_id', r.id, 'event', 'index_update', 'date', CURRENT_DATE)::jsonb
-- [HEADER]                 );
            END IF;
        END LOOP;
    ...
    -- 4. PAYMENT DUE TODAY
    -------------------------------------------------------
-- [HEADER]     IF (rule_payment->>'is_enabled')::boolean IS TRUE THEN
-- [HEADER]         FOR r IN
            SELECT py.id, py.user_id, py.amount, py.date, p.city, p.address
-- [HEADER]             FROM public.payments py
-- [HEADER]             JOIN public.contracts c ON c.id = py.contract_id
...
-- [HEADER]             AND py.date = CURRENT_DATE
-- [HEADER]         LOOP
-- [HEADER]             IF NOT EXISTS (SELECT 1 FROM public.notifications WHERE user_id = r.user_id AND metadata->>'payment_id' = r.id::text AND metadata->>'event' = 'payment_due') THEN
                INSERT INTO public.notifications (user_id, type, title, message, metadata)
                VALUES (
-- [HEADER]                     r.user_id, 
-- [HEADER]                     'warning', 
                    (rule_payment->>'name')::text, 
-- [HEADER]                     format((rule_payment->>'message_template')::text, r.amount, r.city, r.address), 
-- [HEADER]                     json_build_object('payment_id', r.id, 'event', 'payment_due')::jsonb
-- [HEADER]                 );
            END IF;
        END LOOP;
...
-- [HEADER]     r RECORD;
BEGIN
-- [HEADER]     FOR r IN (SELECT schemaname, tablename, policyname 
-- [HEADER]               FROM pg_policies 
-- [HEADER]               WHERE schemaname = 'public' 
-- [HEADER]               AND tablename IN ('properties', 'tenants', 'contracts', 'payments'))
-- [HEADER]     LOOP
...
-- [HEADER]     r RECORD;
BEGIN
-- [HEADER]     FOR r IN (SELECT tablename, rowsecurity 
-- [HEADER]               FROM pg_tables 
-- [HEADER]               WHERE schemaname = 'public' 
-- [HEADER]               AND tablename IN ('properties', 'tenants', 'contracts', 'payments'))
-- [HEADER]     LOOP
-- [HEADER]         IF NOT r.rowsecurity THEN
...
-- [HEADER]         NEW.id,
-- [HEADER]         NEW.email,
-- [HEADER]         COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email),
-- [HEADER]         'user'::user_role,
-- [HEADER]         'active'::subscription_status,
-- [HEADER]         'free_forever'::subscription_plan_type
-- [HEADER]     )
-- [HEADER]     ON CONFLICT (id) DO NOTHING; -- Idempotency: If it exists, skip.
    ...
    BEGIN
        ...
        SET user_id = NEW.id
-- [HEADER]         WHERE user_id IS NULL 
-- [HEADER]         AND billing_email = NEW.email;
...
-- [HEADER]     FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();
...
-- [HEADER]     IF NEW.status = 'active' THEN
-- [HEADER]         IF EXISTS (
            SELECT 1 FROM public.contracts
-- [HEADER]             WHERE property_id = NEW.property_id
-- [HEADER]             AND status = 'active'
-- [HEADER]             AND id != NEW.id -- Exclude self during updates
-- [HEADER]             AND (
...
DROP TRIGGER IF EXISTS "trigger_check_active_contract" ON contracts;
...
-- [HEADER]     IF (OLD.status = 'active' AND NEW.status != 'active') THEN
...
DO $$
DECLARE
-- [HEADER]     target_email TEXT := 'rentmate.rubi@gmail.com'; -- <--- YOUR EMAIL HERE
-- [HEADER]     v_user_id UUID;
BEGIN
...
-- [HEADER]         v_user_id,
-- [HEADER]         target_email,
-- [HEADER]         'Admin User', -- Default name
-- [HEADER]         'admin',      -- Give yourself Admin access
-- [HEADER]         'active',
-- [HEADER]         'free_forever'
-- [HEADER]     )
...
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_profiles' AND column_name = 'first_name') THEN
        ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS first_name TEXT;
    END IF;

-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_profiles' AND column_name = 'last_name') THEN
        ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS last_name TEXT;
    END IF;
...
BEGIN
    -- Add has_parking
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'has_parking') THEN
        ALTER TABLE properties ADD COLUMN IF NOT EXISTS has_parking BOOLEAN DEFAULT false;
    END IF;

    -- Add has_storage
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'has_storage') THEN
        ALTER TABLE properties ADD COLUMN IF NOT EXISTS has_storage BOOLEAN DEFAULT false;
    END IF;

    -- Add property_type
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'property_type') THEN
        ALTER TABLE properties ADD COLUMN IF NOT EXISTS property_type TEXT DEFAULT 'apartment';
    END IF;

    -- Add image_url
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'properties' AND column_name = 'image_url') THEN
...
-- [HEADER]         role, 
-- [HEADER]         subscription_status, 
-- [HEADER]         plan_id, -- New relation
-- [HEADER]         subscription_plan -- Legacy enum fallback
-- [HEADER]     )
    VALUES (
-- [HEADER]         NEW.id,
-- [HEADER]         NEW.email,
...
-- [HEADER]         'user'::user_role,
-- [HEADER]         'active'::subscription_status,
-- [HEADER]         'free', -- Default to 'free' plan ID
-- [HEADER]         'free_forever'::subscription_plan_type -- Legacy fallback
-- [HEADER]     );
-- [HEADER]     RETURN NEW;
-- [HEADER] EXCEPTION WHEN OTHERS THEN
    RAISE EXCEPTION 'Profile creation failed: %', SQLERRM;
...
DECLARE
-- [HEADER]     v_user_id UUID;
-- [HEADER]     target_email TEXT := 'rentmate.rubi@gmail.com';
BEGIN
    SELECT id INTO v_user_id FROM auth.users WHERE email = target_email;
...
-- [HEADER]     )
    VALUES (
-- [HEADER]         NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email), 
-- [HEADER]         'user', 'active', 'free_forever'
-- [HEADER]     )
-- [HEADER]     ON CONFLICT (id) DO NOTHING;
...
-- [HEADER]         WHERE user_id IS NULL AND billing_email = NEW.email;
-- [HEADER]     EXCEPTION WHEN OTHERS THEN NULL;
    END;
...
-- [HEADER]     GET DIAGNOSTICS recovered_count = ROW_COUNT;
...
AS $$
DECLARE
-- [HEADER]     new_device_type TEXT;
-- [HEADER]     session_count INT;
-- [HEADER]     oldest_session_id UUID;
    -- FIX: Increased from 1 to 5 to prevent aggressive logouts
-- [HEADER]     max_sessions_per_type INT := 5; 
BEGIN
...
-- [HEADER]     IF session_count >= max_sessions_per_type THEN
...
-- [HEADER]         NEW.id,
-- [HEADER]         NEW.email,
-- [HEADER]         NEW.raw_user_meta_data->>'full_name',
-- [HEADER]         'user' -- Default role
-- [HEADER]     )
-- [HEADER]     ON CONFLICT (id) DO NOTHING; -- Prevent errors if retry
-- [HEADER]     RETURN NEW;
END;
$$;
...
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
...
-- [HEADER]         NEW.email,
-- [HEADER]         COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email),
-- [HEADER]         'user',           -- Text, let Postgres cast to user_role
-- [HEADER]         'active',         -- Text, let Postgres cast to subscription_status
-- [HEADER]         'free_forever'    -- Text, let Postgres cast to subscription_plan_type
-- [HEADER]     );
...
-- [HEADER]         RETURN 'desktop'; -- Default fallback
    END IF;

    -- Standard mobile indicators
    -- "Mobi" catches many browsers, "Android", "iPhone", "iPad" are specific
-- [HEADER]     IF user_agent ~* '(Mobi|Android|iPhone|iPad|iPod)' THEN
...
-- [HEADER]     session_count INT;
-- [HEADER]     oldest_session_id UUID;
-- [HEADER]     max_sessions_per_type INT := 1; -- Hardcoded limit: 1 per group
BEGIN
    -- Identify what kind of device is trying to log in
-- [HEADER]     new_device_type := public.get_device_type(NEW.user_agent);
...
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.linkage_type != 'none'
-- [HEADER]         AND (c.start_date + INTERVAL '1 year' = CURRENT_DATE OR c.start_date + INTERVAL '2 years' = CURRENT_DATE OR c.start_date + INTERVAL '3 years' = CURRENT_DATE)
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (SELECT 1 FROM public.notifications WHERE user_id = r.user_id AND metadata->>'contract_id' = r.id::text AND metadata->>'event' = 'index_update' AND metadata->>'date' = CURRENT_DATE::text) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
...
-- [HEADER]         original_user_id,
-- [HEADER]         email,
-- [HEADER]         subcription_status_at_deletion
-- [HEADER]     )
    VALUES (
-- [HEADER]         OLD.id,
-- [HEADER]         OLD.email,
-- [HEADER]         OLD.subscription_status::text
-- [HEADER]     );
-- [HEADER]     RETURN OLD;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
AS $$
DECLARE
-- [HEADER]     project_url text := 'https://mtxwavmmywiewjrsxchi.supabase.co'; -- Replace with your actual project URL or use a config table
-- [HEADER]     function_secret text := 'YOUR_FUNCTION_SECRET'; -- Ideally this is handled via vault or not needed if using net extension with service role
BEGIN
...
-- [HEADER]             'type', 'INSERT',
-- [HEADER]             'table', 'user_profiles',
-- [HEADER]             'record', row_to_json(NEW)
-- [HEADER]         )::jsonb
-- [HEADER]       );
...
;
DROP TRIGGER IF EXISTS "user_preferences_updated_at" ON CREATE;
...
-- [HEADER]     'fetch-index-data-daily', -- Job name
-- [HEADER]     '0 6 * * *',              -- Schedule (6:00 AM daily)
    $$
    SELECT
-- [HEADER]         net.http_post(
...
-- [HEADER]         WHERE tablename = 'index_data' 
-- [HEADER]         AND policyname = 'Allow public read access to index data'
-- [HEADER]     ) THEN
...
SELECT cron.schedule(
-- [HEADER]     'index-update-day15',
-- [HEADER]     '0 */2 15 * *',  -- Every 2 hours on day 15
    $$
    SELECT
-- [HEADER]         net.http_post(
-- [HEADER]             url := 'https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/fetch-index-data',
...
-- [HEADER]                 'Authorization', 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFmdnJla3Z1Z2RqbndobmF1Y216Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NzQzNjQxNiwiZXhwIjoyMDgzMDEyNDE2fQ._Fmq-2x4zpzPkHP9btdqSUj0gbX7RmqscwvGElNbdNA'
-- [HEADER]             ),
-- [HEADER]             body := '{}'::jsonb
-- [HEADER]         ) AS request_id;
    $$
...
SELECT cron.schedule(
-- [HEADER]     'index-update-day16',
-- [HEADER]     '0 */2 16 * *',  -- Every 2 hours on day 16
    $$
    SELECT
-- [HEADER]         net.http_post(
-- [HEADER]             url := 'https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/fetch-index-data',
...
SELECT cron.schedule(
-- [HEADER]     'index-update-day17',
-- [HEADER]     '0 */2 17 * *',  -- Every 2 hours on day 17
    $$
    SELECT
-- [HEADER]         net.http_post(
-- [HEADER]             url := 'https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/fetch-index-data',
...
-- [HEADER]         id, email, full_name, role, subscription_status, plan_id
-- [HEADER]     )
    VALUES (
-- [HEADER]         NEW.id,
-- [HEADER]         NEW.email,
...
-- [HEADER]         'user',
-- [HEADER]         'active',
-- [HEADER]         'free' -- Default to free plan
-- [HEADER]     );
-- [HEADER]     RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- [HEADER]     session_count INT;
-- [HEADER]     oldest_session_id UUID;
-- [HEADER]     user_plan_limit INT;
BEGIN
    -- 1. Get User's Plan Limit
    SELECT sp.max_sessions
-- [HEADER]     INTO user_plan_limit
-- [HEADER]     FROM public.user_profiles up
-- [HEADER]     JOIN public.subscription_plans sp ON up.plan_id = sp.id
-- [HEADER]     WHERE up.id = NEW.user_id;
...
-- [HEADER]     IF user_plan_limit = -1 THEN
-- [HEADER]         RETURN NEW;
    END IF;
...
-- [HEADER]     WHERE user_id = NEW.user_id;
    ...
    -- 4. Enforce Limit
-- [HEADER]     IF session_count >= user_plan_limit THEN
...
-- [HEADER]     subscription_status subscription_status,
-- [HEADER]     plan_id TEXT,
...
-- [HEADER]         up.full_name,
-- [HEADER]         up.role,
-- [HEADER]         up.subscription_status,
-- [HEADER]         up.plan_id,
-- [HEADER]         up.created_at,
...
-- [HEADER]     ) p ON up.id = p.user_id
    -- Join Tenant Counts
-- [HEADER]     LEFT JOIN (
        SELECT user_id, count(*) as count 
-- [HEADER]         FROM tenants 
-- [HEADER]         GROUP BY user_id
-- [HEADER]     ) t ON up.id = t.user_id
    -- Join Contract Counts
-- [HEADER]     LEFT JOIN (
        SELECT user_id, count(*) as count 
-- [HEADER]         FROM contracts 
-- [HEADER]         GROUP BY user_id
-- [HEADER]     ) c ON up.id = c.user_id
...
-- [HEADER]     IF target_user_id = auth.uid() THEN
...
AS $$
DECLARE
-- [HEADER]     cutoff_date TIMESTAMP WITH TIME ZONE;
-- [HEADER]     user_record RECORD;
BEGIN
    -- Calculate cutoff date (14 days ago)
-- [HEADER]     cutoff_date := NOW() - INTERVAL '14 days';
    
    -- Find all users marked for deletion more than 14 days ago
-- [HEADER]     FOR user_record IN 
        SELECT id 
-- [HEADER]         FROM user_profiles 
...
-- [HEADER]         auth.uid(), 
-- [HEADER]         'delete_user', 
-- [HEADER]         jsonb_build_object('target_user_id', target_user_id, 'target_email', target_email)
-- [HEADER]     );
...
AS $$
BEGIN
-- [HEADER]     IF (OLD.role IS DISTINCT FROM NEW.role) OR 
...
-- [HEADER]             'update_user_profile',
-- [HEADER]             jsonb_build_object(
-- [HEADER]                 'target_user_id', NEW.id,
-- [HEADER]                 'changes', jsonb_build_object(
-- [HEADER]                     'role', CASE WHEN OLD.role IS DISTINCT FROM NEW.role THEN jsonb_build_array(OLD.role, NEW.role) ELSE NULL END,
-- [HEADER]                     'plan_id', CASE WHEN OLD.plan_id IS DISTINCT FROM NEW.plan_id THEN jsonb_build_array(OLD.plan_id, NEW.plan_id) ELSE NULL END,
-- [HEADER]                     'status', CASE WHEN OLD.subscription_status IS DISTINCT FROM NEW.subscription_status THEN jsonb_build_array(OLD.subscription_status, NEW.subscription_status) ELSE NULL END
-- [HEADER]                 )
-- [HEADER]             )
-- [HEADER]         );
    END IF;
...
-- [HEADER]     p_category TEXT DEFAULT NULL
-- [HEADER] ) RETURNS BOOLEAN AS $$
DECLARE
-- [HEADER]     v_total_usage BIGINT;
-- [HEADER]     v_cat_usage BIGINT;
-- [HEADER]     v_max_total_mb INTEGER;
-- [HEADER]     v_max_cat_mb INTEGER;
-- [HEADER]     v_col_name TEXT;
BEGIN
    -- 1. Get current usage and plan limits
    SELECT 
-- [HEADER]         u.total_bytes,
-- [HEADER]         CASE 
-- [HEADER]             WHEN p_category IN ('photo', 'video') THEN u.media_bytes
-- [HEADER]             WHEN p_category LIKE 'utility_%' THEN u.utilities_bytes
-- [HEADER]             WHEN p_category = 'maintenance' THEN u.maintenance_bytes
-- [HEADER]             ELSE u.documents_bytes
-- [HEADER]         END,
-- [HEADER]         s.max_storage_mb,
-- [HEADER]         CASE 
-- [HEADER]             WHEN p_category IN ('photo', 'video') THEN s.max_media_mb
-- [HEADER]             WHEN p_category LIKE 'utility_%' THEN s.max_utilities_mb
-- [HEADER]             WHEN p_category = 'maintenance' THEN s.max_maintenance_mb
-- [HEADER]             ELSE s.max_documents_mb
        END
-- [HEADER]     INTO 
-- [HEADER]         v_total_usage,
-- [HEADER]         v_cat_usage,
-- [HEADER]         v_max_total_mb,
-- [HEADER]         v_max_cat_mb
-- [HEADER]     FROM user_profiles up
-- [HEADER]     JOIN subscription_plans s ON up.plan_id = s.id
-- [HEADER]     LEFT JOIN user_storage_usage u ON u.user_id = up.id
...
-- [HEADER]     IF v_max_total_mb != -1 AND (v_total_usage + p_file_size) > (v_max_total_mb * 1024 * 1024) THEN
-- [HEADER]         RETURN FALSE;
    END IF;

    -- 3. Check Category Limit (if specified and not unlimited)
-- [HEADER]     IF p_category IS NOT NULL AND v_max_cat_mb != -1 THEN
-- [HEADER]         IF (v_cat_usage + p_file_size) > (v_max_cat_mb * 1024 * 1024) THEN
-- [HEADER]             RETURN FALSE;
        END IF;
    END IF;
...
-- [HEADER]     id TEXT PRIMARY KEY, -- Short ID (e.g., "abc123")
-- [HEADER]     user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
-- [HEADER]     calculation_data JSONB NOT NULL,
-- [HEADER]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [HEADER]     expires_at TIMESTAMPTZ DEFAULT (NOW() + INTERVAL '30 days'),
-- [HEADER]     view_count INTEGER DEFAULT 0
-- [HEADER] );
...
RETURNS TEXT AS $$
DECLARE
-- [HEADER]     chars TEXT := 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
-- [HEADER]     result TEXT := '';
-- [HEADER]     i INTEGER;
BEGIN
-- [HEADER]     FOR i IN 1..length LOOP
-- [HEADER]         result := result || substr(chars, floor(random() * length(chars) + 1)::int, 1);
    END LOOP;
...
RETURNS TEXT AS $$
DECLARE
-- [HEADER]     v_short_id TEXT;
-- [HEADER]     v_max_attempts INTEGER := 10;
-- [HEADER]     v_attempt INTEGER := 0;
BEGIN
-- [HEADER]     LOOP
-- [HEADER]         v_short_id := generate_short_id(6);
...
-- [HEADER]             IF v_attempt >= v_max_attempts THEN
...
RETURNS INTEGER AS $$
DECLARE
-- [HEADER]     v_deleted_count INTEGER;
BEGIN
    DELETE FROM calculation_shares
-- [HEADER]     WHERE expires_at < NOW();
    
-- [HEADER]     GET DIAGNOSTICS v_deleted_count = ROW_COUNT;
...
-- [HEADER]         'photo',           -- Property photos
-- [HEADER]         'video',           -- Property videos
-- [HEADER]         'utility_water',   -- Water bills
-- [HEADER]         'utility_electric',-- Electric bills
-- [HEADER]         'utility_gas',     -- Gas bills
-- [HEADER]         'utility_municipality', -- Municipality bills (arnona)
-- [HEADER]         'utility_management',   -- Building management fees
-- [HEADER]         'maintenance',     -- Repair/maintenance records
-- [HEADER]         'invoice',         -- General invoices
-- [HEADER]         'receipt',         -- Payment receipts
-- [HEADER]         'insurance',       -- Insurance documents
-- [HEADER]         'warranty',        -- Warranty documents
-- [HEADER]         'legal',           -- Legal documents
...
-- [HEADER]             total_bytes = user_storage_usage.total_bytes + NEW.file_size,
...
-- [HEADER] FOR EACH ROW EXECUTE FUNCTION update_user_storage();
...
-- [HEADER] ) RETURNS BOOLEAN AS $$
DECLARE
-- [HEADER]     v_current_usage BIGINT;
-- [HEADER]     v_max_storage_mb INTEGER;
-- [HEADER]     v_max_storage_bytes BIGINT;
BEGIN
    -- Get current usage
    SELECT COALESCE(total_bytes, 0)
-- [HEADER]     INTO v_current_usage
-- [HEADER]     FROM user_storage_usage
...
-- [HEADER]     WHERE up.id = p_user_id;
    ...
    -- -1 means unlimited
-- [HEADER]     IF v_max_storage_mb = -1 THEN
-- [HEADER]         RETURN TRUE;
    END IF;
...
DECLARE
-- [HEADER]     v_col TEXT;
-- [HEADER]     v_size BIGINT;
-- [HEADER]     v_user_id UUID;
-- [HEADER]     v_cat TEXT;
BEGIN
-- [HEADER]     IF TG_OP = 'INSERT' THEN
-- [HEADER]         v_size := NEW.file_size;
-- [HEADER]         v_user_id := NEW.user_id;
-- [HEADER]         v_cat := NEW.category;
-- [HEADER]     ELSE
-- [HEADER]         v_size := OLD.file_size;
-- [HEADER]         v_user_id := OLD.user_id;
-- [HEADER]         v_cat := OLD.category;
    END IF;

    -- Determine which column to update based on category
-- [HEADER]     IF v_cat IN ('photo', 'video') THEN
...
-- [HEADER]                 updated_at = NOW()
-- [HEADER]         ', v_col, v_col, v_col) USING v_user_id, v_size;
...
-- [HEADER]         ', v_col, v_col) USING v_size, v_user_id;
    END IF;
...
RETURNS JSON AS $$
DECLARE
-- [HEADER]     v_usage RECORD;
-- [HEADER]     v_limit RECORD;
-- [HEADER]     v_user_tier TEXT;
-- [HEADER]     v_result JSON;
BEGIN
    -- Get user's subscription tier
    SELECT subscription_tier INTO v_user_tier
-- [HEADER]     FROM user_profiles
-- [HEADER]     WHERE id = p_user_id;
...
-- [HEADER]     IF v_usage.last_reset_at < DATE_TRUNC('month', NOW()) THEN
...
-- [HEADER]     IF v_limit.monthly_message_limit != -1 AND v_usage.message_count >= v_limit.monthly_message_limit THEN
-- [HEADER]         v_result := json_build_object(
-- [HEADER]             'allowed', false,
-- [HEADER]             'reason', 'message_limit_exceeded',
-- [HEADER]             'current_usage', v_usage.message_count,
-- [HEADER]             'limit', v_limit.monthly_message_limit,
-- [HEADER]             'tier', v_user_tier
-- [HEADER]         );
-- [HEADER]         RETURN v_result;
    END IF;
    
-- [HEADER]     IF v_limit.monthly_token_limit != -1 AND v_usage.tokens_used >= v_limit.monthly_token_limit THEN
-- [HEADER]         v_result := json_build_object(
-- [HEADER]             'allowed', false,
-- [HEADER]             'reason', 'token_limit_exceeded',
-- [HEADER]             'current_usage', v_usage.tokens_used,
-- [HEADER]             'limit', v_limit.monthly_token_limit,
-- [HEADER]             'tier', v_user_tier
-- [HEADER]         );
-- [HEADER]         RETURN v_result;
    END IF;
...
-- [HEADER]     v_result := json_build_object(
-- [HEADER]         'allowed', true,
-- [HEADER]         'current_messages', v_usage.message_count + 1,
-- [HEADER]         'message_limit', v_limit.monthly_message_limit,
-- [HEADER]         'current_tokens', v_usage.tokens_used + p_tokens_used,
-- [HEADER]         'token_limit', v_limit.monthly_token_limit,
-- [HEADER]         'tier', v_user_tier
-- [HEADER]     );
...
AS $$
DECLARE
-- [HEADER]     expiring_contract RECORD;
-- [HEADER]     count_new integer := 0;
-- [HEADER]     pref_days integer;
BEGIN
-- [HEADER]     FOR expiring_contract IN
        SELECT 
-- [HEADER]             c.id, 
-- [HEADER]             c.end_date, 
-- [HEADER]             c.property_id, 
-- [HEADER]             p.user_id, 
-- [HEADER]             p.address, 
-- [HEADER]             p.city,
-- [HEADER]             up.notification_preferences
-- [HEADER]         FROM public.contracts c
-- [HEADER]         JOIN public.properties p ON c.property_id = p.id
-- [HEADER]         JOIN public.user_profiles up ON p.user_id = up.id
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]     LOOP
        -- Extract preference, default to 60, cap at 180
-- [HEADER]         pref_days := COALESCE((expiring_contract.notification_preferences->>'contract_expiry_days')::int, 60);
-- [HEADER]         IF pref_days > 180 THEN pref_days := 180; END IF;
-- [HEADER]         IF pref_days < 1 THEN pref_days := 1; END IF;

        -- Check if contract expires in this window
-- [HEADER]         IF expiring_contract.end_date <= (CURRENT_DATE + (pref_days || ' days')::interval)
-- [HEADER]            AND expiring_contract.end_date >= CURRENT_DATE THEN
           
-- [HEADER]             IF NOT EXISTS (
                SELECT 1 
-- [HEADER]                 FROM public.notifications n 
-- [HEADER]                 WHERE n.user_id = expiring_contract.user_id
-- [HEADER]                 AND n.type = 'warning'
-- [HEADER]                 AND n.metadata->>'contract_id' = expiring_contract.id::text
...
-- [HEADER]                  AND n.created_at > (CURRENT_DATE - INTERVAL '6 months') -- Simple debounce for same contract
-- [HEADER]             ) THEN
...
-- [HEADER]                     metadata
-- [HEADER]                 ) VALUES (
-- [HEADER]                     expiring_contract.user_id,
-- [HEADER]                     'warning',
-- [HEADER]                     'Contract Expiring Soon',
-- [HEADER]                     'Contract for ' || expiring_contract.address || ' ends in ' || (expiring_contract.end_date - CURRENT_DATE)::text || ' days (' || to_char(expiring_contract.end_date, 'DD/MM/YYYY') || '). Review and renew today.',
-- [HEADER]                     jsonb_build_object('contract_id', expiring_contract.id)
-- [HEADER]                 );
...
AS $$
DECLARE
-- [HEADER]     due_payment RECORD;
-- [HEADER]     count_new integer := 0;
-- [HEADER]     pref_days integer;
BEGIN
-- [HEADER]     FOR due_payment IN
        SELECT 
-- [HEADER]             pay.id,
-- [HEADER]             pay.due_date,
-- [HEADER]             pay.amount,
...
-- [HEADER]             p.address,
-- [HEADER]             up.notification_preferences
-- [HEADER]         FROM public.payments pay
-- [HEADER]         JOIN public.contracts c ON pay.contract_id = c.id
-- [HEADER]         JOIN public.properties p ON c.property_id = p.id
-- [HEADER]         JOIN public.user_profiles up ON p.user_id = up.id
-- [HEADER]         WHERE pay.status = 'pending'
-- [HEADER]     LOOP
        -- Extract preference, default to 3, cap at 180 (though less makes sense for rent)
-- [HEADER]         pref_days := COALESCE((due_payment.notification_preferences->>'rent_due_days')::int, 3);
-- [HEADER]         IF pref_days > 180 THEN pref_days := 180; END IF;

-- [HEADER]         IF due_payment.due_date <= (CURRENT_DATE + (pref_days || ' days')::interval)
-- [HEADER]            AND due_payment.due_date >= CURRENT_DATE THEN

-- [HEADER]             IF NOT EXISTS (
                SELECT 1 
-- [HEADER]                 FROM public.notifications n 
-- [HEADER]                 WHERE n.user_id = due_payment.user_id
-- [HEADER]                 AND n.type = 'info'
-- [HEADER]                 AND n.metadata->>'payment_id' = due_payment.id::text
-- [HEADER]             ) THEN
                INSERT INTO public.notifications (
-- [HEADER]                     user_id,
-- [HEADER]                     type,
...
-- [HEADER]                     metadata
-- [HEADER]                 ) VALUES (
-- [HEADER]                     due_payment.user_id,
-- [HEADER]                     'info',
-- [HEADER]                     'Rent Due Soon',
-- [HEADER]                     'Rent of ' || due_payment.amount || ' ' || due_payment.currency || ' for ' || due_payment.address || ' is due on ' || to_char(due_payment.due_date, 'DD/MM/YYYY') || '.',
-- [HEADER]                     jsonb_build_object('payment_id', due_payment.id)
-- [HEADER]                 );
-- [HEADER]                 count_new := count_new + 1;
            END IF;
        END IF;
...
-- [HEADER]         'media', COUNT(*) FILTER (WHERE category IN ('photo', 'video')),
-- [HEADER]         'utilities', COUNT(*) FILTER (WHERE category LIKE 'utility_%'),
-- [HEADER]         'maintenance', COUNT(*) FILTER (WHERE category = 'maintenance'),
-- [HEADER]         'documents', COUNT(*) FILTER (WHERE category NOT IN ('photo', 'video', 'maintenance') AND category NOT LIKE 'utility_%')
-- [HEADER]     ) INTO result
-- [HEADER]     FROM public.property_documents
-- [HEADER]     WHERE user_id = p_user_id;
...
AS $$
DECLARE
-- [HEADER]     income_stats RECORD;
-- [HEADER]     doc_counts JSONB;
BEGIN
    -- 1. Get Income Stats
    SELECT 
-- [HEADER]         COALESCE(SUM(amount) FILTER (WHERE status = 'paid'), 0) as collected,
-- [HEADER]         COALESCE(SUM(amount) FILTER (WHERE status = 'pending'), 0) as pending,
-- [HEADER]         COALESCE(SUM(amount) FILTER (WHERE status IN ('paid', 'pending')), 0) as total
-- [HEADER]     INTO income_stats
-- [HEADER]     FROM public.payments
-- [HEADER]     WHERE user_id = p_user_id
-- [HEADER]     AND due_date >= date_trunc('month', now())
-- [HEADER]     AND due_date < date_trunc('month', now() + interval '1 month');
...
-- [HEADER]         'income', jsonb_build_object(
-- [HEADER]             'collected', income_stats.collected,
-- [HEADER]             'pending', income_stats.pending,
-- [HEADER]             'monthlyTotal', income_stats.total
-- [HEADER]         ),
-- [HEADER]         'storage', doc_counts,
...
-- [HEADER]         WHERE c.status = 'active'
        -- Changed to 60 days
-- [HEADER]         AND c.end_date <= (CURRENT_DATE + INTERVAL '60 days')
-- [HEADER]         AND c.end_date >= CURRENT_DATE
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 
-- [HEADER]             FROM public.notifications n 
...
-- [HEADER]             AND n.type = 'warning'
-- [HEADER]             AND n.metadata->>'contract_id' = expiring_contract.id::text
-- [HEADER]             AND n.title = 'Contract Expiring Soon' 
-- [HEADER]         ) THEN
            INSERT INTO public.notifications (
-- [HEADER]                 user_id,
-- [HEADER]                 type,
...
-- [HEADER]         JOIN public.properties p ON c.property_id = p.id
-- [HEADER]         WHERE pay.status = 'pending'
-- [HEADER]         AND pay.due_date <= (CURRENT_DATE + INTERVAL '3 days')
-- [HEADER]         AND pay.due_date >= CURRENT_DATE
-- [HEADER]     LOOP
...
-- [HEADER]     '{extension_option_end_days}',
-- [HEADER]     '7'
-- [HEADER] )
...
AS $$
DECLARE
-- [HEADER]     deadline_record RECORD;
-- [HEADER]     count_new integer := 0;
-- [HEADER]     pref_days integer;
BEGIN
-- [HEADER]     FOR deadline_record IN
        SELECT 
-- [HEADER]             c.id, 
-- [HEADER]             c.extension_option_end,
-- [HEADER]             c.property_id, 
-- [HEADER]             p.user_id, 
-- [HEADER]             p.address,
-- [HEADER]             up.notification_preferences
...
-- [HEADER]         JOIN public.user_profiles up ON p.user_id = up.id
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.extension_option_end IS NOT NULL
-- [HEADER]     LOOP
        -- Extract preference, default to 7, cap at 180
-- [HEADER]         pref_days := COALESCE((deadline_record.notification_preferences->>'extension_option_end_days')::int, 7);
        
        -- Skip if disabled (0)
-- [HEADER]         IF pref_days = 0 THEN
-- [HEADER]             CONTINUE;
        END IF;
...
-- [HEADER]         IF deadline_record.extension_option_end <= (CURRENT_DATE + (pref_days || ' days')::interval)
-- [HEADER]            AND deadline_record.extension_option_end >= CURRENT_DATE THEN
           
-- [HEADER]             IF NOT EXISTS (
                SELECT 1 
-- [HEADER]                 FROM public.notifications n 
-- [HEADER]                 WHERE n.user_id = deadline_record.user_id
-- [HEADER]                 AND n.type = 'warning'
-- [HEADER]                 AND n.metadata->>'contract_id' = deadline_record.id::text
-- [HEADER]                 AND n.title = 'Extension Option Deadline Approaching'
-- [HEADER]                 AND n.created_at > (CURRENT_DATE - INTERVAL '6 months')
-- [HEADER]             ) THEN
                INSERT INTO public.notifications (
-- [HEADER]                     user_id,
-- [HEADER]                     type,
...
-- [HEADER]                     metadata
-- [HEADER]                 ) VALUES (
-- [HEADER]                     deadline_record.user_id,
-- [HEADER]                     'warning',
-- [HEADER]                     'Extension Option Deadline Approaching',
-- [HEADER]                     'Deadline to announce extension option for ' || deadline_record.address || ' is in ' || (deadline_record.extension_option_end - CURRENT_DATE)::text || ' days (' || to_char(deadline_record.extension_option_end, 'DD/MM/YYYY') || '). Contact tenant soon.',
-- [HEADER]                     jsonb_build_object('contract_id', deadline_record.id)
-- [HEADER]                 );
-- [HEADER]                 count_new := count_new + 1;
            END IF;
        END IF;
...
AS $$
DECLARE
-- [HEADER]     extension_record RECORD;
-- [HEADER]     count_new integer := 0;
-- [HEADER]     pref_days integer;
BEGIN
-- [HEADER]     FOR extension_record IN
        SELECT 
-- [HEADER]             c.id, 
-- [HEADER]             c.extension_option_start,
-- [HEADER]             c.property_id, 
-- [HEADER]             p.user_id, 
-- [HEADER]             p.address,
-- [HEADER]             up.notification_preferences
...
-- [HEADER]         JOIN public.user_profiles up ON p.user_id = up.id
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.extension_option_start IS NOT NULL
-- [HEADER]     LOOP
        -- Extract preference, default to 30, cap at 180
-- [HEADER]         pref_days := COALESCE((extension_record.notification_preferences->>'extension_option_days')::int, 30);
-- [HEADER]         IF pref_days > 180 THEN pref_days := 180; END IF;
-- [HEADER]         IF pref_days < 1 THEN pref_days := 1; END IF;

        -- Check if extension option starts in this window
-- [HEADER]         IF extension_record.extension_option_start <= (CURRENT_DATE + (pref_days || ' days')::interval)
-- [HEADER]            AND extension_record.extension_option_start >= CURRENT_DATE THEN
           
-- [HEADER]             IF NOT EXISTS (
                SELECT 1 
-- [HEADER]                 FROM public.notifications n 
-- [HEADER]                 WHERE n.user_id = extension_record.user_id
-- [HEADER]                 AND n.type = 'info'
-- [HEADER]                 AND n.metadata->>'contract_id' = extension_record.id::text
-- [HEADER]                 AND n.title = 'Extension Option Available'
-- [HEADER]                 AND n.created_at > (CURRENT_DATE - INTERVAL '6 months')
-- [HEADER]             ) THEN
                INSERT INTO public.notifications (
-- [HEADER]                     user_id,
...
-- [HEADER]                     metadata
-- [HEADER]                 ) VALUES (
-- [HEADER]                     extension_record.user_id,
-- [HEADER]                     'info',
-- [HEADER]                     'Extension Option Available',
-- [HEADER]                     'Extension option period for ' || extension_record.address || ' starts in ' || (extension_record.extension_option_start - CURRENT_DATE)::text || ' days (' || to_char(extension_record.extension_option_start, 'DD/MM/YYYY') || '). Consider discussing with tenant.',
-- [HEADER]                     jsonb_build_object('contract_id', extension_record.id)
-- [HEADER]                 );
-- [HEADER]                 count_new := count_new + 1;
            END IF;
        END IF;
...
-- [HEADER]                 'warning',
-- [HEADER]                 'Contract Expiring Soon',
-- [HEADER]                 'The contract for ' || expiring_contract.address || ', ' || expiring_contract.city || ' ends on ' || to_char(expiring_contract.end_date, 'YYYY-MM-DD') || '.',
-- [HEADER]                 jsonb_build_object('contract_id', expiring_contract.id)
-- [HEADER]             );
...
AS $$
DECLARE
-- [HEADER]     v_limit INTEGER;
-- [HEADER]     v_current_usage INTEGER;
-- [HEADER]     v_month_start TIMESTAMPTZ;
-- [HEADER]     v_requester_role TEXT;
BEGIN
    -- SECURITY CHECK: 
...
-- [HEADER]     IF p_user_id != auth.uid() AND COALESCE(v_requester_role, 'user') != 'admin' THEN
...
-- [HEADER]       AND created_at >= v_month_start;
    ...
    -- 3. Check if allowed
-- [HEADER]     IF v_limit = -1 OR (v_current_usage + p_count) <= v_limit THEN
...
-- [HEADER]         RETURN jsonb_build_object(
-- [HEADER]             'allowed', true,
-- [HEADER]             'current_usage', v_current_usage + p_count,
-- [HEADER]             'limit', v_limit
-- [HEADER]         );
-- [HEADER]     ELSE
-- [HEADER]         RETURN jsonb_build_object(
-- [HEADER]             'allowed', false,
-- [HEADER]             'current_usage', v_current_usage,
-- [HEADER]             'limit', v_limit
-- [HEADER]         );
    END IF;
END;
...
AS $$
DECLARE
-- [HEADER]     total_mrr decimal := 0;
-- [HEADER]     total_users int := 0;
-- [HEADER]     active_subs int := 0;
-- [HEADER]     new_users_30d int := 0;
-- [HEADER]     churn_rate decimal := 0; -- Placeholder for now
...
-- [HEADER]         'mrr', total_mrr,
-- [HEADER]         'total_users', total_users,
-- [HEADER]         'active_subscribers', active_subs,
-- [HEADER]         'new_users_30d', new_users_30d,
-- [HEADER]         'churn_rate', 0 -- TODO: Implement churn logic later
-- [HEADER]     );
END;
$$;
-- ============================================
...
DECLARE
-- [HEADER]     result JSON;
-- [HEADER]     total_users_count INTEGER;
-- [HEADER]     total_contracts_count INTEGER;
-- [HEADER]     total_revenue_amount NUMERIC;
-- [HEADER]     active_users_count INTEGER;
BEGIN
...
-- [HEADER]         'totalUsers', total_users_count,
-- [HEADER]         'totalContracts', total_contracts_count,
-- [HEADER]         'totalRevenue', total_revenue_amount,
-- [HEADER]         'activeUsers', active_users_count
-- [HEADER]     );
...
DO $$ 
BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_profiles') THEN
...
AS $$
DECLARE
-- [HEADER]     project_url text := 'https://tipnjnfbbnbskdlodrww.supabase.co'; -- UPDATED TO CORRECT PROJECT
BEGIN
    PERFORM
-- [HEADER]       net.http_post(
-- [HEADER]         url := project_url || '/functions/v1/send-admin-alert',
...
AS $$
DECLARE
-- [HEADER]     project_url text := 'https://tipnjnfbbnbskdlodrww.supabase.co';
-- [HEADER]     user_email text;
BEGIN
    -- Only forward high-priority or action-oriented types
-- [HEADER]     IF NEW.type IN ('warning', 'error', 'urgent', 'action') THEN
        -- Get user email
...
-- [HEADER]                     'notification', row_to_json(NEW)
-- [HEADER]                 )::jsonb
-- [HEADER]               );
        END IF;
    END IF;
...
-- [HEADER]         WHERE tgname = 'on_auth_user_created'
-- [HEADER]     ) THEN
...
-- [HEADER]     IF is_super IS NOT TRUE THEN RAISE EXCEPTION 'Access Denied: Super Admin Only'; END IF;
...
-- [HEADER]         'active_subscribers', active_subs,
-- [HEADER]         'new_users_30d', new_users_30d,
-- [HEADER]         'storage', json_build_object(
-- [HEADER]             'total_mb', ROUND(total_storage_mb, 2),
-- [HEADER]             'media_mb', ROUND(media_storage_mb, 2),
-- [HEADER]             'docs_mb', ROUND(docs_storage_mb, 2)
-- [HEADER]         ),
-- [HEADER]         'system_status', json_build_object(
-- [HEADER]             'maintenance_mode', COALESCE(is_maint_active, false),
-- [HEADER]             'ai_disabled', COALESCE(is_ai_disabled, false)
-- [HEADER]         )
-- [HEADER]     );
END;
$$;
...
-- [HEADER]         NEW.email,
-- [HEADER]         COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)),
-- [HEADER]         COALESCE(NEW.raw_user_meta_data->>'first_name', split_part(NEW.email, '@', 1)),
-- [HEADER]         COALESCE(NEW.raw_user_meta_data->>'last_name', ''),
-- [HEADER]         'User',
-- [HEADER]         'active',
-- [HEADER]         'free',
...
-- [HEADER]         first_name = COALESCE(EXCLUDED.first_name, user_profiles.first_name),
-- [HEADER]         last_name = COALESCE(EXCLUDED.last_name, user_profiles.last_name),
-- [HEADER]         marketing_consent = COALESCE(EXCLUDED.marketing_consent, user_profiles.marketing_consent),
-- [HEADER]         marketing_consent_at = COALESCE(EXCLUDED.marketing_consent_at, user_profiles.marketing_consent_at),
-- [HEADER]         updated_at = NOW();
...
-- [HEADER]     IF TG_OP = 'INSERT' THEN
        PERFORM
-- [HEADER]           net.http_post(
-- [HEADER]             url := project_url || '/functions/v1/send-welcome-email',
-- [HEADER]             headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || current_setting('app.settings.service_role_key', true) || '"}',
-- [HEADER]             body := json_build_object(
-- [HEADER]                 'email', NEW.email,
-- [HEADER]                 'full_name', NEW.full_name
-- [HEADER]             )::jsonb
-- [HEADER]           );
    END IF;
...
DO $$ 
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
-- [HEADER]                    WHERE table_name='property_documents' AND column_name='counter_read') THEN
...
DECLARE
-- [HEADER]     property_address text;
-- [HEADER]     user_lang text;
-- [HEADER]     notif_title text;
-- [HEADER]     notif_message text;
BEGIN
    -- Only trigger for maintenance category
-- [HEADER]     IF NEW.category != 'maintenance' THEN
-- [HEADER]         RETURN NEW;
    END IF;
...
-- [HEADER]         'info',
-- [HEADER]         notif_title,
-- [HEADER]         notif_message,
-- [HEADER]         json_build_object(
-- [HEADER]             'document_id', NEW.id,
-- [HEADER]             'property_id', NEW.property_id,
-- [HEADER]             'event', 'maintenance_record'
-- [HEADER]         )::jsonb
-- [HEADER]     );
...
-- [HEADER]     IF (NEW.type IN ('warning', 'error', 'urgent', 'action')) OR 
...
-- [HEADER]         FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE;
    END IF;
END $$;
...
-- [HEADER]         WHERE table_name = 'subscription_plans' AND column_name = 'price_yearly'
-- [HEADER]     ) THEN
...
-- [HEADER]     IF NOT EXISTS (
        SELECT 1 
-- [HEADER]         FROM information_schema.table_constraints 
-- [HEADER]         WHERE constraint_name = 'ai_chat_usage_user_id_profiles_fkey'
-- [HEADER]     ) THEN
        ALTER TABLE ai_chat_usage 
-- [HEADER]         ADD CONSTRAINT ai_chat_usage_user_id_profiles_fkey 
-- [HEADER]         FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE;
    END IF;
END $$;
...
-- [HEADER]         headers := jsonb_build_object(
-- [HEADER]           'Content-Type', 'application/json',
-- [HEADER]           'Authorization', 'Bearer ' || COALESCE(current_setting('app.settings.service_role_key', true), 'DUMMY_KEY')
-- [HEADER]         ),
-- [HEADER]         body := jsonb_build_object(
-- [HEADER]           'type', TG_ARGV[0],
-- [HEADER]           'data', CASE 
-- [HEADER]                     WHEN TG_ARGV[0] = 'new_user' THEN jsonb_build_object('email', NEW.email, 'full_name', NEW.full_name)
-- [HEADER]                     WHEN TG_ARGV[0] = 'first_payment' THEN jsonb_build_object('email', (SELECT email FROM user_profiles WHERE id = NEW.user_id), 'amount', NEW.paid_amount, 'plan', NEW.plan_id)
-- [HEADER]                     ELSE '{}'::jsonb
                  END
-- [HEADER]         )
-- [HEADER]       );
...
-- [HEADER]     FROM payments
-- [HEADER]     WHERE user_id = NEW.user_id
-- [HEADER]     AND status = 'paid';

-- [HEADER]     IF payment_count = 1 THEN
...
SELECT cron.schedule(
-- [HEADER]     'daily-summary-8am',
-- [HEADER]     '0 8 * * *', -- 8:00 AM every day
    $$
...
-- [HEADER]     IF (TG_OP = 'INSERT') OR (TG_OP = 'UPDATE' AND OLD.subscription_plan IS DISTINCT FROM NEW.subscription_plan) THEN
        PERFORM
-- [HEADER]           net.http_post(
-- [HEADER]             url := project_url || '/functions/v1/send-admin-alert',
-- [HEADER]             headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || current_setting('app.settings.service_role_key', true) || '"}',
...
-- [HEADER]                 'table', 'user_profiles',
-- [HEADER]                 'record', row_to_json(NEW),
-- [HEADER]                 'old_record', CASE WHEN TG_OP = 'UPDATE' THEN row_to_json(OLD) ELSE NULL END
-- [HEADER]             )::jsonb
-- [HEADER]           );
    END IF;
...
-- [HEADER]     IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
...
-- [HEADER]                 'record', row_to_json(NEW),
-- [HEADER]                 'user', row_to_json(user_record)
-- [HEADER]             )::jsonb
-- [HEADER]           );
    END IF;
...
DROP TRIGGER IF EXISTS "on_invoice_paid_notify_admin" ON Starts;
...
-- [HEADER]     WHERE email = 'rubi@rentmate.co.il';
    ...
    -- But let's be safe.
-- [HEADER]     IF EXISTS (SELECT 1 FROM auth.users WHERE email = 'rubi@rentmate.co.il') THEN
...
DO $$ 
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'support_tickets' AND column_name = 'assigned_to') THEN
        ALTER TABLE public.support_tickets ADD COLUMN IF NOT EXISTS assigned_to UUID REFERENCES auth.users(id) ON DELETE SET NULL;
    END IF;
    
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'support_tickets' AND column_name = 'chat_context') THEN
        ALTER TABLE public.support_tickets ADD COLUMN IF NOT EXISTS chat_context JSONB;
    END IF;

-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'support_tickets' AND column_name = 'resolution_notes') THEN
        ALTER TABLE public.support_tickets ADD COLUMN IF NOT EXISTS resolution_notes TEXT;
    END IF;

-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'support_tickets' AND column_name = 'resolved_at') THEN
...
BEGIN
    ...
    VALUES (
-- [HEADER]         'support_ticket',
-- [HEADER]         NEW.user_id,
-- [HEADER]         jsonb_build_object(
-- [HEADER]             'ticket_id', NEW.id,
-- [HEADER]             'title', NEW.title,
-- [HEADER]             'category', NEW.category,
...
DECLARE
-- [HEADER]     r RECORD;
-- [HEADER]     extension_days_default int := 60;
-- [HEADER]     pref jsonb;
BEGIN
    -------------------------------------------------------
...
-- [HEADER]         FROM public.contracts c
-- [HEADER]         JOIN public.properties p ON p.id = c.property_id
-- [HEADER]         JOIN public.user_profiles up ON up.id = c.user_id
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.end_date = CURRENT_DATE + (COALESCE((up.notification_preferences->>'contract_expiry_days')::int, 30) || ' days')::INTERVAL
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
-- [HEADER]             AND metadata->>'contract_id' = r.id::text 
-- [HEADER]             AND metadata->>'event' = 'ending_soon'
-- [HEADER]             AND created_at > (CURRENT_DATE - INTERVAL '1 day')
-- [HEADER]         ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [HEADER]                 r.user_id,
-- [HEADER]                 'warning',
-- [HEADER]                 'Contract Ending Soon',
-- [HEADER]                 format('Contract for %s, %s ends in %s days.', r.city, r.address, COALESCE((r.notification_preferences->>'contract_expiry_days')::int, 30)),
-- [HEADER]                 json_build_object('contract_id', r.id, 'event', 'ending_soon')::jsonb
-- [HEADER]             );
        END IF;
    END LOOP;
...
-- [HEADER]         WHERE c.status = 'active'
-- [HEADER]         AND c.extension_option = TRUE
-- [HEADER]         AND c.end_date = CURRENT_DATE + (COALESCE((up.notification_preferences->>'extension_option_end_days')::int, 60) || ' days')::INTERVAL
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
...
-- [HEADER]                 'action',
-- [HEADER]                 'Extension Deadline Approaching',
-- [HEADER]                 format('Extension option for %s, %s ends in %s days.', r.city, r.address, COALESCE((r.notification_preferences->>'extension_option_end_days')::int, 60)),
-- [HEADER]                 json_build_object('contract_id', r.id, 'event', 'extension_deadline')::jsonb
-- [HEADER]             );
        END IF;
    END LOOP;
...
-- [HEADER]         JOIN public.contracts c ON c.id = py.contract_id
-- [HEADER]         JOIN public.properties p ON p.id = c.property_id
-- [HEADER]         JOIN public.user_profiles up ON up.id = py.user_id
-- [HEADER]         WHERE py.status = 'pending'
-- [HEADER]         AND py.date = CURRENT_DATE + (COALESCE((up.notification_preferences->>'rent_due_days')::int, 0) || ' days')::INTERVAL
-- [HEADER]         AND (up.notification_preferences->>'rent_due_days')::int > 0
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
-- [HEADER]             AND metadata->>'payment_id' = r.id::text 
-- [HEADER]             AND metadata->>'event' = 'payment_warning'
-- [HEADER]             AND created_at > (CURRENT_DATE - INTERVAL '1 day')
-- [HEADER]         ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
...
-- [HEADER]                 format('Payment of ג‚×%s for %s, %s is due in %s days.', r.amount, r.city, r.address, (r.notification_preferences->>'rent_due_days')::int),
-- [HEADER]                 json_build_object('payment_id', r.id, 'event', 'payment_warning')::jsonb
-- [HEADER]             );
        END IF;
    END LOOP;
...
-- [HEADER]         WHERE py.status = 'pending'
-- [HEADER]         AND py.date = CURRENT_DATE
-- [HEADER]         AND COALESCE((up.notification_preferences->>'rent_due_today')::boolean, true) = true
-- [HEADER]     LOOP
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [HEADER]             WHERE user_id = r.user_id 
...
-- [HEADER]     IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [HEADER]         WHERE table_name = 'user_profiles' AND column_name = 'subscription_tier'
-- [HEADER]     ) THEN
...
RETURNS TRIGGER AS $$
BEGIN
-- [HEADER]     NEW.subscription_tier := NEW.plan_id;
-- [HEADER]     RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- [HEADER]         up.full_name,
-- [HEADER]         up.role::TEXT,
-- [HEADER]         up.subscription_status::TEXT,
-- [HEADER]         up.plan_id,
-- [HEADER]         up.created_at,
-- [HEADER]         COALESCE(p.count, 0) as properties_count,
-- [HEADER]         COALESCE(t.count, 0) as tenants_count,
...
BEGIN
    -- 1. PROPERTIES
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'properties') THEN
...
-- [HEADER]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    ...
    -- 2. TENANTS
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenants') THEN
...
-- [HEADER]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    ...
    -- 3. CONTRACTS
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'contracts') THEN
...
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'admin_notifications') THEN
...
-- [HEADER]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    ...
    -- 5. SUPPORT_TICKETS
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'support_tickets') THEN
...
-- [HEADER]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    ...
    -- 6. TICKET_COMMENTS
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ticket_comments') THEN
...
-- [HEADER]     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'property_documents') THEN
...
AS $$
DECLARE
-- [HEADER]     v_cost_input NUMERIC;
-- [HEADER]     v_cost_output NUMERIC;
-- [HEADER]     v_total_cost NUMERIC;
BEGIN
    -- Determine costs based on model
    -- Prices per 1M tokens
-- [HEADER]     IF p_model LIKE 'gpt-4o-mini%' THEN
-- [HEADER]         v_cost_input := 0.15;
-- [HEADER]         v_cost_output := 0.60;
-- [HEADER]     ELSIF p_model LIKE 'gpt-4o%' THEN
-- [HEADER]         v_cost_input := 2.50;
-- [HEADER]         v_cost_output := 10.00;
-- [HEADER]     ELSE
...
-- [HEADER]     total_revenue_amount NUMERIC;
-- [HEADER]     active_users_count INTEGER;
-- [HEADER]     total_ai_cost_usd NUMERIC;
BEGIN
    -- Check if the current user is an admin
-- [HEADER]     IF NOT EXISTS (
        SELECT 1 FROM user_profiles
...
-- [HEADER]         'totalRevenue', total_revenue_amount,
-- [HEADER]         'activeUsers', active_users_count,
-- [HEADER]         'totalAiCost', total_ai_cost_usd
-- [HEADER]     );
...
AS $$
DECLARE
-- [HEADER]     v_conv_id UUID;
-- [HEADER]     v_final_user_id UUID;
BEGIN
    -- Determine user ID: prefer explicit, fallback to auth.uid()
-- [HEADER]     v_final_user_id := COALESCE(p_user_id, auth.uid());
...
-- [HEADER]     status TEXT DEFAULT 'active' CHECK (status IN ('active', 'closed')),
...
-- [HEADER]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [HEADER]     conversation_id UUID REFERENCES public.human_conversations(id) ON DELETE CASCADE,
-- [HEADER]     sender_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
-- [HEADER]     sender_role TEXT CHECK (sender_role IN ('user', 'admin')),
...
-- [HEADER] FOR SELECT TO authenticated
USING (auth.uid() = user_id);
...
-- [HEADER] FOR ALL TO authenticated
USING (
-- [HEADER]     EXISTS (
...
DO $$ 
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_preferences' AND column_name = 'ai_data_consent') THEN
        ALTER TABLE user_preferences 
-- [HEADER]         ADD COLUMN IF NOT EXISTS ai_data_consent BOOLEAN DEFAULT false;
    END IF;
END $$;
...
DO $$ 
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = '00000000-0000-0000-0000-000000000000' OR email = 'guest-leads@rentmate.co.il') THEN
        INSERT INTO auth.users (id, email, raw_user_meta_data, created_at)
        VALUES (
-- [HEADER]           '00000000-0000-0000-0000-000000000000', 
-- [HEADER]           'guest-leads@rentmate.co.il', 
-- [HEADER]           '{"full_name": "Potential Lead"}'::jsonb, 
-- [HEADER]           NOW()
-- [HEADER]         );
    END IF;
END $$;
...
DO $$ 
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM public.user_profiles WHERE id = '00000000-0000-0000-0000-000000000000' OR email = 'guest-leads@rentmate.co.il') THEN
        INSERT INTO public.user_profiles (id, email, full_name, first_name, last_name, role)
        VALUES (
-- [HEADER]           '00000000-0000-0000-0000-000000000000', 
-- [HEADER]           'guest-leads@rentmate.co.il', 
-- [HEADER]           'Potential Lead', 
-- [HEADER]           'Potential',
-- [HEADER]           'Lead',
-- [HEADER]           'user'
-- [HEADER]         );
...
-- [HEADER]     active_users_count INTEGER;
-- [HEADER]     total_ai_cost_usd NUMERIC;
-- [HEADER]     total_automated_actions INTEGER;
BEGIN
    -- Check if the current user is an admin
-- [HEADER]     IF NOT EXISTS (
        SELECT 1 FROM user_profiles
...
-- [HEADER]         'activeUsers', active_users_count,
-- [HEADER]         'totalAiCost', total_ai_cost_usd,
-- [HEADER]         'totalAutomatedActions', total_automated_actions
-- [HEADER]     );
...
-- [HEADER]      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_profiles' AND column_name = 'plan_id') THEN
...
AS $$
DECLARE
-- [HEADER]     default_plan_id TEXT := 'free';
BEGIN
    -- Verify plan exists, fallback to NULL if 'free' is missing (to prevent crash)
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM public.subscription_plans WHERE id = default_plan_id) THEN
...
-- [HEADER]     FOR UPDATE TO authenticated
    USING (auth.uid() = user_id);
...
-- [HEADER]     FOR INSERT TO authenticated
    WITH CHECK (auth.uid() = user_id);
...
BEGIN
-- [HEADER]   payload := jsonb_build_object(
-- [HEADER]     'type', TG_OP,
-- [HEADER]     'table', TG_TABLE_NAME,
-- [HEADER]     'record', row_to_json(NEW),
-- [HEADER]     'old_record', CASE WHEN TG_OP = 'UPDATE' THEN row_to_json(OLD) ELSE NULL END
-- [HEADER]   );
...
-- [HEADER]       headers := jsonb_build_object(
-- [HEADER]         'Content-Type', 'application/json',
-- [HEADER]         'Authorization', 'Bearer ' || (SELECT value FROM system_settings WHERE key = 'supabase_service_role_key')
-- [HEADER]       ),
-- [HEADER]       body := payload
-- [HEADER]     );
...
-- [HEADER] FOR EACH ROW EXECUTE FUNCTION public.handle_automated_engagement_webhook();
...
-- [HEADER]     total_revenue_amount NUMERIC;
-- [HEADER]     active_users_count INTEGER;
-- [HEADER]     total_ai_cost NUMERIC;
-- [HEADER]     automated_actions_count INTEGER;
-- [HEADER]     stagnant_tickets_count INTEGER;
-- [HEADER]     avg_sentiment_score NUMERIC;
-- [HEADER]     last_automation_run TIMESTAMPTZ;
BEGIN
    -- Check if the current user is an admin
-- [HEADER]     IF NOT EXISTS (
        SELECT 1 FROM user_profiles
-- [HEADER]         WHERE id = auth.uid()
-- [HEADER]         AND role IN ('admin', 'super_admin')
-- [HEADER]     ) THEN
        RAISE EXCEPTION 'Access denied: Admin role required';
    END IF;
...
-- [HEADER]         'totalRevenue', total_revenue_amount,
-- [HEADER]         'activeUsers', active_users_count,
-- [HEADER]         'totalAiCost', total_ai_cost,
-- [HEADER]         'totalAutomatedActions', automated_actions_count,
-- [HEADER]         'stagnantTickets', stagnant_tickets_count,
-- [HEADER]         'avgSentiment', avg_sentiment_score,
-- [HEADER]         'lastAutomationRun', last_automation_run
-- [HEADER]     );
...
-- [HEADER]     )
    VALUES (
-- [HEADER]         p_user_id,
-- [HEADER]         p_user_id, -- In this context, target is usually the same user
-- [HEADER]         p_action,
-- [HEADER]         p_details || jsonb_build_object(
-- [HEADER]             'audited_by', 'AI Engine',
-- [HEADER]             'contract_id', p_contract_id,
-- [HEADER]             'timestamp', NOW()
-- [HEADER]         ),
-- [HEADER]         NOW()
-- [HEADER]     );
...
-- [HEADER]     IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [HEADER]         WHERE table_name = 'user_profiles' AND column_name = 'phone'
-- [HEADER]     ) THEN
        ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS phone TEXT;
    END IF;
END $$;
...
BEGIN
    ...
    SET phone = au.phone
-- [HEADER]     FROM auth.users au
-- [HEADER]     WHERE up.id = au.id
-- [HEADER]     AND up.phone IS NULL
-- [HEADER]     AND au.phone IS NOT NULL;
-- [HEADER] EXCEPTION WHEN OTHERS THEN
...
SELECT cron.schedule(
-- [HEADER]     'boi-rates-daily-update',
-- [HEADER]     '0 15 * * *',  -- Every day at 15:00 UTC
    $$
    SELECT
-- [HEADER]         net.http_post(
-- [HEADER]             url := 'https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/fetch-index-data',
-- [HEADER]             headers := jsonb_build_object(
-- [HEADER]                 'Content-Type', 'application/json',
-- [HEADER]                 'Authorization', 'Bearer ' || current_setting('request.header.apikey', true)
-- [HEADER]             ),
-- [HEADER]             body := '{}'::jsonb
-- [HEADER]         ) AS request_id;
    $$
...
-- [HEADER]         up.phone,
-- [HEADER]         up.role::TEXT,
-- [HEADER]         COALESCE(up.subscription_status::TEXT, 'active'),
-- [HEADER]         up.plan_id,
-- [HEADER]         up.created_at,
-- [HEADER]         up.last_login,
...
-- [HEADER] ON CONFLICT (index_type, date) 
...
DO $$
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'rent_periods') THEN
...
DO $$
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'contracts' AND column_name = 'tenants') THEN
        ALTER TABLE public.contracts ADD COLUMN IF NOT EXISTS tenants JSONB DEFAULT '[]'::jsonb;
    END IF;
END $$;
...
DROP TRIGGER IF EXISTS trigger_update_property_status ON public.contracts;
DROP TRIGGER IF EXISTS "trigger_update_property_status" ON DROP;
CREATE TRIGGER trigger_update_property_status
-- [HEADER] AFTER INSERT OR UPDATE ON public.contracts
-- [HEADER] FOR EACH ROW
EXECUTE FUNCTION public.update_property_status_from_contract();
...
RETURNS TRIGGER AS $$
DECLARE
-- [HEADER]     target_property_id uuid;
BEGIN
    -- Determine which property we are talking about
    -- TG_OP is the operation (INSERT, UPDATE, DELETE)
-- [HEADER]     IF (TG_OP = 'DELETE') THEN
-- [HEADER]         target_property_id := OLD.property_id;
-- [HEADER]     ELSE
-- [HEADER]         target_property_id := NEW.property_id;
    END IF;
...
-- [HEADER]         IF EXISTS (
            SELECT 1 FROM public.contracts 
-- [HEADER]             WHERE property_id = prop.id 
-- [HEADER]             AND status = 'active'
-- [HEADER]         ) THEN
            UPDATE public.properties
            SET status = 'Occupied'
-- [HEADER]             WHERE id = prop.id;
-- [HEADER]         ELSE
            -- Otherwise Vacant
            UPDATE public.properties
            SET status = 'Vacant'
...
BEGIN
    ...
    UPDATE public.contracts
    SET status = 'archived'
-- [HEADER]     WHERE status = 'active'
-- [HEADER]     AND end_date < CURRENT_DATE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- [HEADER]     IF v_value IS NULL OR v_value = '' THEN
        BEGIN
-- [HEADER]             v_value := current_setting('app.settings.' || p_key, true);
-- [HEADER]         EXCEPTION WHEN OTHERS THEN
-- [HEADER]             v_value := NULL;
        END;
    END IF;
...
-- [HEADER]     'type', TG_OP,
-- [HEADER]     'table', TG_TABLE_NAME,
-- [HEADER]     'record', to_jsonb(NEW),
-- [HEADER]     'old_record', CASE WHEN TG_OP = 'UPDATE' THEN to_jsonb(OLD) ELSE NULL END
-- [HEADER]   );
...
-- [HEADER]       headers := jsonb_build_object(
-- [HEADER]         'Content-Type', 'application/json',
-- [HEADER]         'Authorization', 'Bearer ' || v_service_key
-- [HEADER]       ),
...
-- [HEADER]     v_project_ref text;
-- [HEADER]     v_service_key text;
-- [HEADER]     v_target_email text;
-- [HEADER]     v_asset_alerts_enabled boolean;
BEGIN
    -- Get project config
-- [HEADER]     v_project_ref := public.get_supabase_config('supabase_project_ref');
-- [HEADER]     v_service_key := public.get_supabase_config('supabase_service_role_key');
...
-- [HEADER]     WHERE u.id = NEW.user_id;
    ...
    -- 1. High priority type (warning, error, urgent, action)
    -- 2. OR is a maintenance event AND the user hasn't explicitly disabled asset alerts
-- [HEADER]     IF (v_project_ref IS NOT NULL AND v_service_key IS NOT NULL AND v_target_email IS NOT NULL) AND 
...
-- [HEADER]             ),
-- [HEADER]             body := jsonb_build_object(
-- [HEADER]                 'email', v_target_email,
-- [HEADER]                 'notification', to_jsonb(NEW)
-- [HEADER]             )
-- [HEADER]           );
    END IF;
...
-- [HEADER]             WHERE c.property_id = p.id
-- [HEADER]             AND c.status = 'active'
-- [HEADER]             AND c.start_date <= CURRENT_DATE
-- [HEADER]             AND (c.end_date IS NULL OR c.end_date >= CURRENT_DATE)
-- [HEADER]         ) THEN 'Occupied'
-- [HEADER]         ELSE 'Vacant'
    END
-- [HEADER]     WHERE p.id IS NOT NULL; -- Added safe WHERE clause
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DO $$
BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'recalculate_all_property_statuses') THEN
        PERFORM public.recalculate_all_property_statuses();
    END IF;
END $$;
...
DO $$ 
BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'is_admin') THEN
        DROP POLICY IF EXISTS "Admins view all payments" ON public.payments;
        CREATE POLICY "Admins view all payments" 
-- [HEADER]             ON public.payments FOR SELECT 
            USING (public.is_admin());
...
AS $$
DECLARE
-- [HEADER]     v_full_name TEXT;
-- [HEADER]     v_first_name TEXT;
-- [HEADER]     v_last_name TEXT;
-- [HEADER]     v_plan_id TEXT := 'free';
BEGIN
    -- Parse metadata
-- [HEADER]     v_full_name := COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1));
-- [HEADER]     v_first_name := COALESCE(NEW.raw_user_meta_data->>'first_name', split_part(v_full_name, ' ', 1), 'User');
-- [HEADER]     v_last_name := COALESCE(NEW.raw_user_meta_data->>'last_name', 'User');
...
-- [HEADER]         'active', 
-- [HEADER]         v_plan_id,
-- [HEADER]         'free_forever', -- Legacy field support
-- [HEADER]         COALESCE((NEW.raw_user_meta_data->>'marketing_consent')::boolean, FALSE),
-- [HEADER]         CASE WHEN (NEW.raw_user_meta_data->>'marketing_consent')::boolean THEN NOW() ELSE NULL END
-- [HEADER]     )
-- [HEADER]     ON CONFLICT (id) DO UPDATE SET
...
-- [HEADER]     IF EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [HEADER]         WHERE table_name = 'subscription_plans' AND column_name = 'max_tenants'
-- [HEADER]     ) THEN
...
-- [HEADER]         'support_level', 'priority',
-- [HEADER]         'export_data', true,
-- [HEADER]         'legal_library', true,
-- [HEADER]         'whatsapp_bot', true,
-- [HEADER]         'maintenance_tracker', true,
-- [HEADER]         'portfolio_visualizer', true,
-- [HEADER]         'api_access', true,
-- [HEADER]         'ai_assistant', true,
-- [HEADER]         'bill_analysis', true
-- [HEADER]     )
-- [HEADER] WHERE id = 'free';
...
-- [HEADER]     IF EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [HEADER]         WHERE table_name = 'subscription_plans' AND column_name = 'max_storage_mb'
-- [HEADER]     ) THEN
...
AS $$
DECLARE
-- [HEADER]     selected_plan TEXT;
BEGIN
    -- Extract plan_id from metadata or default to 'free'
-- [HEADER]     selected_plan := COALESCE(NEW.raw_user_meta_data->>'plan_id', 'free');
...
DO $$ 
BEGIN 
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_preferences' AND column_name = 'pinned_cities') THEN
...
SELECT cron.schedule(
-- [HEADER]     'sync-rental-trends-quarterly', -- Job name
-- [HEADER]     '0 6 1 2,5,8,11 *',            -- Schedule (Quarterly)
    $$
    SELECT
-- [HEADER]         net.http_post(
...
-- [HEADER]             headers:=jsonb_build_object(
-- [HEADER]                 'Content-Type', 'application/json',
-- [HEADER]                 'Authorization', 'Bearer ' || (SELECT value FROM system_settings WHERE key = 'service_role_key' LIMIT 1)
-- [HEADER]             ),
-- [HEADER]             body:='{}'::jsonb
-- [HEADER]         ) as request_id;
    $$
...
-- [HEADER]         'avgSentiment', avg_sentiment_score,
-- [HEADER]         'lastAutomationRun', last_automation_run,
-- [HEADER]         'topCities', COALESCE(top_cities, '[]'::json)
-- [HEADER]     );
...
SELECT cron.schedule(
-- [HEADER]     'daily-admin-summary',
-- [HEADER]     '30 5 * * *', -- 05:30 UTC = 07:30/08:30 IL time depending on DST. 
...
-- [HEADER]         headers := jsonb_build_object(
-- [HEADER]           'Content-Type', 'application/json',
-- [HEADER]           'Authorization', 'Bearer ' || public.get_supabase_config('supabase_service_role_key')
-- [HEADER]         ),
-- [HEADER]         body := '{}'::jsonb
-- [HEADER]       )
    $$
...
-- [HEADER] EXCEPTION WHEN OTHERS THEN
-- [HEADER]     NULL; -- Skip if not scheduled
END $$;

SELECT cron.schedule(
-- [HEADER]     'daily-admin-summary',
-- [HEADER]     '30 5 * * *', -- 05:30 UTC = 07:30/08:30 IL time (08:00 Target)
    $$
    SELECT
-- [HEADER]       net.http_post(
-- [HEADER]         url := 'https://' || public.get_supabase_config('supabase_project_ref') || '.supabase.co/functions/v1/send-daily-admin-summary',
...
-- [HEADER]         pref_days := COALESCE((due_payment.notification_preferences->>'rent_due_days')::int, 3);
-- [HEADER]         IF pref_days > 60 THEN pref_days := 60; END IF;
-- [HEADER]         IF pref_days < 1 THEN pref_days := 1; END IF;
...
-- [HEADER] add column if not exists has_seen_welcome_v1 boolean default false;
...
RETURNS TRIGGER AS $$
BEGIN
-- [HEADER]     IF (OLD.role != NEW.role OR OLD.is_super_admin != NEW.is_super_admin) THEN
        -- Check if the PERFOMING user is an admin
-- [HEADER]         IF NOT EXISTS (
            SELECT 1 FROM public.user_profiles 
-- [HEADER]             WHERE id = auth.uid() AND (role = 'admin' OR is_super_admin = true)
-- [HEADER]         ) THEN
...
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'feedback') THEN
        ALTER TABLE public.feedback ENABLE ROW LEVEL SECURITY;
        DROP POLICY IF EXISTS "Users can view own feedback" ON public.feedback;
DROP POLICY IF EXISTS "Users can view own feedback" ON public.feedback;
        CREATE POLICY "Users can view own feedback" ON public.feedback
-- [HEADER]             FOR SELECT USING (auth.uid() = user_id OR public.is_admin());
...
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'ai_conversations') THEN
        ALTER TABLE public.ai_conversations ENABLE ROW LEVEL SECURITY;
...
-- [HEADER]             FOR ALL USING (auth.uid() = user_id OR public.is_admin());
    END IF;
END $$;

-- 3. HARDEN INVOICES
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'invoices') THEN
...
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_preferences') THEN
...
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'audit_logs') THEN
        ALTER TABLE public.audit_logs ENABLE ROW LEVEL SECURITY;
        DROP POLICY IF EXISTS "Admins can view all audit logs" ON public.audit_logs;
DROP POLICY IF EXISTS "Admins can view all audit logs" ON public.audit_logs;
        CREATE POLICY "Admins can view all audit logs" ON public.audit_logs
-- [HEADER]             FOR SELECT USING (public.is_admin());
    END IF;
END $$;

-- 6. HARDEN NOTIFICATIONS
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'notifications') THEN
        ALTER TABLE public.notifications ENABLE ROW LEVEL SECURITY;
        DROP POLICY IF EXISTS "Users can view their own notifications" ON public.notifications;
...
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_storage_usage') THEN
...
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'whatsapp_conversations') THEN
...
DO $$ BEGIN
-- [HEADER]     IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'whatsapp_messages') THEN
...
-- [HEADER]             FOR SELECT USING (
-- [HEADER]                 EXISTS (
                    SELECT 1 FROM public.whatsapp_conversations c
-- [HEADER]                     WHERE c.id = whatsapp_messages.conversation_id
-- [HEADER]                     AND (c.user_id = auth.uid() OR public.is_admin())
-- [HEADER]                 )
-- [HEADER]             );
    END IF;
END $$;
...
-- [HEADER]             FOR SELECT USING (auth.uid() = id OR public.is_admin());
            
        DROP POLICY IF EXISTS "Users can update own profile" ON public.user_profiles;
DROP POLICY IF EXISTS "Users can update own profile" ON public.user_profiles;
        CREATE POLICY "Users can update own profile" ON public.user_profiles
-- [HEADER]             FOR UPDATE USING (auth.uid() = id)
            WITH CHECK (auth.uid() = id);
    END IF;
END $$;
...
-- [HEADER]     IF auth.uid() != p_user_id AND NOT public.is_admin() THEN
...
-- [HEADER]       AND created_at >= v_month_start;
    ...
    -- 3. Check if allowed
-- [HEADER]     IF v_limit = -1 OR (v_current_usage + 1) <= v_limit THEN
        -- Log the usage
...
-- [HEADER]         RETURN jsonb_build_object(
-- [HEADER]             'allowed', true,
-- [HEADER]             'current_usage', v_current_usage + 1,
-- [HEADER]             'limit', v_limit
-- [HEADER]         );
-- [HEADER]     ELSE
-- [HEADER]         RETURN jsonb_build_object(
...
-- [HEADER]     v_phone := COALESCE(NEW.raw_user_meta_data->>'phone_number', NEW.phone);
...
-- [HEADER]     FOR INSERT WITH CHECK (true);
...
-- [HEADER]     IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
-- [HEADER]         WHERE tablename = 'analytics_events' AND policyname = 'Admins can read all analytics'
-- [HEADER]     ) THEN
;
...
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM public.user_profiles WHERE id = auth.uid() AND role = 'admin') THEN
...
-- [HEADER]         'daily_trends', (
            SELECT jsonb_agg(t) FROM (
                SELECT 
-- [HEADER]                     date_trunc('day', created_at)::date as day,
-- [HEADER]                     count(*) as count
-- [HEADER]                 FROM analytics_events
-- [HEADER]                 WHERE created_at > now() - (days_limit || ' days')::interval
-- [HEADER]                 GROUP BY 1
...
-- [HEADER]         ON public.security_logs FOR SELECT
        USING (public.is_admin());
-- [HEADER] EXCEPTION WHEN OTHERS THEN
    CREATE POLICY "Admins can view security logs"
-- [HEADER]         ON public.security_logs FOR SELECT
...
-- [HEADER]     IF p_severity = 'critical' THEN
        UPDATE public.user_profiles 
...
DECLARE
-- [HEADER]     v_user_id UUID;
-- [HEADER]     v_event_code TEXT;
-- [HEADER]     v_severity TEXT;
-- [HEADER]     v_details JSONB;
-- [HEADER]     v_hour_ago TIMESTAMPTZ := NOW() - INTERVAL '1 hour';
BEGIN
    -- 1. WHATSAPP SPIKE DETECTION
    -- Users who sent more than 30 messages in the last hour
-- [HEADER]     FOR v_user_id, v_details IN 
        SELECT w.user_id, jsonb_build_object('count', count(*), 'period', '1h')
-- [HEADER]         FROM public.whatsapp_usage_logs w
-- [HEADER]         WHERE w.created_at >= v_hour_ago
-- [HEADER]         GROUP BY w.user_id
-- [HEADER]         HAVING count(*) > 30
-- [HEADER]     LOOP
        PERFORM public.log_security_event(v_user_id, 'WHATSAPP_SPIKE', 'medium', v_details);
-- [HEADER]         user_id := v_user_id;
-- [HEADER]         event_code := 'WHATSAPP_SPIKE';
...
DO $$ 
BEGIN
-- [HEADER]     IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_preferences' AND column_name = 'disclaimer_accepted') THEN
        ALTER TABLE user_preferences 
-- [HEADER]         ADD COLUMN IF NOT EXISTS disclaimer_accepted BOOLEAN DEFAULT false;
    END IF;
END $$;
-- ============================================
...
BEGIN
    -- Fetch config 
-- [HEADER]     v_ref := public.get_supabase_config('supabase_project_ref');
-- [HEADER]     v_key := public.get_supabase_config('supabase_service_role_key');
...
-- [HEADER]         headers := jsonb_build_object(
-- [HEADER]           'Content-Type', 'application/json',
-- [HEADER]           'Authorization', 'Bearer ' || v_key
-- [HEADER]         ),
-- [HEADER]         body := '{}'::jsonb
-- [HEADER]     );
-- [HEADER] EXCEPTION WHEN OTHERS THEN
...
-- [HEADER]         'daily-admin-summary',
-- [HEADER]         '30 6 * * *', 
-- [HEADER]         'SELECT public.trigger_daily_admin_summary()'
-- [HEADER]     );
END $$;
...
DO $$
DECLARE
-- [HEADER]     job_names TEXT[] := ARRAY['index-update-day15', 'index-update-day16', 'index-update-day17', 'index-sync-primary', 'index-sync-retry-15', 'index-sync-retry-16'];
-- [HEADER]     jname TEXT;
BEGIN
    -- Unschedule legacy jobs safely
-- [HEADER]     FOREACH jname IN ARRAY job_names LOOP
-- [HEADER]         IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = jname) THEN
    ...
    PERFORM cron.schedule(
-- [HEADER]         'index-sync-primary',
-- [HEADER]         '0 17 15 * *',
-- [HEADER]         'SELECT public.trigger_index_sync()'
-- [HEADER]     );

    -- Schedule Retry Syncs (Every 2 hours on the 15th evening and 16th)
    PERFORM cron.schedule(
-- [HEADER]         'index-sync-retry-15',
-- [HEADER]         '0 19,21,23 15 * *', 
-- [HEADER]         'SELECT public.trigger_index_sync()'
-- [HEADER]     );

    PERFORM cron.schedule(
-- [HEADER]         'index-sync-retry-16',
-- [HEADER]         '0 */2 16 * *',
-- [HEADER]         'SELECT public.trigger_index_sync()'
-- [HEADER]     );
END $$;
