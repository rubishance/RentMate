...
-- [GARBAGE]     id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     email TEXT,
-- [GARBAGE]     full_name TEXT,
-- [GARBAGE]     role TEXT DEFAULT 'user',
-- [GARBAGE]     subscription_status TEXT DEFAULT 'active',
-- [GARBAGE]     subscription_plan TEXT DEFAULT 'free_forever',
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- PROPERTIES
CREATE TABLE IF NOT EXISTS public.properties (
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
-- [GARBAGE]     user_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE,
-- [GARBAGE]     title TEXT,
-- [GARBAGE]     address TEXT,
-- [GARBAGE]     city TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
-- [GARBAGE]     user_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE,
-- [GARBAGE]     property_id UUID REFERENCES public.properties(id) ON DELETE CASCADE,
-- [GARBAGE]     tenant_id UUID REFERENCES public.tenants(id) ON DELETE CASCADE,
-- [GARBAGE]     start_date DATE,
-- [GARBAGE]     end_date DATE,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Add extraction fields to contracts table
ALTER TABLE contracts 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS guarantors_info TEXT, -- Summarized text of all guarantors
-- [GARBAGE] ADD COLUMN IF NOT EXISTS special_clauses TEXT; -- Summarized text of special clauses
...
AS $$
DECLARE
-- [GARBAGE]     property_address text;
-- [GARBAGE]     notification_title text;
-- [GARBAGE]     notification_body text;
BEGIN
    -- Only proceed if status changed
-- [HEADER]     IF OLD.status IS NOT DISTINCT FROM NEW.status THEN
        RETURN NEW;
    END IF;

    -- Fetch property address
    SELECT city || ', ' || address INTO property_address
-- [GARBAGE]     FROM public.properties
-- [GARBAGE]     WHERE id = NEW.property_id;

    -- Determine message
-- [GARBAGE]     notification_title := 'Contract Status Updated';
-- [GARBAGE]     notification_body := format('Contract for %s is now %s.', property_address, NEW.status);
...
-- [GARBAGE]         notification_title,
-- [GARBAGE]         notification_body,
-- [GARBAGE]         json_build_object(
            'contract_id', NEW.id,
            'event', 'status_change',
...
-- [GARBAGE]     AFTER UPDATE ON public.contracts
    FOR EACH ROW
...
DECLARE
-- [GARBAGE]     r RECORD;
-- [GARBAGE]     extension_days int := 60; -- Default extension notice period
BEGIN
    -------------------------------------------------------
...
-- [GARBAGE]         FROM public.contracts c
-- [GARBAGE]         JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.end_date = CURRENT_DATE + INTERVAL '30 days'
    LOOP
        -- Check if we already sent this notification (idempotency)
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
-- [GARBAGE]             AND metadata->>'contract_id' = r.id::text 
-- [GARBAGE]             AND metadata->>'event' = 'ending_soon'
        ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [GARBAGE]                 r.user_id,
...
-- [GARBAGE]                 format('Contract for %s, %s ends in 30 days (%s).', r.city, r.address, r.end_date),
-- [GARBAGE]                 json_build_object('contract_id', r.id, 'event', 'ending_soon')::jsonb
            );
        END IF;
    END LOOP;
...
-- [GARBAGE]         JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.extension_option = TRUE
        -- Assuming deadline IS the end_date if not specified otherwise, or checking user preference
-- [GARBAGE]         AND c.end_date = CURRENT_DATE + (extension_days || ' days')::INTERVAL
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
-- [GARBAGE]             AND metadata->>'contract_id' = r.id::text 
-- [GARBAGE]             AND metadata->>'event' = 'extension_deadline'
        ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [GARBAGE]                 r.user_id,
...
-- [GARBAGE]                 format('Extension option for %s, %s ends in %s days.', r.city, r.address, extension_days),
-- [GARBAGE]                 json_build_object('contract_id', r.id, 'event', 'extension_deadline')::jsonb
            );
        END IF;
    END LOOP;
...
-- [GARBAGE]         JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.linkage_type != 'none' -- Only if linked
-- [GARBAGE]         AND (
-- [GARBAGE]             c.start_date + INTERVAL '1 year' = CURRENT_DATE OR
-- [GARBAGE]             c.start_date + INTERVAL '2 years' = CURRENT_DATE OR
-- [GARBAGE]             c.start_date + INTERVAL '3 years' = CURRENT_DATE
        )
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
-- [GARBAGE]             AND metadata->>'contract_id' = r.id::text 
-- [GARBAGE]             AND metadata->>'event' = 'index_update'
-- [GARBAGE]             AND metadata->>'date' = CURRENT_DATE::text
        ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [GARBAGE]                 r.user_id,
                'urgent',
                'Annual Index Update',
-- [GARBAGE]                 format('Annual index update required for %s, %s.', r.city, r.address),
-- [GARBAGE]                 json_build_object('contract_id', r.id, 'event', 'index_update', 'date', CURRENT_DATE)::jsonb
            );
        END IF;
    END LOOP;
...
-- [GARBAGE]         FROM public.payments py
-- [GARBAGE]         JOIN public.contracts c ON c.id = py.contract_id
-- [GARBAGE]         JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]         WHERE py.status = 'pending'
-- [GARBAGE]         AND py.date = CURRENT_DATE
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
-- [GARBAGE]             AND metadata->>'payment_id' = r.id::text 
-- [GARBAGE]             AND metadata->>'event' = 'payment_due'
        ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [GARBAGE]                 r.user_id,
                'warning',
                'Payment Due Today',
-- [GARBAGE]                 format('Payment of ג‚×%s for %s, %s is due today.', r.amount, r.city, r.address),
-- [GARBAGE]                 json_build_object('payment_id', r.id, 'event', 'payment_due')::jsonb
            );
        END IF;
    END LOOP;
...
ALTER TABLE contracts 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS needs_painting BOOLEAN DEFAULT false;
...
-- [GARBAGE] ADD CONSTRAINT properties_property_type_check 
-- [GARBAGE] CHECK (property_type IN ('apartment', 'penthouse', 'garden', 'house', 'other'));
-- Add parking and storage columns to properties
ALTER TABLE properties
-- [GARBAGE] ADD COLUMN IF NOT EXISTS has_parking BOOLEAN DEFAULT false,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS has_storage BOOLEAN DEFAULT false;
-- Add property_type column
ALTER TABLE properties
-- [GARBAGE] ADD COLUMN IF NOT EXISTS property_type TEXT DEFAULT 'apartment';
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS rent_price NUMERIC(10, 2);
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS stripe_customer_id TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS stripe_subscription_id TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS subscription_status TEXT DEFAULT 'inactive' CHECK (subscription_status IN ('active', 'inactive', 'canceled', 'past_due'));
...
-- [GARBAGE]   id uuid default gen_random_uuid() primary key,
-- [GARBAGE]   user_id uuid references auth.users(id) not null,
-- [GARBAGE]   type text not null check (type in ('upgrade_request', 'system_alert')),
-- [GARBAGE]   content jsonb not null default '{}'::jsonb,
-- [GARBAGE]   status text not null default 'pending' check (status in ('pending', 'processing', 'resolved', 'dismissed')),
-- [GARBAGE]   created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
-- [HEADER] create policy "Admins can view all notifications"
-- [GARBAGE]   on admin_notifications for select
-- [GARBAGE]   to authenticated
  using (
-- [GARBAGE]     exists (
      select 1 from user_profiles
-- [GARBAGE]       where id = auth.uid() and role = 'admin'
    )
  );

-- Policy: Admins can update notifications
-- [HEADER] create policy "Admins can update notifications"
-- [GARBAGE]   on admin_notifications for update
-- [GARBAGE]   to authenticated
  using (
-- [GARBAGE]     exists (
      select 1 from user_profiles
...
-- [HEADER] create policy "Users can insert upgrade requests"
-- [GARBAGE]   on admin_notifications for insert
-- [GARBAGE]   to authenticated
  with check (
-- [GARBAGE]     user_id = auth.uid() 
-- [GARBAGE]     and type = 'upgrade_request'
  );
...
-- [GARBAGE]     id UUID NOT NULL DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     user_name TEXT NOT NULL,
-- [GARBAGE]     user_email TEXT NOT NULL,
-- [GARBAGE]     message TEXT NOT NULL,
-- [GARBAGE]     status TEXT NOT NULL DEFAULT 'new' CHECK (status IN ('new', 'read', 'replied', 'archived')),
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
-- [GARBAGE]     CONSTRAINT contact_messages_pkey PRIMARY KEY (id)
);
...
-- Policies
-- [HEADER] CREATE POLICY "Users can view own messages"
-- [GARBAGE]     ON contact_messages FOR SELECT
    USING (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can insert own messages"
-- [GARBAGE]     ON contact_messages FOR INSERT
    WITH CHECK (user_id = auth.uid());

-- Admin policy (if you want admins to see all messages)
-- [HEADER] CREATE POLICY "Admins can view all messages"
-- [GARBAGE]     ON contact_messages FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
...
-- [GARBAGE] ON CONFLICT (id) DO NOTHING;

-- Policy: Allow authenticated users to upload files to 'contracts' bucket
-- [HEADER] CREATE POLICY "Allow authenticated uploads"
-- [GARBAGE] ON storage.objects FOR INSERT
-- [GARBAGE] TO authenticated
WITH CHECK (bucket_id = 'contracts');

-- Policy: Allow authenticated users to view files in 'contracts' bucket
-- [HEADER] CREATE POLICY "Allow authenticated view"
-- [GARBAGE] ON storage.objects FOR SELECT
-- [GARBAGE] TO authenticated
USING (bucket_id = 'contracts');

-- Policy: Allow users to update their own files (optional, but good for redaction flow)
-- [HEADER] CREATE POLICY "Allow authenticated update"
-- [GARBAGE] ON storage.objects FOR UPDATE
-- [GARBAGE] TO authenticated
USING (bucket_id = 'contracts');

-- Policy: Allow users to delete their own files
-- [HEADER] CREATE POLICY "Allow authenticated delete"
-- [GARBAGE] ON storage.objects FOR DELETE
-- [GARBAGE] TO authenticated
USING (bucket_id = 'contracts');
-- CREATE TABLE IF NOT EXISTS for storing index base periods and chaining factors
CREATE TABLE IF NOT EXISTS index_bases (
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
-- [GARBAGE]     index_type TEXT NOT NULL, -- e.g., 'cpi', 'construction', 'housing'
-- [GARBAGE]     base_period_start DATE NOT NULL, -- The start date of this base period (e.g., '2023-01-01')
-- [GARBAGE]     base_value NUMERIC NOT NULL DEFAULT 100.0, -- The value of the base index (usually 100.0)
-- [GARBAGE]     previous_base_period_start DATE, -- The start date of the *previous* base period
-- [GARBAGE]     chain_factor NUMERIC, -- The factor to multiply when moving FROM this base TO the previous base (or vice versa depending on logic)
                          -- CBS usually publishes "Linkage Coefficient" (׳׳§׳“׳ ׳§׳©׳¨) to the previous base.
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- [GARBAGE]   id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
-- [GARBAGE]   index_type TEXT NOT NULL CHECK (index_type IN ('cpi', 'housing', 'construction', 'usd', 'eur')),
-- [GARBAGE]   date TEXT NOT NULL, -- Format: 'YYYY-MM'
-- [GARBAGE]   value DECIMAL(10, 4) NOT NULL,
-- [GARBAGE]   source TEXT DEFAULT 'cbs' CHECK (source IN ('cbs', 'exchange-api', 'manual')),
-- [GARBAGE]   created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]   updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]   UNIQUE(index_type, date)
);
...
-- [HEADER] CREATE POLICY "Allow authenticated users to read index data"
-- [GARBAGE]   ON index_data
  FOR SELECT
-- [GARBAGE]   TO authenticated
  USING (true);
...
-- [HEADER] CREATE POLICY "Allow authenticated users to manage index data"
-- [GARBAGE]   ON index_data
  FOR ALL
-- [GARBAGE]   TO authenticated
  USING (true)
...
-- [GARBAGE]     id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     type TEXT NOT NULL CHECK (type IN ('info', 'success', 'warning', 'error')),
-- [GARBAGE]     title TEXT NOT NULL,
-- [GARBAGE]     message TEXT NOT NULL,
-- [GARBAGE]     read_at TIMESTAMP WITH TIME ZONE,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- [HEADER] CREATE POLICY "Users can view their own notifications"
-- [GARBAGE]     ON public.notifications FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY "Users can update their own notifications (mark as read)"
-- [GARBAGE]     ON public.notifications FOR UPDATE
    USING (auth.uid() = user_id);
...
-- [HEADER] CREATE POLICY "Public can view property images"
-- [GARBAGE]     ON storage.objects
    FOR SELECT
    USING ( bucket_id = 'property-images' );

-- Policy: Authenticated users can UPLOAD files
DROP POLICY IF EXISTS "Authenticated users can upload property images" ON storage.objects;
-- [HEADER] CREATE POLICY "Authenticated users can upload property images"
-- [GARBAGE]     ON storage.objects
    FOR INSERT
    WITH CHECK (
-- [GARBAGE]         bucket_id = 'property-images'
-- [GARBAGE]         AND
-- [GARBAGE]         auth.role() = 'authenticated'
    );
...
-- [HEADER] CREATE POLICY "Authenticated users can update property images"
-- [GARBAGE]     ON storage.objects
    FOR UPDATE
    USING ( bucket_id = 'property-images' AND auth.role() = 'authenticated' );

DROP POLICY IF EXISTS "Authenticated users can delete property images" ON storage.objects;
-- [HEADER] CREATE POLICY "Authenticated users can delete property images"
-- [GARBAGE]     ON storage.objects
    FOR DELETE
    USING ( bucket_id = 'property-images' AND auth.role() = 'authenticated' );
-- Create a table to track rate limits
CREATE TABLE IF NOT EXISTS public.rate_limits (
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     ip_address TEXT,
-- [GARBAGE]     endpoint TEXT NOT NULL,
-- [GARBAGE]     request_count INTEGER DEFAULT 1,
-- [GARBAGE]     last_request_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc', now()),
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc', now())
);
...
-- [GARBAGE]     WHERE last_request_at < (now() - INTERVAL '1 hour');
END;
$$ LANGUAGE plpgsql;
...
-- [GARBAGE]     key TEXT PRIMARY KEY,
-- [GARBAGE]     value JSONB NOT NULL,
-- [GARBAGE]     description TEXT,
-- [GARBAGE]     updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]     updated_by UUID REFERENCES auth.users(id)
);
...
-- [GARBAGE]     id TEXT PRIMARY KEY, -- e.g. 'contract_ending', 'payment_due'
-- [GARBAGE]     name TEXT NOT NULL,
-- [GARBAGE]     description TEXT,
-- [GARBAGE]     is_enabled BOOLEAN DEFAULT true,
-- [GARBAGE]     days_offset INT DEFAULT 0, -- e.g. 30 (days before)
-- [GARBAGE]     channels JSONB DEFAULT '["in_app"]'::jsonb, -- e.g. ["in_app", "email", "push"]
-- [GARBAGE]     target_audience TEXT DEFAULT 'user' CHECK (target_audience IN ('user', 'admin', 'both')),
-- [GARBAGE]     message_template TEXT NOT NULL,
-- [GARBAGE]     updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- [GARBAGE] ON CONFLICT (key) DO NOTHING;
...
-- [GARBAGE]     rule_ending_soon JSONB;
-- [GARBAGE]     rule_extension JSONB;
-- [GARBAGE]     rule_index JSONB;
-- [GARBAGE]     rule_payment JSONB;
BEGIN
    -- Fetch Rules
...
-- [GARBAGE]             JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]             WHERE c.status = 'active'
-- [GARBAGE]             AND c.end_date = CURRENT_DATE + ((rule_ending_soon->>'days_offset')::int || ' days')::INTERVAL
        LOOP
...
-- [GARBAGE]                     format((rule_ending_soon->>'message_template')::text, r.city, r.address, (rule_ending_soon->>'days_offset')::text), 
-- [GARBAGE]                     json_build_object('contract_id', r.id, 'event', 'ending_soon')::jsonb
                );
            END IF;
        END LOOP;
...
-- [GARBAGE]             WHERE c.status = 'active'
-- [GARBAGE]             AND c.extension_option = TRUE
-- [GARBAGE]             AND c.end_date = CURRENT_DATE + ((rule_extension->>'days_offset')::int || ' days')::INTERVAL
        LOOP
...
-- [GARBAGE]                     format((rule_extension->>'message_template')::text, r.city, r.address, (rule_extension->>'days_offset')::text), 
-- [GARBAGE]                     json_build_object('contract_id', r.id, 'event', 'extension_deadline')::jsonb
                );
            END IF;
        END LOOP;
...
-- [GARBAGE]             JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]             WHERE c.status = 'active'
-- [GARBAGE]             AND c.linkage_type != 'none'
-- [GARBAGE]             AND (
-- [GARBAGE]                 c.start_date + INTERVAL '1 year' = CURRENT_DATE OR
-- [GARBAGE]                 c.start_date + INTERVAL '2 years' = CURRENT_DATE OR
-- [GARBAGE]                 c.start_date + INTERVAL '3 years' = CURRENT_DATE
...
-- [GARBAGE]                     format((rule_index->>'message_template')::text, r.city, r.address), 
-- [GARBAGE]                     json_build_object('contract_id', r.id, 'event', 'index_update', 'date', CURRENT_DATE)::jsonb
                );
            END IF;
        END LOOP;
...
-- [GARBAGE]                     format((rule_payment->>'message_template')::text, r.amount, r.city, r.address), 
-- [GARBAGE]                     json_build_object('payment_id', r.id, 'event', 'payment_due')::jsonb
                );
            END IF;
        END LOOP;
...
-- [GARBAGE]     array_agg(id) as all_ids
-- [GARBAGE]   FROM properties
-- [GARBAGE]   GROUP BY address, city, user_id
-- [GARBAGE]   HAVING COUNT(*) > 1
),
-- [GARBAGE] busted_duplicates AS (
  SELECT
-- [GARBAGE]     keep_id,
-- [GARBAGE]     unnest(all_ids) as duplicate_id
-- [GARBAGE]   FROM duplicates
)
-- 1. Update Tenants to point to the kept property
UPDATE tenants
SET property_id = bd.keep_id
-- [GARBAGE] FROM busted_duplicates bd
-- [GARBAGE] WHERE tenants.property_id = bd.duplicate_id
-- [GARBAGE] AND tenants.property_id != bd.keep_id;
...
SET property_id = bd.keep_id
-- [GARBAGE] FROM busted_duplicates bd
-- [GARBAGE] WHERE contracts.property_id = bd.duplicate_id
-- [GARBAGE] AND contracts.property_id != bd.keep_id;
...
ALTER TABLE properties 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
...
-- [HEADER] CREATE POLICY "Users can view own properties"
-- [GARBAGE]     ON properties FOR SELECT
    USING (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can insert own properties"
-- [GARBAGE]     ON properties FOR INSERT
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can update own properties"
-- [GARBAGE]     ON properties FOR UPDATE
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can delete own properties"
-- [GARBAGE]     ON properties FOR DELETE
    USING (user_id = auth.uid());

-- 5. CREATE SECURE POLICIES FOR TENANTS
-- [HEADER] CREATE POLICY "Users can view own tenants"
-- [GARBAGE]     ON tenants FOR SELECT
    USING (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can insert own tenants"
-- [GARBAGE]     ON tenants FOR INSERT
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can update own tenants"
-- [GARBAGE]     ON tenants FOR UPDATE
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can delete own tenants"
-- [GARBAGE]     ON tenants FOR DELETE
    USING (user_id = auth.uid());

-- 6. CREATE SECURE POLICIES FOR CONTRACTS
-- [HEADER] CREATE POLICY "Users can view own contracts"
-- [GARBAGE]     ON contracts FOR SELECT
    USING (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can insert own contracts"
-- [GARBAGE]     ON contracts FOR INSERT
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can update own contracts"
-- [GARBAGE]     ON contracts FOR UPDATE
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can delete own contracts"
-- [GARBAGE]     ON contracts FOR DELETE
    USING (user_id = auth.uid());

-- 7. CREATE SECURE POLICIES FOR PAYMENTS
-- [HEADER] CREATE POLICY "Users can view own payments"
-- [GARBAGE]     ON payments FOR SELECT
    USING (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can insert own payments"
-- [GARBAGE]     ON payments FOR INSERT
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can update own payments"
-- [GARBAGE]     ON payments FOR UPDATE
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

-- [HEADER] CREATE POLICY "Users can delete own payments"
-- [GARBAGE]     ON payments FOR DELETE
    USING (user_id = auth.uid());
...
-- [GARBAGE]               FROM pg_policies 
-- [GARBAGE]               WHERE schemaname = 'public' 
-- [GARBAGE]               AND tablename IN ('properties', 'tenants', 'contracts', 'payments'))
    LOOP
...
-- [GARBAGE]         full_name, 
-- [GARBAGE]         role, 
-- [GARBAGE]         subscription_status, 
-- [GARBAGE]         subscription_plan
    )
    VALUES (
-- [GARBAGE]         NEW.id,
-- [GARBAGE]         NEW.email,
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email),
        'user'::user_role,
        'active'::subscription_status,
        'free_forever'::subscription_plan_type
    )
-- [GARBAGE]     ON CONFLICT (id) DO NOTHING; -- Idempotency: If it exists, skip.
    ...
    BEGIN
        ...
        SET user_id = NEW.id
-- [GARBAGE]         WHERE user_id IS NULL 
-- [GARBAGE]         AND billing_email = NEW.email;
...
-- [GARBAGE]     AFTER INSERT ON auth.users
    FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();
-- Enable RLS just in case
ALTER TABLE public.user_profiles ENABLE ROW LEVEL SECURITY;

-- Allow Admins to UPDATE any profile
-- [HEADER] CREATE POLICY "Admins can update all profiles" 
-- [GARBAGE] ON public.user_profiles 
FOR UPDATE 
USING (
...
-- [GARBAGE]             WHERE property_id = NEW.property_id
-- [GARBAGE]             AND status = 'active'
-- [GARBAGE]             AND id != NEW.id -- Exclude self during updates
-- [GARBAGE]             AND (
...
-- [GARBAGE] BEFORE INSERT OR UPDATE ON public.contracts
FOR EACH ROW
...
-- [GARBAGE]             status = 'active'
-- [GARBAGE]         WHERE id = NEW.tenant_id;
...
-- [GARBAGE]             status = 'past'
-- [GARBAGE]         WHERE id = NEW.tenant_id 
-- [GARBAGE]         AND property_id = NEW.property_id; -- Only if they are still linked to this property
    END IF;
...
-- [GARBAGE] AFTER INSERT OR UPDATE ON public.contracts
FOR EACH ROW
...
-- [GARBAGE]     ELSIF (NEW.status IN ('ended', 'terminated')) THEN
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}'::jsonb;
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS billing_name TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS billing_email TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS billing_address TEXT;
...
BEGIN
    -- A. Create User Profile
    INSERT INTO public.user_profiles (
-- [GARBAGE]         id, email, full_name, role, subscription_status, subscription_plan
    )
    VALUES (
-- [GARBAGE]         NEW.id,
-- [GARBAGE]         NEW.email,
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS property_id uuid REFERENCES public.properties(id),
-- [GARBAGE] ADD COLUMN IF NOT EXISTS tenant_id uuid REFERENCES public.tenants(id);

-- 2. Other Missing Columns (preventing future errors)
ALTER TABLE public.contracts
-- [GARBAGE] ADD COLUMN IF NOT EXISTS signing_date date,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS start_date date,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS end_date date,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS base_rent numeric(10, 2),
-- [GARBAGE] ADD COLUMN IF NOT EXISTS currency text DEFAULT 'ILS',
-- [GARBAGE] ADD COLUMN IF NOT EXISTS payment_frequency text,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS payment_day integer,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_type text DEFAULT 'none',
-- [GARBAGE] ADD COLUMN IF NOT EXISTS security_deposit_amount numeric(10, 2),
-- [GARBAGE] ADD COLUMN IF NOT EXISTS status text DEFAULT 'active';

-- 3. Linkage Details
ALTER TABLE public.contracts
-- [GARBAGE] ADD COLUMN IF NOT EXISTS base_index_date date,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS base_index_value numeric(10, 4),
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_sub_type text,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_ceiling numeric(5, 2),
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_floor numeric(5, 2);
...
BEGIN
    RETURN EXISTS (
        SELECT 1 
-- [GARBAGE]         FROM public.user_profiles 
-- [GARBAGE]         WHERE id = auth.uid() 
-- [GARBAGE]         AND role = 'admin'
    );
END;
$$;
...
-- A. User Profiles
-- [HEADER] CREATE POLICY "Users can view own profile" 
-- [GARBAGE]     ON user_profiles FOR SELECT 
    USING (auth.uid() = id);

-- [HEADER] CREATE POLICY "Users can update own profile" 
-- [GARBAGE]     ON user_profiles FOR UPDATE 
    USING (auth.uid() = id);

-- [HEADER] CREATE POLICY "Admins can view all profiles" 
-- [GARBAGE]     ON user_profiles FOR SELECT 
    USING (is_admin());
...
-- [HEADER] CREATE POLICY "Admins manage CRM"
-- [GARBAGE]     ON crm_interactions FOR ALL
    USING (is_admin());

-- C. Audit Logs (Admin Only)
-- [HEADER] CREATE POLICY "Admins view audit logs"
-- [GARBAGE]     ON audit_logs FOR SELECT
    USING (is_admin());
...
-- [HEADER] CREATE POLICY "Users view own invoices"
-- [GARBAGE]     ON invoices FOR SELECT
    USING (auth.uid() = user_id);

-- [HEADER] CREATE POLICY "Admins view all invoices"
-- [GARBAGE]     ON invoices FOR SELECT
    USING (is_admin());
-- Ensure contract_file_url exists on contracts table
ALTER TABLE contracts
-- [GARBAGE] ADD COLUMN IF NOT EXISTS contract_file_url TEXT;
...
DO $$
DECLARE
-- [GARBAGE]     target_email TEXT := 'rentmate.rubi@gmail.com'; -- <--- YOUR EMAIL HERE
-- [GARBAGE]     v_user_id UUID;
BEGIN
    -- 1. Find the User ID from the Auth table
    SELECT id INTO v_user_id FROM auth.users WHERE email = target_email;

-- [HEADER]     IF v_user_id IS NULL THEN
...
-- [GARBAGE]     au.id,
-- [GARBAGE]     au.email,
-- [GARBAGE]     COALESCE(au.raw_user_meta_data->>'full_name', split_part(au.email, '@', 1)),
-- [GARBAGE]     COALESCE(au.raw_user_meta_data->>'full_name', split_part(au.email, '@', 1)),
    'User',
    'user',
    'active',
    'free'
-- [GARBAGE] FROM auth.users au
-- [GARBAGE] LEFT JOIN public.user_profiles up ON au.id = up.id
-- [GARBAGE] WHERE up.id IS NULL
-- [GARBAGE] ON CONFLICT (id) DO UPDATE SET
-- [GARBAGE]     email = EXCLUDED.email,
-- [GARBAGE]     full_name = COALESCE(EXCLUDED.full_name, user_profiles.full_name),
-- [GARBAGE]     first_name = COALESCE(EXCLUDED.first_name, user_profiles.first_name),
-- [GARBAGE]     last_name = COALESCE(EXCLUDED.last_name, user_profiles.last_name),
-- [GARBAGE]     updated_at = NOW();

-- 2. Log the fix
DO $$
DECLARE
-- [GARBAGE]     orphaned_count INTEGER;
BEGIN
...
BEGIN
    ...
    SET 
-- [GARBAGE]         first_name = COALESCE(full_name, 'User'),
-- [GARBAGE]         last_name = 'aaa'
-- [GARBAGE]     WHERE first_name IS NULL OR last_name IS NULL;
    ...
    -- SELECT
-- [HEADER]     CREATE POLICY "Users view own"
-- [GARBAGE]     ON public.user_profiles FOR SELECT
    USING (auth.uid() = id);

    -- UPDATE (Explicitly Allow)
-- [HEADER]     CREATE POLICY "Users update own"
-- [GARBAGE]     ON public.user_profiles FOR UPDATE
    USING (auth.uid() = id);

    -- INSERT (Crucial for 'upsert' if row is missing/ghosted)
-- [HEADER]     CREATE POLICY "Users insert own"
-- [GARBAGE]     ON public.user_profiles FOR INSERT
...
BEGIN
    ALTER TABLE properties DROP CONSTRAINT IF EXISTS properties_property_type_check;
-- [HEADER]     ALTER TABLE properties ADD CONSTRAINT properties_property_type_check 
-- [GARBAGE]     CHECK (property_type IN ('apartment', 'penthouse', 'garden', 'house', 'other'));
EXCEPTION
-- [GARBAGE]     WHEN OTHERS THEN NULL;
END $$;
...
-- [GARBAGE]         role, 
-- [GARBAGE]         subscription_status, 
-- [GARBAGE]         plan_id, -- New relation
-- [GARBAGE]         subscription_plan -- Legacy enum fallback
    )
    VALUES (
-- [GARBAGE]         NEW.id,
-- [GARBAGE]         NEW.email,
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS id_number TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS email TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS phone TEXT;
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_type TEXT DEFAULT 'none',
-- [GARBAGE] ADD COLUMN IF NOT EXISTS base_index_date DATE,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS base_index_value NUMERIC(10, 4), -- More precision for index
-- [GARBAGE] ADD COLUMN IF NOT EXISTS security_deposit_amount NUMERIC(10, 2),
-- [GARBAGE] ADD COLUMN IF NOT EXISTS signing_date DATE,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS start_date DATE,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS end_date DATE,
...
ALTER TABLE public.contracts
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_sub_type TEXT, -- 'known', 'respect_of', 'base'
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_ceiling NUMERIC(5, 2), -- Percentage
-- [GARBAGE] ADD COLUMN IF NOT EXISTS linkage_floor NUMERIC(5, 2); -- Percentage
...
-- [GARBAGE] WHERE email = 'rentmate.rubi@gmail.com';  -- Your Email

-- 2. FIX DATABASE SCHEMA (Add missing columns)
ALTER TABLE public.user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS subscription_status TEXT DEFAULT 'active',
-- [GARBAGE] ADD COLUMN IF NOT EXISTS subscription_plan TEXT DEFAULT 'free_forever';

-- 3. FORCE CREATE ADMIN PROFILE
DO $$
DECLARE
-- [GARBAGE]     v_user_id UUID;
-- [GARBAGE]     target_email TEXT := 'rentmate.rubi@gmail.com';
BEGIN
    SELECT id INTO v_user_id FROM auth.users WHERE email = target_email;

-- [HEADER]     IF v_user_id IS NOT NULL THEN
        -- Insert or Update the profile to be an Admin
        INSERT INTO public.user_profiles (
-- [GARBAGE]             id, email, full_name, role, subscription_status, subscription_plan
        )
        VALUES (
-- [GARBAGE]             v_user_id, target_email, 'Admin User', 'admin', 'active', 'free_forever'
        )
-- [GARBAGE]         ON CONFLICT (id) DO UPDATE 
        SET role = 'admin', 
-- [GARBAGE]             subscription_status = 'active', 
-- [GARBAGE]             subscription_plan = 'free_forever';
...
-- [GARBAGE]         id, email, full_name, role, subscription_status, subscription_plan
    ...
    VALUES (
-- [GARBAGE]         NEW.id, NEW.email, COALESCE(NEW.raw_user_meta_data->>'full_name', NEW.email), 
...
-- [GARBAGE]         WHERE user_id IS NULL AND billing_email = NEW.email;
...
-- [GARBAGE]     GET DIAGNOSTICS recovered_count = ROW_COUNT;
...
ALTER TABLE invoices
-- [GARBAGE] ADD CONSTRAINT invoices_user_id_fkey
-- [GARBAGE] FOREIGN KEY (user_id)
-- [GARBAGE] REFERENCES user_profiles(id)
-- [GARBAGE] ON DELETE SET NULL;
...
UPDATE invoices i
SET 
-- [GARBAGE]   billing_name = p.full_name,
-- [GARBAGE]   billing_email = p.email
-- [GARBAGE] FROM user_profiles p
-- [GARBAGE] WHERE i.user_id = p.id;
...
-- [HEADER]     IF NEW.billing_name IS NULL OR NEW.billing_email IS NULL THEN
        SELECT full_name, email INTO NEW.billing_name, NEW.billing_email
-- [GARBAGE]         FROM user_profiles
-- [GARBAGE]         WHERE id = NEW.user_id;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- [GARBAGE]     BEFORE INSERT ON invoices
    FOR EACH ROW
...
AS $$
DECLARE
-- [GARBAGE]     new_device_type TEXT;
-- [GARBAGE]     session_count INT;
-- [GARBAGE]     oldest_session_id UUID;
    -- FIX: Increased from 1 to 5 to prevent aggressive logouts
-- [GARBAGE]     max_sessions_per_type INT := 5; 
BEGIN
    -- Identify what kind of device is trying to log in
-- [GARBAGE]     new_device_type := public.get_device_type(NEW.user_agent);

    -- Count EXISTING sessions for this user of the SAME type
    SELECT COUNT(*)
-- [GARBAGE]     INTO session_count
-- [GARBAGE]     FROM auth.sessions
-- [GARBAGE]     WHERE user_id = NEW.user_id
-- [GARBAGE]     AND public.get_device_type(user_agent) = new_device_type;
    ...
    IF session_count >= max_sessions_per_type THEN
        ...
        SELECT id
-- [GARBAGE]         INTO oldest_session_id
-- [GARBAGE]         FROM auth.sessions
-- [GARBAGE]         WHERE user_id = NEW.user_id
-- [GARBAGE]         AND public.get_device_type(user_agent) = new_device_type
-- [GARBAGE]         ORDER BY created_at ASC
-- [GARBAGE]         LIMIT 1;

        -- Delete it
-- [HEADER]         IF oldest_session_id IS NOT NULL THEN
...
ALTER TABLE public.user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'user';
...
RETURNS BOOLEAN 
LANGUAGE plpgsql 
-- [HEADER] SECURITY DEFINER -- Bypasses RLS
...
-- [GARBAGE]         NEW.id,
-- [GARBAGE]         NEW.email,
-- [GARBAGE]         NEW.raw_user_meta_data->>'full_name',
        'user' -- Default role
    )
-- [GARBAGE]     ON CONFLICT (id) DO NOTHING; -- Prevent errors if retry
    RETURN NEW;
END;
$$;
...
-- [HEADER] CREATE POLICY "Admins view all" 
-- [GARBAGE]     ON public.user_profiles FOR SELECT 
    USING (public.is_admin());

-- Policy: Admins update all
-- [HEADER] CREATE POLICY "Admins update all" 
-- [GARBAGE]     ON public.user_profiles FOR UPDATE 
    USING (public.is_admin());
...
-- [GARBAGE]                WHERE table_name = 'tenants' AND constraint_type = 'FOREIGN KEY') THEN
    ...
    ALTER TABLE public.tenants
-- [GARBAGE]     ADD CONSTRAINT tenants_property_id_fkey
-- [GARBAGE]     FOREIGN KEY (property_id)
-- [GARBAGE]     REFERENCES public.properties(id)
-- [GARBAGE]     ON DELETE SET NULL;
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE;

-- Backfill user_id for payments from contracts
UPDATE public.payments p
SET user_id = c.user_id
-- [GARBAGE] FROM public.contracts c
-- [GARBAGE] WHERE p.contract_id = c.id
-- [GARBAGE] AND p.user_id IS NULL;
...
RETURNS TEXT
LANGUAGE plpgsql
-- [GARBAGE] IMMUTABLE -- Optimization: Always returns same result for same input
AS $$
BEGIN
-- [HEADER]     IF user_agent IS NULL THEN
...
RETURNS TRIGGER
LANGUAGE plpgsql
-- [HEADER] SECURITY DEFINER -- Runs with admin privileges to delete other sessions
...
-- [GARBAGE]     session_count INT;
-- [GARBAGE]     oldest_session_id UUID;
-- [GARBAGE]     max_sessions_per_type INT := 1; -- Hardcoded limit: 1 per group
BEGIN
    -- Identify what kind of device is trying to log in
-- [GARBAGE]     new_device_type := public.get_device_type(NEW.user_agent);
...
-- [GARBAGE]     BEFORE INSERT ON auth.sessions
    FOR EACH ROW
...
-- [GARBAGE]     id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     type TEXT NOT NULL CHECK (type IN ('info', 'success', 'warning', 'error', 'action', 'urgent')),
-- [GARBAGE]     title TEXT NOT NULL,
-- [GARBAGE]     message TEXT NOT NULL,
-- [GARBAGE]     read_at TIMESTAMP WITH TIME ZONE,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]     metadata JSONB DEFAULT '{}'::jsonb
);
...
-- [HEADER] CREATE POLICY "Users can update their own notifications"
-- [GARBAGE]     ON public.notifications FOR UPDATE
    USING (auth.uid() = user_id);
...
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.linkage_type != 'none'
-- [GARBAGE]         AND (c.start_date + INTERVAL '1 year' = CURRENT_DATE OR c.start_date + INTERVAL '2 years' = CURRENT_DATE OR c.start_date + INTERVAL '3 years' = CURRENT_DATE)
    LOOP
        IF NOT EXISTS (SELECT 1 FROM public.notifications WHERE user_id = r.user_id AND metadata->>'contract_id' = r.id::text AND metadata->>'event' = 'index_update' AND metadata->>'date' = CURRENT_DATE::text) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
...
SET status = 'active' 
-- [GARBAGE] WHERE status = 'pending';

UPDATE public.contracts 
SET status = 'archived' 
-- [GARBAGE] WHERE status IN ('ended', 'terminated');
...
ALTER TABLE public.contracts 
-- [GARBAGE] ADD CONSTRAINT contracts_status_check 
-- [GARBAGE] CHECK (status IN ('active', 'archived'));
...
BEGIN
    ...
    ALTER TABLE public.user_profiles
-- [GARBAGE]     ADD COLUMN IF NOT EXISTS first_name TEXT,
-- [GARBAGE]     ADD COLUMN IF NOT EXISTS last_name TEXT;
...
-- [HEADER] CREATE POLICY "Admins full access to secure_documents"
-- [GARBAGE]     ON storage.objects
    FOR ALL
    USING (
-- [GARBAGE]         bucket_id = 'secure_documents' 
-- [GARBAGE]         AND 
-- [GARBAGE]         EXISTS (SELECT 1 FROM user_profiles WHERE id = auth.uid() AND role = 'admin')
    )
    WITH CHECK (
-- [GARBAGE]         bucket_id = 'secure_documents' 
-- [GARBAGE]         AND 
...
-- [HEADER] CREATE POLICY "Users view own secure documents"
-- [GARBAGE]     ON storage.objects
    FOR SELECT
    USING (
-- [GARBAGE]         bucket_id = 'secure_documents'
...
-- [HEADER] CREATE POLICY "Users upload own documents"
-- [GARBAGE]     ON storage.objects
    FOR INSERT
    WITH CHECK (
-- [GARBAGE]         bucket_id = 'secure_documents'
...
-- [GARBAGE]     id BIGSERIAL PRIMARY KEY,
-- [GARBAGE]     original_user_id UUID,
-- [GARBAGE]     email TEXT,
-- [GARBAGE]     phone TEXT,
-- [GARBAGE]     subscription_status_at_deletion TEXT,
-- [GARBAGE]     deleted_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [GARBAGE]         original_user_id,
-- [GARBAGE]         email,
-- [GARBAGE]         subcription_status_at_deletion
    )
    VALUES (
-- [GARBAGE]         OLD.id,
-- [GARBAGE]         OLD.email,
-- [GARBAGE]         OLD.subscription_status::text
    );
    RETURN OLD;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- [GARBAGE]     BEFORE DELETE ON user_profiles
    FOR EACH ROW
...
AS $$
DECLARE
-- [GARBAGE]     project_url text := 'https://mtxwavmmywiewjrsxchi.supabase.co'; -- Replace with your actual project URL or use a config table
-- [GARBAGE]     function_secret text := 'YOUR_FUNCTION_SECRET'; -- Ideally this is handled via vault or not needed if using net extension with service role
BEGIN
    ...
    PERFORM
-- [GARBAGE]       net.http_post(
-- [GARBAGE]         url := project_url || '/functions/v1/send-admin-alert',
-- [GARBAGE]         headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || current_setting('app.settings.service_role_key', true) || '"}',
-- [GARBAGE]         body := json_build_object(
            'type', 'INSERT',
            'table', 'user_profiles',
...
-- [GARBAGE]     AFTER INSERT ON public.user_profiles
    FOR EACH ROW
...
-- [GARBAGE] FROM pg_policies 
-- [GARBAGE] WHERE tablename IN ('properties', 'contracts', 'tenants', 'payments')
-- [GARBAGE] ORDER BY tablename, cmd;
...
-- [GARBAGE]     gender TEXT CHECK (gender IN ('male', 'female', 'unspecified')),
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     UNIQUE(user_id)
);
...
-- [HEADER] CREATE POLICY "Users can manage their own preferences"
-- [GARBAGE]     ON user_preferences
    FOR ALL
    USING (auth.uid() = user_id)
...
-- [GARBAGE]     BEFORE UPDATE ON user_preferences
    FOR EACH ROW
...
SELECT cron.schedule(
    ...
    SELECT
-- [GARBAGE]         net.http_post(
-- [GARBAGE]             url:='https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/fetch-index-data',
-- [GARBAGE]             headers:='{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFmdnJla3Z1Z2RqbndobmF1Y216Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njc0MzY0MTYsImV4cCI6MjA4MzAxMjQxNn0.xA3JI4iGElpIpZjVHLCA_FGw0hfmNUJTtw_fuLlhkoA"}'::jsonb,
-- [GARBAGE]             body:='{}'::jsonb
        ) as request_id;
    $$
);
...
-- [GARBAGE]     id UUID NOT NULL DEFAULT gen_random_uuid(),
-- [GARBAGE]     contract_id UUID NOT NULL REFERENCES public.contracts(id) ON DELETE CASCADE,
-- [GARBAGE]     amount NUMERIC NOT NULL,
-- [GARBAGE]     currency TEXT NOT NULL CHECK (currency IN ('ILS', 'USD', 'EUR')),
-- [GARBAGE]     due_date DATE NOT NULL,
-- [GARBAGE]     status TEXT NOT NULL CHECK (status IN ('pending', 'paid', 'overdue', 'cancelled')),
-- [GARBAGE]     paid_date DATE DEFAULT NULL,
-- [GARBAGE]     payment_method TEXT DEFAULT NULL,
-- [GARBAGE]     reference TEXT DEFAULT NULL,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
-- [GARBAGE]     CONSTRAINT payments_pkey PRIMARY KEY (id)
);
...
-- [HEADER] create policy "Allow public read access"
-- [GARBAGE]     on public.saved_calculations for select
    using (true);

-- Allow authenticated users to insert their own calculations
-- [HEADER] create policy "Allow authenticated insert"
-- [GARBAGE]     on public.saved_calculations for insert
    with check (auth.uid() = user_id);
...
-- [HEADER] create policy "Allow public insert"
-- [GARBAGE]     on public.saved_calculations for insert
    with check (
...
-- [GARBAGE]         WHERE tablename = 'index_data' 
-- [GARBAGE]         AND policyname = 'Allow public read access to index data'
    ) THEN
-- [HEADER]         CREATE POLICY "Allow public read access to index data"
-- [GARBAGE]           ON index_data
          FOR SELECT
-- [GARBAGE]           TO anon
          USING (true);
...
-- [GARBAGE]         net.http_post(
-- [GARBAGE]             url := 'https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/fetch-index-data',
-- [GARBAGE]             headers := jsonb_build_object(
...
-- [GARBAGE]     id TEXT PRIMARY KEY, -- 'free', 'pro', 'enterprise'
-- [GARBAGE]     name TEXT NOT NULL,
-- [GARBAGE]     price_monthly NUMERIC(10, 2) DEFAULT 0,
    
    -- Resource Limits (-1 for unlimited)
-- [GARBAGE]     max_properties INTEGER DEFAULT 1,
-- [GARBAGE]     max_tenants INTEGER DEFAULT 1,
-- [GARBAGE]     max_contracts INTEGER DEFAULT 1,
-- [GARBAGE]     max_sessions INTEGER DEFAULT 1,
    
    -- Modular Features
-- [GARBAGE]     features JSONB DEFAULT '{}'::jsonb, -- e.g. {"can_export": true, "ai_assistant": false}
...
-- [HEADER] CREATE POLICY "Public Read Plans" 
-- [GARBAGE]     ON subscription_plans FOR SELECT 
    USING (true);
...
-- [GARBAGE] ON CONFLICT (id) DO UPDATE SET
-- [GARBAGE]     name = EXCLUDED.name,
-- [GARBAGE]     price_monthly = EXCLUDED.price_monthly,
-- [GARBAGE]     max_properties = EXCLUDED.max_properties,
-- [GARBAGE]     max_tenants = EXCLUDED.max_tenants,
-- [GARBAGE]     max_contracts = EXCLUDED.max_contracts,
-- [GARBAGE]     max_sessions = EXCLUDED.max_sessions,
-- [GARBAGE]     features = EXCLUDED.features;
-- ============================================
...
ALTER TABLE user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS plan_id TEXT REFERENCES subscription_plans(id) DEFAULT 'free';
...
-- [GARBAGE]         id, email, full_name, role, subscription_status, plan_id
    )
    VALUES (
-- [GARBAGE]         NEW.id,
-- [GARBAGE]         NEW.email,
...
-- [GARBAGE]     session_count INT;
-- [GARBAGE]     oldest_session_id UUID;
-- [GARBAGE]     user_plan_limit INT;
BEGIN
    -- 1. Get User's Plan Limit
    SELECT sp.max_sessions
-- [GARBAGE]     INTO user_plan_limit
-- [GARBAGE]     FROM public.user_profiles up
-- [GARBAGE]     JOIN public.subscription_plans sp ON up.plan_id = sp.id
-- [GARBAGE]     WHERE up.id = NEW.user_id;

    -- Fallback if no plan found (shouldn't happen)
-- [HEADER]     IF user_plan_limit IS NULL THEN
-- [GARBAGE]         user_plan_limit := 1;
    END IF;
...
-- [GARBAGE]     email TEXT,
-- [GARBAGE]     full_name TEXT,
-- [GARBAGE]     role user_role,
-- [GARBAGE]     subscription_status subscription_status,
-- [GARBAGE]     plan_id TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ,
    
    -- Stats
-- [GARBAGE]     properties_count BIGINT,
-- [GARBAGE]     tenants_count BIGINT,
-- [GARBAGE]     contracts_count BIGINT
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- [GARBAGE]         up.id,
-- [GARBAGE]         up.email,
-- [GARBAGE]         up.full_name,
-- [GARBAGE]         up.role,
-- [GARBAGE]         up.subscription_status,
-- [GARBAGE]         up.plan_id,
-- [GARBAGE]         up.created_at,
        
        -- Counts (Coalesce to 0)
-- [GARBAGE]         COALESCE(p.count, 0) as properties_count,
-- [GARBAGE]         COALESCE(t.count, 0) as tenants_count,
-- [GARBAGE]         COALESCE(c.count, 0) as contracts_count
-- [GARBAGE]     FROM user_profiles up
    -- Join Property Counts
-- [GARBAGE]     LEFT JOIN (
        SELECT user_id, count(*) as count 
-- [GARBAGE]         FROM properties 
-- [GARBAGE]         GROUP BY user_id
    ) p ON up.id = p.user_id
    -- Join Tenant Counts
-- [GARBAGE]     LEFT JOIN (
        SELECT user_id, count(*) as count 
-- [GARBAGE]         FROM tenants 
-- [GARBAGE]         GROUP BY user_id
    ) t ON up.id = t.user_id
    -- Join Contract Counts
-- [GARBAGE]     LEFT JOIN (
        SELECT user_id, count(*) as count 
-- [GARBAGE]         FROM contracts 
-- [GARBAGE]         GROUP BY user_id
    ) c ON up.id = c.user_id
    
-- [GARBAGE]     ORDER BY up.created_at DESC;
END;
$$;
-- ============================================
...
ALTER TABLE user_profiles
-- [GARBAGE] ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP WITH TIME ZONE,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS account_status TEXT DEFAULT 'active' CHECK (account_status IN ('active', 'suspended', 'deleted'));
...
AS $$
DECLARE
-- [GARBAGE]     cutoff_date TIMESTAMP WITH TIME ZONE;
-- [GARBAGE]     user_record RECORD;
BEGIN
    -- Calculate cutoff date (14 days ago)
-- [GARBAGE]     cutoff_date := NOW() - INTERVAL '14 days';
    
    -- Find all users marked for deletion more than 14 days ago
    FOR user_record IN 
        SELECT id 
-- [GARBAGE]         FROM user_profiles 
-- [GARBAGE]         WHERE deleted_at IS NOT NULL 
-- [GARBAGE]         AND deleted_at < cutoff_date
-- [GARBAGE]         AND account_status = 'suspended'
    LOOP
...
-- [GARBAGE]         auth.uid(), 
        'delete_user', 
-- [GARBAGE]         jsonb_build_object('target_user_id', target_user_id, 'target_email', target_email)
    );
...
BEGIN
    IF (OLD.role IS DISTINCT FROM NEW.role) OR 
       ...
       (OLD.subscription_status IS DISTINCT FROM NEW.subscription_status) THEN
        ...
        INSERT INTO public.audit_logs (user_id, action, details)
        VALUES (
-- [GARBAGE]             auth.uid(), -- The admin performing the update
            'update_user_profile',
-- [GARBAGE]             jsonb_build_object(
...
-- [GARBAGE] AFTER UPDATE ON public.user_profiles
FOR EACH ROW
...
-- [GARBAGE]     id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL, -- Nullable for anonymous feedback
-- [GARBAGE]     message TEXT NOT NULL,
-- [GARBAGE]     type TEXT DEFAULT 'bug', -- 'bug', 'feature', 'other'
-- [GARBAGE]     status TEXT DEFAULT 'new', -- 'new', 'in_progress', 'resolved'
-- [GARBAGE]     screenshot_url TEXT,
-- [GARBAGE]     device_info JSONB
);
...
-- [HEADER] CREATE POLICY "Enable insert for everyone"
-- [GARBAGE] ON public.feedback FOR INSERT
-- [GARBAGE] TO public, anon, authenticated
WITH CHECK (true);

-- Allow Admins to see all
DROP POLICY IF EXISTS "Admins can view all feedback" ON public.feedback;
-- [HEADER] CREATE POLICY "Admins can view all feedback"
-- [GARBAGE] ON public.feedback FOR SELECT
-- [GARBAGE] TO authenticated
USING (
-- [GARBAGE]     EXISTS (
        SELECT 1 FROM public.user_profiles
...
-- [HEADER] CREATE POLICY "Admins can update feedback"
-- [GARBAGE] ON public.feedback FOR UPDATE
-- [GARBAGE] TO authenticated
USING (
-- [GARBAGE]     EXISTS (
        SELECT 1 FROM public.user_profiles
...
-- [HEADER] CREATE POLICY "Anyone can upload feedback screenshots"
-- [GARBAGE] ON storage.objects FOR INSERT
-- [GARBAGE] TO public, anon, authenticated
WITH CHECK ( bucket_id = 'feedback-screenshots' );

DROP POLICY IF EXISTS "Anyone can view feedback screenshots" ON storage.objects;
-- [HEADER] CREATE POLICY "Anyone can view feedback screenshots"
-- [GARBAGE] ON storage.objects FOR SELECT
-- [GARBAGE] TO public, anon, authenticated
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_media_mb INTEGER DEFAULT -1,      -- -1 for unlimited within global cap
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_utilities_mb INTEGER DEFAULT -1,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_maintenance_mb INTEGER DEFAULT -1,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_documents_mb INTEGER DEFAULT -1;

-- Update existing plans with sensible defaults
-- (Assuming Free gets restricted media but more room for documents)
UPDATE subscription_plans SET 
-- [GARBAGE]     max_media_mb = 50,         -- 50MB for photos/video max on free
-- [GARBAGE]     max_utilities_mb = 20,     -- 20MB for bills
-- [GARBAGE]     max_maintenance_mb = 20,   -- 20MB for repairs
-- [GARBAGE]     max_documents_mb = 10      -- 10MB for contracts
-- [GARBAGE] WHERE id = 'free';

-- Update the quota check function to support categories
CREATE OR REPLACE FUNCTION check_storage_quota(
-- [GARBAGE]     p_user_id UUID,
-- [GARBAGE]     p_file_size BIGINT,
-- [GARBAGE]     p_category TEXT DEFAULT NULL
) RETURNS BOOLEAN AS $$
DECLARE
-- [GARBAGE]     v_total_usage BIGINT;
-- [GARBAGE]     v_cat_usage BIGINT;
-- [GARBAGE]     v_max_total_mb INTEGER;
-- [GARBAGE]     v_max_cat_mb INTEGER;
-- [GARBAGE]     v_col_name TEXT;
BEGIN
    -- 1. Get current usage and plan limits
    SELECT 
-- [GARBAGE]         u.total_bytes,
-- [GARBAGE]         CASE 
-- [GARBAGE]             WHEN p_category IN ('photo', 'video') THEN u.media_bytes
-- [GARBAGE]             WHEN p_category LIKE 'utility_%' THEN u.utilities_bytes
-- [GARBAGE]             WHEN p_category = 'maintenance' THEN u.maintenance_bytes
-- [GARBAGE]             ELSE u.documents_bytes
-- [GARBAGE]         END,
-- [GARBAGE]         s.max_storage_mb,
-- [GARBAGE]         CASE 
-- [GARBAGE]             WHEN p_category IN ('photo', 'video') THEN s.max_media_mb
-- [GARBAGE]             WHEN p_category LIKE 'utility_%' THEN s.max_utilities_mb
-- [GARBAGE]             WHEN p_category = 'maintenance' THEN s.max_maintenance_mb
-- [GARBAGE]             ELSE s.max_documents_mb
        END
-- [GARBAGE]     INTO 
-- [GARBAGE]         v_total_usage,
-- [GARBAGE]         v_cat_usage,
-- [GARBAGE]         v_max_total_mb,
-- [GARBAGE]         v_max_cat_mb
-- [GARBAGE]     FROM user_profiles up
-- [GARBAGE]     JOIN subscription_plans s ON up.plan_id = s.id
-- [GARBAGE]     LEFT JOIN user_storage_usage u ON u.user_id = up.id
-- [GARBAGE]     WHERE up.id = p_user_id;

    -- Initialize usage if user has no records yet
-- [GARBAGE]     v_total_usage := COALESCE(v_total_usage, 0);
-- [GARBAGE]     v_cat_usage := COALESCE(v_cat_usage, 0);
...
ALTER TABLE subscription_plans
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_storage_mb INTEGER DEFAULT 100,  -- MB per user
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_file_size_mb INTEGER DEFAULT 10; -- MB per file

-- Update existing plans with storage limits
UPDATE subscription_plans SET 
-- [GARBAGE]     max_storage_mb = 100,    -- 100MB total
-- [GARBAGE]     max_file_size_mb = 5     -- 5MB per file
-- [GARBAGE] WHERE id = 'free';

UPDATE subscription_plans SET 
-- [GARBAGE]     max_storage_mb = 5120,   -- 5GB total
-- [GARBAGE]     max_file_size_mb = 50    -- 50MB per file
-- [GARBAGE] WHERE id = 'pro';

UPDATE subscription_plans SET 
-- [GARBAGE]     max_storage_mb = -1,     -- Unlimited
-- [GARBAGE]     max_file_size_mb = 500   -- 500MB per file
-- [GARBAGE] WHERE id = 'enterprise';

-- Comments
-- CREATE TABLE IF NOT EXISTS for short URLs
CREATE TABLE IF NOT EXISTS calculation_shares (
-- [GARBAGE]     id TEXT PRIMARY KEY, -- Short ID (e.g., "abc123")
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
-- [GARBAGE]     calculation_data JSONB NOT NULL,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     expires_at TIMESTAMPTZ DEFAULT (NOW() + INTERVAL '30 days'),
-- [GARBAGE]     view_count INTEGER DEFAULT 0
);
...
-- [HEADER] CREATE POLICY "Public can view calculation shares"
-- [GARBAGE]     ON calculation_shares FOR SELECT
    USING (true);

-- Authenticated users can create
-- [HEADER] CREATE POLICY "Authenticated users can create shares"
-- [GARBAGE]     ON calculation_shares FOR INSERT
    WITH CHECK (auth.uid() IS NOT NULL);

-- Users can update their own shares (for view count)
-- [HEADER] CREATE POLICY "Anyone can update view count"
-- [GARBAGE]     ON calculation_shares FOR UPDATE
    USING (true)
    WITH CHECK (true);
...
RETURNS TEXT AS $$
DECLARE
-- [GARBAGE]     chars TEXT := 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
-- [GARBAGE]     result TEXT := '';
-- [GARBAGE]     i INTEGER;
BEGIN
-- [HEADER]     FOR i IN 1..length LOOP
-- [GARBAGE]         result := result || substr(chars, floor(random() * length(chars) + 1)::int, 1);
    END LOOP;
    RETURN result;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TEXT AS $$
DECLARE
-- [GARBAGE]     v_short_id TEXT;
-- [GARBAGE]     v_max_attempts INTEGER := 10;
-- [GARBAGE]     v_attempt INTEGER := 0;
BEGIN
    LOOP
-- [GARBAGE]         v_short_id := generate_short_id(6);
...
RETURNS INTEGER AS $$
DECLARE
-- [GARBAGE]     v_deleted_count INTEGER;
BEGIN
    DELETE FROM calculation_shares
-- [GARBAGE]     WHERE expires_at < NOW();
    
-- [GARBAGE]     GET DIAGNOSTICS v_deleted_count = ROW_COUNT;
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     property_id UUID NOT NULL REFERENCES properties(id) ON DELETE CASCADE,
    
    -- Document Classification
-- [GARBAGE]     category TEXT NOT NULL CHECK (category IN (
    ...
    -- Storage Info
-- [GARBAGE]     storage_bucket TEXT NOT NULL,
-- [GARBAGE]     storage_path TEXT NOT NULL,
-- [GARBAGE]     file_name TEXT NOT NULL,
-- [GARBAGE]     file_size BIGINT,
-- [GARBAGE]     mime_type TEXT,
...
-- [GARBAGE]     tags TEXT[],
    ...
    -- Date Info
-- [GARBAGE]     document_date DATE,  -- When the bill/invoice was issued
-- [GARBAGE]     period_start DATE,   -- For recurring bills (e.g., monthly utility)
-- [GARBAGE]     period_end DATE,
    
    -- Financial Data (for bills/invoices)
-- [GARBAGE]     amount DECIMAL(10,2),
-- [GARBAGE]     currency TEXT DEFAULT 'ILS',
-- [GARBAGE]     paid BOOLEAN DEFAULT false,
-- [GARBAGE]     payment_date DATE,
    
    -- Maintenance Specific
-- [GARBAGE]     vendor_name TEXT,
-- [GARBAGE]     issue_type TEXT,     -- e.g., "plumbing", "electrical", "painting"
...
-- [HEADER] CREATE POLICY "Users can view their property documents"
-- [GARBAGE]     ON property_documents FOR SELECT
    USING (auth.uid() = user_id);

-- [HEADER] CREATE POLICY "Users can insert their property documents"
-- [GARBAGE]     ON property_documents FOR INSERT
    WITH CHECK (auth.uid() = user_id);

-- [HEADER] CREATE POLICY "Users can update their property documents"
-- [GARBAGE]     ON property_documents FOR UPDATE
    USING (auth.uid() = user_id);

-- [HEADER] CREATE POLICY "Users can delete their property documents"
-- [GARBAGE]     ON property_documents FOR DELETE
    USING (auth.uid() = user_id);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     property_id UUID NOT NULL REFERENCES properties(id) ON DELETE CASCADE,
-- [GARBAGE]     category TEXT NOT NULL, -- e.g., 'utility_electric', 'maintenance', 'media', 'other'
-- [GARBAGE]     name TEXT NOT NULL, -- The user-friendly subject/title
-- [GARBAGE]     folder_date DATE NOT NULL DEFAULT CURRENT_DATE,
-- [GARBAGE]     description TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [HEADER] CREATE POLICY "Users can view folders for their properties"
-- [GARBAGE]     ON document_folders FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
-- [HEADER]             SELECT 1 FROM properties p
-- [GARBAGE]             WHERE p.id = document_folders.property_id
-- [GARBAGE]             AND p.user_id = auth.uid()
        )
    );

-- [HEADER] CREATE POLICY "Users can insert folders for their properties"
-- [GARBAGE]     ON document_folders FOR INSERT
    WITH CHECK (
-- [GARBAGE]         EXISTS (
-- [HEADER]             SELECT 1 FROM properties p
-- [GARBAGE]             WHERE p.id = document_folders.property_id
...
-- [HEADER] CREATE POLICY "Users can update folders for their properties"
-- [GARBAGE]     ON document_folders FOR UPDATE
    USING (
-- [GARBAGE]         EXISTS (
-- [HEADER]             SELECT 1 FROM properties p
-- [GARBAGE]             WHERE p.id = document_folders.property_id
...
-- [HEADER] CREATE POLICY "Users can delete folders for their properties"
-- [GARBAGE]     ON document_folders FOR DELETE
    USING (
-- [GARBAGE]         EXISTS (
-- [HEADER]             SELECT 1 FROM properties p
-- [GARBAGE]             WHERE p.id = document_folders.property_id
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS folder_id UUID REFERENCES document_folders(id) ON DELETE CASCADE;
...
-- [GARBAGE]     id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
-- [GARBAGE]     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     property_id UUID NOT NULL REFERENCES public.properties(id) ON DELETE CASCADE,
-- [GARBAGE]     drive_file_id TEXT NOT NULL,
-- [GARBAGE]     drive_web_view_link TEXT NOT NULL,
-- [GARBAGE]     drive_thumbnail_link TEXT,
-- [GARBAGE]     name TEXT NOT NULL,
-- [GARBAGE]     mime_type TEXT,
-- [GARBAGE]     size BIGINT,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
...
-- Policies
-- [HEADER] CREATE POLICY "Users can view their own property media"
-- [GARBAGE]     ON public.property_media FOR SELECT
    USING (auth.uid() = user_id);

-- [HEADER] CREATE POLICY "Users can insert their own property media"
-- [GARBAGE]     ON public.property_media FOR INSERT
    WITH CHECK (auth.uid() = user_id);

-- [HEADER] CREATE POLICY "Users can delete their own property media"
-- [GARBAGE]     ON public.property_media FOR DELETE
    USING (auth.uid() = user_id);
...
-- [GARBAGE]     slug TEXT PRIMARY KEY,
-- [GARBAGE]     original_url TEXT NOT NULL,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
-- [GARBAGE]     expires_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now() + interval '90 days') NOT NULL,
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL -- Optional: track who created it
);
...
-- [HEADER] CREATE POLICY "Public can read short links"
-- [GARBAGE] ON public.short_links FOR SELECT
USING (true);
...
-- [HEADER] CREATE POLICY "Authenticated users can create short links"
-- [GARBAGE] ON public.short_links FOR INSERT
...
-- [HEADER] CREATE POLICY "Public can create short links"
-- [GARBAGE] ON public.short_links FOR INSERT
WITH CHECK (true);
...
-- [GARBAGE]     user_id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     total_bytes BIGINT DEFAULT 0,
-- [GARBAGE]     file_count INTEGER DEFAULT 0,
-- [GARBAGE]     last_calculated_at TIMESTAMPTZ DEFAULT NOW(),
    
    -- Breakdown by category
-- [GARBAGE]     media_bytes BIGINT DEFAULT 0,
-- [GARBAGE]     utilities_bytes BIGINT DEFAULT 0,
-- [GARBAGE]     maintenance_bytes BIGINT DEFAULT 0,
-- [GARBAGE]     documents_bytes BIGINT DEFAULT 0,
...
-- [HEADER] CREATE POLICY "Users can view their own storage usage"
-- [GARBAGE]     ON user_storage_usage FOR SELECT
    USING (auth.uid() = user_id);
...
-- [GARBAGE]         ON CONFLICT (user_id) DO UPDATE SET
-- [GARBAGE]             total_bytes = user_storage_usage.total_bytes + NEW.file_size,
-- [GARBAGE]             file_count = user_storage_usage.file_count + 1,
-- [GARBAGE]             updated_at = NOW();
            
-- [GARBAGE]     ELSIF TG_OP = 'DELETE' THEN
        UPDATE user_storage_usage
        SET 
-- [GARBAGE]             total_bytes = GREATEST(0, total_bytes - OLD.file_size),
-- [GARBAGE]             file_count = GREATEST(0, file_count - 1),
-- [GARBAGE]             updated_at = NOW()
-- [GARBAGE]         WHERE user_id = OLD.user_id;
    END IF;
...
-- [GARBAGE] AFTER INSERT OR DELETE ON property_documents
...
DECLARE
-- [GARBAGE]     v_current_usage BIGINT;
-- [GARBAGE]     v_max_storage_mb INTEGER;
-- [GARBAGE]     v_max_storage_bytes BIGINT;
BEGIN
    -- Get current usage
    SELECT COALESCE(total_bytes, 0)
-- [GARBAGE]     INTO v_current_usage
-- [GARBAGE]     FROM user_storage_usage
-- [GARBAGE]     WHERE user_id = p_user_id;
    
    -- Get plan limit
    SELECT sp.max_storage_mb
-- [GARBAGE]     INTO v_max_storage_mb
-- [GARBAGE]     FROM user_profiles up
-- [GARBAGE]     JOIN subscription_plans sp ON up.plan_id = sp.id
-- [GARBAGE]     WHERE up.id = p_user_id;
...
-- [GARBAGE]     v_max_storage_bytes := v_max_storage_mb * 1024 * 1024;
...
-- [GARBAGE] NOTIFY pgrst, 'reload schema';
...
DECLARE
-- [GARBAGE]     v_col TEXT;
-- [GARBAGE]     v_size BIGINT;
-- [GARBAGE]     v_user_id UUID;
-- [GARBAGE]     v_cat TEXT;
BEGIN
    IF TG_OP = 'INSERT' THEN
-- [GARBAGE]         v_size := NEW.file_size;
-- [GARBAGE]         v_user_id := NEW.user_id;
-- [GARBAGE]         v_cat := NEW.category;
-- [GARBAGE]     ELSE
-- [GARBAGE]         v_size := OLD.file_size;
-- [GARBAGE]         v_user_id := OLD.user_id;
-- [GARBAGE]         v_cat := OLD.category;
    END IF;

    -- Determine which column to update based on category
    IF v_cat IN ('photo', 'video') THEN
-- [GARBAGE]         v_col := 'media_bytes';
-- [GARBAGE]     ELSIF v_cat LIKE 'utility_%' THEN
-- [GARBAGE]         v_col := 'utilities_bytes';
-- [GARBAGE]     ELSIF v_cat = 'maintenance' THEN
-- [GARBAGE]         v_col := 'maintenance_bytes';
-- [GARBAGE]     ELSE
-- [GARBAGE]         v_col := 'documents_bytes';
    END IF;
    ...
    IF TG_OP = 'INSERT' THEN
        EXECUTE format('
            ...
            VALUES ($1, $2, 1, $2)
-- [GARBAGE]             ON CONFLICT (user_id) DO UPDATE SET
-- [GARBAGE]                 total_bytes = user_storage_usage.total_bytes + $2,
-- [GARBAGE]                 file_count = user_storage_usage.file_count + 1,
...
-- [GARBAGE]     ELSIF TG_OP = 'DELETE' THEN
        EXECUTE format('
            UPDATE user_storage_usage
            SET 
-- [GARBAGE]                 total_bytes = GREATEST(0, total_bytes - $1),
-- [GARBAGE]                 file_count = GREATEST(0, file_count - 1),
                %I = GREATEST(0, %I - $1),
-- [GARBAGE]                 updated_at = NOW()
-- [GARBAGE]             WHERE user_id = $2
...
ALTER TABLE public.contracts
-- [GARBAGE] ADD COLUMN IF NOT EXISTS extension_option_start DATE;

-- AI Chat Usage Tracking
CREATE TABLE IF NOT EXISTS ai_chat_usage (
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     message_count INTEGER DEFAULT 0,
-- [GARBAGE]     tokens_used INTEGER DEFAULT 0,
-- [GARBAGE]     last_reset_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     UNIQUE(user_id)
);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     tier_name TEXT NOT NULL UNIQUE,
-- [GARBAGE]     monthly_message_limit INTEGER NOT NULL,
-- [GARBAGE]     monthly_token_limit INTEGER NOT NULL,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [GARBAGE] ON CONFLICT (tier_name) DO NOTHING;

-- Function to check and log AI usage
CREATE OR REPLACE FUNCTION check_ai_chat_usage(
-- [GARBAGE]     p_user_id UUID,
-- [GARBAGE]     p_tokens_used INTEGER DEFAULT 500
)
RETURNS JSON AS $$
DECLARE
-- [GARBAGE]     v_usage RECORD;
-- [GARBAGE]     v_limit RECORD;
-- [GARBAGE]     v_user_tier TEXT;
-- [GARBAGE]     v_result JSON;
BEGIN
    -- Get user's subscription tier
    SELECT subscription_tier INTO v_user_tier
-- [GARBAGE]     FROM user_profiles
-- [GARBAGE]     WHERE id = p_user_id;
    
    -- Default to free if no tier found
-- [GARBAGE]     v_user_tier := COALESCE(v_user_tier, 'free');
    
    -- Get limits for this tier
    SELECT * INTO v_limit
-- [GARBAGE]     FROM ai_usage_limits
-- [GARBAGE]     WHERE tier_name = v_user_tier;
    
    -- Get or create usage record
    INSERT INTO ai_chat_usage (user_id, message_count, tokens_used)
    VALUES (p_user_id, 0, 0)
-- [GARBAGE]     ON CONFLICT (user_id) DO NOTHING;
    
    SELECT * INTO v_usage
-- [GARBAGE]     FROM ai_chat_usage
-- [GARBAGE]     WHERE user_id = p_user_id;
    ...
    IF v_usage.last_reset_at < DATE_TRUNC('month', NOW()) THEN
        ...
        SET message_count = 0,
-- [GARBAGE]             tokens_used = 0,
-- [GARBAGE]             last_reset_at = NOW(),
-- [GARBAGE]             updated_at = NOW()
-- [GARBAGE]         WHERE user_id = p_user_id;
        
-- [GARBAGE]         v_usage.message_count := 0;
-- [GARBAGE]         v_usage.tokens_used := 0;
    END IF;
...
-- [GARBAGE]         v_result := json_build_object(
            'allowed', false,
...
-- [GARBAGE]         tokens_used = tokens_used + p_tokens_used,
-- [GARBAGE]         updated_at = NOW()
-- [GARBAGE]     WHERE user_id = p_user_id;
...
-- [HEADER] CREATE POLICY "Users can view own AI usage"
-- [GARBAGE]     ON ai_chat_usage FOR SELECT
    USING (auth.uid() = user_id);

-- Admins can view all usage
-- [HEADER] CREATE POLICY "Admins can view all AI usage"
-- [GARBAGE]     ON ai_chat_usage FOR ALL
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
-- [GARBAGE]             WHERE id = auth.uid() AND role = 'admin'
...
-- [HEADER] CREATE POLICY "Anyone can view AI limits"
-- [GARBAGE]     ON ai_usage_limits FOR SELECT
-- [GARBAGE]     TO authenticated
    USING (true);

-- Only admins can modify limits
-- [HEADER] CREATE POLICY "Admins can modify AI limits"
-- [GARBAGE]     ON ai_usage_limits FOR ALL
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
-- [GARBAGE]             WHERE id = auth.uid() AND role = 'admin'
...
ALTER TABLE public.user_profiles
-- [GARBAGE] ADD COLUMN IF NOT EXISTS notification_preferences JSONB DEFAULT '{"contract_expiry_days": 60, "rent_due_days": 3}';
...
AS $$
DECLARE
-- [GARBAGE]     expiring_contract RECORD;
-- [GARBAGE]     count_new integer := 0;
-- [GARBAGE]     pref_days integer;
BEGIN
    FOR expiring_contract IN
        SELECT 
-- [GARBAGE]             c.id, 
-- [GARBAGE]             c.end_date, 
-- [GARBAGE]             c.property_id, 
-- [GARBAGE]             p.user_id, 
-- [GARBAGE]             p.address, 
-- [GARBAGE]             p.city,
-- [GARBAGE]             up.notification_preferences
-- [GARBAGE]         FROM public.contracts c
-- [GARBAGE]         JOIN public.properties p ON c.property_id = p.id
-- [GARBAGE]         JOIN public.user_profiles up ON p.user_id = up.id
-- [GARBAGE]         WHERE c.status = 'active'
    LOOP
        -- Extract preference, default to 60, cap at 180
-- [GARBAGE]         pref_days := COALESCE((expiring_contract.notification_preferences->>'contract_expiry_days')::int, 60);
...
-- [GARBAGE]            AND expiring_contract.end_date >= CURRENT_DATE THEN
           
            IF NOT EXISTS (
                SELECT 1 
-- [GARBAGE]                 FROM public.notifications n 
-- [GARBAGE]                 WHERE n.user_id = expiring_contract.user_id
-- [GARBAGE]                 AND n.type = 'warning'
-- [GARBAGE]                 AND n.metadata->>'contract_id' = expiring_contract.id::text
...
-- [GARBAGE]                  AND n.created_at > (CURRENT_DATE - INTERVAL '6 months') -- Simple debounce for same contract
            ) THEN
...
-- [GARBAGE]                     expiring_contract.user_id,
                    'warning',
...
-- [GARBAGE]                     jsonb_build_object('contract_id', expiring_contract.id)
                );
-- [GARBAGE]                 count_new := count_new + 1;
            END IF;
        END IF;
    END LOOP;
END;
...
BEGIN
    FOR due_payment IN
        SELECT 
-- [GARBAGE]             pay.id,
-- [GARBAGE]             pay.due_date,
-- [GARBAGE]             pay.amount,
-- [GARBAGE]             pay.currency,
-- [GARBAGE]             p.user_id,
-- [GARBAGE]             p.address,
-- [GARBAGE]             up.notification_preferences
-- [GARBAGE]         FROM public.payments pay
-- [GARBAGE]         JOIN public.contracts c ON pay.contract_id = c.id
-- [GARBAGE]         JOIN public.properties p ON c.property_id = p.id
-- [GARBAGE]         JOIN public.user_profiles up ON p.user_id = up.id
-- [GARBAGE]         WHERE pay.status = 'pending'
    LOOP
        -- Extract preference, default to 3, cap at 180 (though less makes sense for rent)
-- [GARBAGE]         pref_days := COALESCE((due_payment.notification_preferences->>'rent_due_days')::int, 3);
        IF pref_days > 180 THEN pref_days := 180; END IF;

        IF due_payment.due_date <= (CURRENT_DATE + (pref_days || ' days')::interval)
-- [GARBAGE]            AND due_payment.due_date >= CURRENT_DATE THEN

            IF NOT EXISTS (
                SELECT 1 
-- [GARBAGE]                 FROM public.notifications n 
-- [GARBAGE]                 WHERE n.user_id = due_payment.user_id
-- [GARBAGE]                 AND n.type = 'info'
-- [GARBAGE]                 AND n.metadata->>'payment_id' = due_payment.id::text
            ) THEN
                INSERT INTO public.notifications (
-- [GARBAGE]                     user_id,
-- [GARBAGE]                     type,
...
-- [GARBAGE]                     jsonb_build_object('payment_id', due_payment.id)
                );
-- [GARBAGE]                 count_new := count_new + 1;
            END IF;
        END IF;
...
-- [GARBAGE]     FROM public.property_documents
-- [GARBAGE]     WHERE user_id = p_user_id;
...
AS $$
DECLARE
-- [GARBAGE]     income_stats RECORD;
-- [GARBAGE]     doc_counts JSONB;
BEGIN
    -- 1. Get Income Stats
    SELECT 
-- [GARBAGE]         COALESCE(SUM(amount) FILTER (WHERE status = 'paid'), 0) as collected,
-- [GARBAGE]         COALESCE(SUM(amount) FILTER (WHERE status = 'pending'), 0) as pending,
-- [GARBAGE]         COALESCE(SUM(amount) FILTER (WHERE status IN ('paid', 'pending')), 0) as total
-- [GARBAGE]     INTO income_stats
-- [GARBAGE]     FROM public.payments
-- [GARBAGE]     WHERE user_id = p_user_id
-- [GARBAGE]     AND due_date >= date_trunc('month', now())
-- [GARBAGE]     AND due_date < date_trunc('month', now() + interval '1 month');

    -- 2. Get Document Counts (reuse RPC logic)
-- [GARBAGE]     doc_counts := public.get_property_document_counts(p_user_id);
...
-- [GARBAGE]         WHERE c.status = 'active'
        -- Changed to 60 days
-- [GARBAGE]         AND c.end_date <= (CURRENT_DATE + INTERVAL '60 days')
-- [GARBAGE]         AND c.end_date >= CURRENT_DATE
    LOOP
        IF NOT EXISTS (
            SELECT 1 
-- [GARBAGE]             FROM public.notifications n 
...
-- [GARBAGE]             AND n.type = 'warning'
-- [GARBAGE]             AND n.metadata->>'contract_id' = expiring_contract.id::text
-- [GARBAGE]             AND n.title = 'Contract Expiring Soon' 
        ) THEN
            INSERT INTO public.notifications (
-- [GARBAGE]                 user_id,
-- [GARBAGE]                 type,
...
-- [GARBAGE]         JOIN public.properties p ON c.property_id = p.id
-- [GARBAGE]         WHERE pay.status = 'pending'
-- [GARBAGE]         AND pay.due_date <= (CURRENT_DATE + INTERVAL '3 days')
-- [GARBAGE]         AND pay.due_date >= CURRENT_DATE
    LOOP
...
ALTER TABLE public.contracts
-- [GARBAGE] ADD COLUMN IF NOT EXISTS extension_option_end DATE;


-- 2. Add extension_option_end_days to notification preferences
UPDATE public.user_profiles
SET notification_preferences = jsonb_set(
-- [GARBAGE]     COALESCE(notification_preferences, '{}'::jsonb),
    '{extension_option_end_days}',
    '7'
)
-- [GARBAGE] WHERE notification_preferences IS NULL 
-- [GARBAGE]    OR NOT notification_preferences ? 'extension_option_end_days';
...
AS $$
DECLARE
-- [GARBAGE]     deadline_record RECORD;
-- [GARBAGE]     count_new integer := 0;
-- [GARBAGE]     pref_days integer;
BEGIN
    FOR deadline_record IN
        SELECT 
-- [GARBAGE]             c.id, 
-- [GARBAGE]             c.extension_option_end,
-- [GARBAGE]             c.property_id, 
-- [GARBAGE]             p.user_id, 
-- [GARBAGE]             p.address,
-- [GARBAGE]             up.notification_preferences
...
-- [GARBAGE]         JOIN public.user_profiles up ON p.user_id = up.id
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.extension_option_end IS NOT NULL
    LOOP
        -- Extract preference, default to 7, cap at 180
-- [GARBAGE]         pref_days := COALESCE((deadline_record.notification_preferences->>'extension_option_end_days')::int, 7);
...
-- [GARBAGE]            AND deadline_record.extension_option_end >= CURRENT_DATE THEN
           
            IF NOT EXISTS (
                SELECT 1 
-- [GARBAGE]                 FROM public.notifications n 
-- [GARBAGE]                 WHERE n.user_id = deadline_record.user_id
-- [GARBAGE]                 AND n.type = 'warning'
-- [GARBAGE]                 AND n.metadata->>'contract_id' = deadline_record.id::text
-- [GARBAGE]                 AND n.title = 'Extension Option Deadline Approaching'
-- [GARBAGE]                 AND n.created_at > (CURRENT_DATE - INTERVAL '6 months')
            ) THEN
                INSERT INTO public.notifications (
-- [GARBAGE]                     user_id,
-- [GARBAGE]                     type,
...
-- [GARBAGE]                     deadline_record.user_id,
                    'warning',
...
-- [GARBAGE]                     jsonb_build_object('contract_id', deadline_record.id)
                );
-- [GARBAGE]                 count_new := count_new + 1;
            END IF;
        END IF;
...
-- [GARBAGE] WHERE notification_preferences IS NULL 
-- [GARBAGE]    OR NOT notification_preferences ? 'extension_option_days';
...
AS $$
DECLARE
-- [GARBAGE]     extension_record RECORD;
-- [GARBAGE]     count_new integer := 0;
-- [GARBAGE]     pref_days integer;
BEGIN
    FOR extension_record IN
        SELECT 
-- [GARBAGE]             c.id, 
-- [GARBAGE]             c.extension_option_start,
-- [GARBAGE]             c.property_id, 
-- [GARBAGE]             p.user_id, 
-- [GARBAGE]             p.address,
-- [GARBAGE]             up.notification_preferences
...
-- [GARBAGE]         JOIN public.user_profiles up ON p.user_id = up.id
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.extension_option_start IS NOT NULL
    LOOP
        -- Extract preference, default to 30, cap at 180
-- [GARBAGE]         pref_days := COALESCE((extension_record.notification_preferences->>'extension_option_days')::int, 30);
        IF pref_days > 180 THEN pref_days := 180; END IF;
        IF pref_days < 1 THEN pref_days := 1; END IF;
...
-- [GARBAGE]            AND extension_record.extension_option_start >= CURRENT_DATE THEN
           
            IF NOT EXISTS (
                SELECT 1 
-- [GARBAGE]                 FROM public.notifications n 
-- [GARBAGE]                 WHERE n.user_id = extension_record.user_id
-- [GARBAGE]                 AND n.type = 'info'
-- [GARBAGE]                 AND n.metadata->>'contract_id' = extension_record.id::text
-- [GARBAGE]                 AND n.title = 'Extension Option Available'
-- [GARBAGE]                 AND n.created_at > (CURRENT_DATE - INTERVAL '6 months')
            ) THEN
                INSERT INTO public.notifications (
-- [GARBAGE]                     user_id,
...
-- [GARBAGE]                     extension_record.user_id,
                    'info',
...
-- [GARBAGE]                     jsonb_build_object('contract_id', extension_record.id)
                );
-- [GARBAGE]                 count_new := count_new + 1;
            END IF;
        END IF;
...
ALTER TABLE subscription_plans 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_ai_scans INTEGER DEFAULT 5;
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     feature_name TEXT NOT NULL, -- 'bill_scan', 'contract_analysis', etc.
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Policies
-- [HEADER] CREATE POLICY "Users can view their own usage logs"
-- [GARBAGE]     ON ai_usage_logs FOR SELECT
    USING (auth.uid() = user_id);
...
AS $$
DECLARE
-- [GARBAGE]     v_limit INTEGER;
-- [GARBAGE]     v_current_usage INTEGER;
-- [GARBAGE]     v_month_start TIMESTAMPTZ;
-- [GARBAGE]     v_requester_role TEXT;
BEGIN
    -- SECURITY CHECK: 
...
ALTER TABLE public.user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS is_super_admin BOOLEAN DEFAULT false;
...
AS $$
DECLARE
-- [GARBAGE]     total_mrr decimal := 0;
-- [GARBAGE]     total_users int := 0;
-- [GARBAGE]     active_subs int := 0;
-- [GARBAGE]     new_users_30d int := 0;
-- [GARBAGE]     churn_rate decimal := 0; -- Placeholder for now
-- [GARBAGE]     is_super boolean;
BEGIN
...
-- [HEADER]     IF is_super IS NOT TRUE THEN
...
-- [GARBAGE]     FROM user_profiles 
-- [GARBAGE]     WHERE plan_id IS NOT NULL 
-- [GARBAGE]     AND plan_id NOT IN ('free', 'free_forever')
-- [GARBAGE]     AND subscription_status = 'active';
    
    -- 3. MRR Calculation
    -- Sum of price_monthly for all active users based on their plan_id
    SELECT COALESCE(SUM(sp.price_monthly), 0)
-- [GARBAGE]     INTO total_mrr
-- [GARBAGE]     FROM user_profiles up
-- [GARBAGE]     JOIN subscription_plans sp ON up.plan_id = sp.id
-- [GARBAGE]     WHERE up.subscription_status = 'active';
    
    -- 4. Growth (New users in last 30 days)
    SELECT COUNT(*) INTO new_users_30d
-- [GARBAGE]     FROM user_profiles
-- [GARBAGE]     WHERE created_at > (NOW() - INTERVAL '30 days');
...
DECLARE
-- [GARBAGE]     result JSON;
-- [GARBAGE]     total_users_count INTEGER;
-- [GARBAGE]     total_contracts_count INTEGER;
-- [GARBAGE]     total_revenue_amount NUMERIC;
-- [GARBAGE]     active_users_count INTEGER;
BEGIN
...
-- [GARBAGE]     FROM user_profiles
-- [GARBAGE]     WHERE deleted_at IS NULL;
...
-- [GARBAGE]     FROM payments
-- [GARBAGE]     WHERE status = 'paid';

    -- Get active users (users who logged in within last 30 days)
    SELECT COUNT(*) INTO active_users_count
-- [GARBAGE]     FROM user_profiles
-- [GARBAGE]     WHERE deleted_at IS NULL
-- [GARBAGE]     AND updated_at > NOW() - INTERVAL '30 days';

    -- Build JSON result
-- [GARBAGE]     result := json_build_object(
...
-- [HEADER] CREATE POLICY "Admins can view all storage usage"
-- [GARBAGE]     ON public.user_storage_usage FOR SELECT
    USING (public.is_admin());

-- audit_logs
ALTER TABLE public.audit_logs ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Admins can view all audit logs" ON public.audit_logs;
-- [HEADER] CREATE POLICY "Admins can view all audit logs"
-- [GARBAGE]     ON public.audit_logs FOR SELECT
    USING (public.is_admin());
...
AS $$
DECLARE
-- [GARBAGE]     project_url text := 'https://tipnjnfbbnbskdlodrww.supabase.co'; -- UPDATED TO CORRECT PROJECT
BEGIN
    PERFORM
-- [GARBAGE]       net.http_post(
-- [GARBAGE]         url := project_url || '/functions/v1/send-admin-alert',
...
AS $$
DECLARE
-- [GARBAGE]     project_url text := 'https://tipnjnfbbnbskdlodrww.supabase.co';
-- [GARBAGE]     user_email text;
BEGIN
...
-- [HEADER]         IF user_email IS NOT NULL THEN
            PERFORM
-- [GARBAGE]               net.http_post(
-- [GARBAGE]                 url := project_url || '/functions/v1/send-notification-email',
-- [GARBAGE]                 headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || current_setting('app.settings.service_role_key', true) || '"}',
-- [GARBAGE]                 body := json_build_object(
                    'email', user_email,
...
-- [GARBAGE]     AFTER INSERT ON public.notifications
    FOR EACH ROW
...
-- [GARBAGE]         NEW.id,
-- [GARBAGE]         NEW.email,
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)),
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)),
        'User',
        'user',
        'active',
...
-- [GARBAGE]         WHERE tgname = 'on_auth_user_created'
    ) THEN
...
-- [GARBAGE] ON CONFLICT (key) DO UPDATE 
...
-- [GARBAGE]     new_users_30d int := 0;
    ...
    -- Storage stats
-- [GARBAGE]     total_storage_mb decimal := 0;
-- [GARBAGE]     media_storage_mb decimal := 0;
-- [GARBAGE]     docs_storage_mb decimal := 0;
    
    -- System flags
-- [GARBAGE]     is_maint_active boolean;
-- [GARBAGE]     is_ai_disabled boolean;
...
-- [GARBAGE]     WHERE up.subscription_status = 'active';
    ...
    SELECT 
-- [GARBAGE]         COALESCE(SUM(total_bytes) / (1024 * 1024), 0),
-- [GARBAGE]         COALESCE(SUM(media_bytes) / (1024 * 1024), 0),
-- [GARBAGE]         COALESCE(SUM(documents_bytes + utilities_bytes + maintenance_bytes) / (1024 * 1024), 0)
-- [GARBAGE]     INTO total_storage_mb, media_storage_mb, docs_storage_mb
-- [GARBAGE]     FROM public.user_storage_usage;
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     message TEXT NOT NULL,
-- [GARBAGE]     type TEXT NOT NULL CHECK (type IN ('info', 'warning', 'error', 'success')),
-- [GARBAGE]     is_active BOOLEAN DEFAULT true,
-- [GARBAGE]     expires_at TIMESTAMPTZ,
-- [GARBAGE]     target_link TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT now(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT now()
);
...
-- [HEADER] CREATE POLICY "Broadcasts are viewable by everyone"
-- [GARBAGE]     ON public.system_broadcasts FOR SELECT
...
-- [HEADER] CREATE POLICY "Super Admins have full access to broadcasts"
-- [GARBAGE]     ON public.system_broadcasts FOR ALL
-- [GARBAGE]     TO authenticated
    USING (EXISTS (
        SELECT 1 FROM user_profiles 
-- [GARBAGE]         WHERE id = auth.uid() AND is_super_admin = true
    ))
    WITH CHECK (EXISTS (
        SELECT 1 FROM user_profiles 
-- [GARBAGE]         WHERE id = auth.uid() AND is_super_admin = true
...
-- [GARBAGE]     BEFORE UPDATE ON public.system_broadcasts
    FOR EACH ROW
    EXECUTE PROCEDURE update_broadcast_updated_at();
-- Add marketing consent fields to user_profiles
ALTER TABLE public.user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS marketing_consent BOOLEAN DEFAULT FALSE,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS marketing_consent_at TIMESTAMPTZ;
...
-- [GARBAGE]         subscription_status, 
-- [GARBAGE]         plan_id,
-- [GARBAGE]         marketing_consent,
-- [GARBAGE]         marketing_consent_at
    )
    VALUES (
-- [GARBAGE]         NEW.id,
-- [GARBAGE]         NEW.email,
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)),
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'first_name', split_part(NEW.email, '@', 1)),
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'last_name', ''),
        'User',
        'active',
        'free',
-- [GARBAGE]         COALESCE((NEW.raw_user_meta_data->>'marketing_consent')::boolean, FALSE),
-- [GARBAGE]         CASE WHEN (NEW.raw_user_meta_data->>'marketing_consent')::boolean THEN NOW() ELSE NULL END
    )
-- [GARBAGE]     ON CONFLICT (id) DO UPDATE SET
-- [GARBAGE]         email = EXCLUDED.email,
-- [GARBAGE]         full_name = COALESCE(EXCLUDED.full_name, user_profiles.full_name),
-- [GARBAGE]         first_name = COALESCE(EXCLUDED.first_name, user_profiles.first_name),
-- [GARBAGE]         last_name = COALESCE(EXCLUDED.last_name, user_profiles.last_name),
-- [GARBAGE]         marketing_consent = COALESCE(EXCLUDED.marketing_consent, user_profiles.marketing_consent),
-- [GARBAGE]         marketing_consent_at = COALESCE(EXCLUDED.marketing_consent_at, user_profiles.marketing_consent_at),
-- [GARBAGE]         updated_at = NOW();
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        PERFORM
-- [GARBAGE]           net.http_post(
-- [GARBAGE]             url := project_url || '/functions/v1/send-welcome-email',
-- [GARBAGE]             headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || current_setting('app.settings.service_role_key', true) || '"}',
-- [GARBAGE]             body := json_build_object(
                'email', NEW.email,
...
-- [GARBAGE]                    WHERE table_name='property_documents' AND column_name='counter_read') THEN
...
DECLARE
-- [GARBAGE]     property_address text;
-- [GARBAGE]     user_lang text;
-- [GARBAGE]     notif_title text;
-- [GARBAGE]     notif_message text;
BEGIN
...
-- [GARBAGE]         notif_title := '׳ ׳•׳¡׳£ ׳×׳™׳¢׳•׳“ ׳×׳—׳–׳•׳§׳”';
-- [GARBAGE]         notif_message := format('׳ ׳•׳¡׳£ ׳×׳™׳¢׳•׳“ ׳×׳—׳–׳•׳§׳” ׳—׳“׳© ("%s") ׳¢׳‘׳•׳¨ ׳”׳ ׳›׳¡ %s.', COALESCE(NEW.title, '׳׳׳ ׳›׳•׳×׳¨׳×'), property_address);
-- [GARBAGE]     ELSE
-- [GARBAGE]         notif_title := 'Maintenance Record Added';
-- [GARBAGE]         notif_message := format('A new maintenance record ("%s") was added for %s.', COALESCE(NEW.title, 'Untitled'), property_address);
    END IF;
...
-- [GARBAGE]     AFTER INSERT ON public.property_documents
    FOR EACH ROW
...
-- [GARBAGE]     project_url text := 'https://tipnjnfbbnbskdlodrww.supabase.co';
-- [GARBAGE]     target_email text;
-- [GARBAGE]     asset_alerts_enabled boolean;
BEGIN
    -- Get user email and asset alerts preference
    SELECT 
-- [GARBAGE]         u.email, 
-- [GARBAGE]         COALESCE((up.notification_preferences->>'email_asset_alerts')::boolean, true)
-- [GARBAGE]     INTO target_email, asset_alerts_enabled
-- [GARBAGE]     FROM auth.users u
-- [GARBAGE]     LEFT JOIN public.user_profiles up ON up.id = u.id
-- [GARBAGE]     WHERE u.id = NEW.user_id;
...
-- [GARBAGE]     THEN
-- [HEADER]         IF target_email IS NOT NULL THEN
            PERFORM
-- [GARBAGE]               net.http_post(
-- [GARBAGE]                 url := project_url || '/functions/v1/send-notification-email',
-- [GARBAGE]                 headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || current_setting('app.settings.service_role_key', true) || '"}',
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 
-- [GARBAGE]         FROM information_schema.table_constraints 
-- [GARBAGE]         WHERE constraint_name = 'user_storage_usage_user_id_profiles_fkey'
    ) THEN
        ALTER TABLE user_storage_usage 
-- [GARBAGE]         ADD CONSTRAINT user_storage_usage_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE;
    END IF;
END $$;
...
-- [GARBAGE]         WHERE table_name = 'subscription_plans' AND column_name = 'price_yearly'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 
-- [GARBAGE]         FROM information_schema.table_constraints 
-- [GARBAGE]         WHERE constraint_name = 'ai_chat_usage_user_id_profiles_fkey'
    ) THEN
        ALTER TABLE ai_chat_usage 
-- [GARBAGE]         ADD CONSTRAINT ai_chat_usage_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES user_profiles(id) ON DELETE CASCADE;
    END IF;
END $$;
...
-- [GARBAGE]     AND updated_at > NOW() - INTERVAL '30 days';
    ...
    -- Build JSONB result
-- [GARBAGE]     result := jsonb_build_object(
        'totalUsers', total_users_count,
        'totalContracts', total_contracts_count,
        'totalRevenue', total_revenue_amount,
        'activeUsers', active_users_count
...
BEGIN
    ...
    PERFORM
-- [GARBAGE]       net.http_post(
-- [GARBAGE]         url := 'https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/admin-notifications',
-- [GARBAGE]         headers := jsonb_build_object(
          'Content-Type', 'application/json',
          'Authorization', 'Bearer ' || COALESCE(current_setting('app.settings.service_role_key', true), 'DUMMY_KEY')
        ),
-- [GARBAGE]         body := jsonb_build_object(
          'type', TG_ARGV[0],
          'data', CASE 
-- [GARBAGE]                     WHEN TG_ARGV[0] = 'new_user' THEN jsonb_build_object('email', NEW.email, 'full_name', NEW.full_name)
-- [GARBAGE]                     WHEN TG_ARGV[0] = 'first_payment' THEN jsonb_build_object('email', (SELECT email FROM user_profiles WHERE id = NEW.user_id), 'amount', NEW.paid_amount, 'plan', NEW.plan_id)
-- [GARBAGE]                     ELSE '{}'::jsonb
                  END
        )
      );
...
-- [GARBAGE]     AFTER INSERT ON user_profiles
    FOR EACH ROW
...
-- [GARBAGE]     AFTER UPDATE ON payments
    FOR EACH ROW
-- [GARBAGE]     WHEN (OLD.status != 'paid' AND NEW.status = 'paid')
...
-- [GARBAGE]         body := jsonb_build_object('type', 'daily_summary')
    );
    $$
);
...
-- [GARBAGE]     AFTER INSERT OR UPDATE ON public.user_profiles
    FOR EACH ROW
...
-- [GARBAGE]     AFTER UPDATE ON public.invoices
    FOR EACH ROW
...
SELECT cron.schedule(
    ...
    $$
    SELECT net.http_post(
-- [GARBAGE]         url := 'https://tipnjnfbbnbskdlodrww.supabase.co/functions/v1/send-daily-admin-summary',
-- [GARBAGE]         headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || current_setting('app.settings.service_role_key', true) || '"}',
-- [GARBAGE]         body := '{}'::jsonb
    );
    $$
...
-- [GARBAGE]         FROM public.user_profiles 
-- [GARBAGE]         WHERE id = auth.uid() 
-- [GARBAGE]         AND (role = 'admin' OR is_super_admin = true)
    );
END;
$$;
...
-- [HEADER] CREATE POLICY "Admins view all properties" 
-- [GARBAGE]     ON public.properties FOR SELECT 
    USING (public.is_admin());

-- CONTRACTS
DROP POLICY IF EXISTS "Admins view all contracts" ON public.contracts;
-- [HEADER] CREATE POLICY "Admins view all contracts" 
-- [GARBAGE]     ON public.contracts FOR SELECT 
    USING (public.is_admin());

-- TENANTS
DROP POLICY IF EXISTS "Admins view all tenants" ON public.tenants;
-- [HEADER] CREATE POLICY "Admins view all tenants" 
-- [GARBAGE]     ON public.tenants FOR SELECT 
    USING (public.is_admin());

-- PAYMENTS
DROP POLICY IF EXISTS "Admins view all payments" ON public.payments;
-- [HEADER] CREATE POLICY "Admins view all payments" 
-- [GARBAGE]     ON public.payments FOR SELECT 
    USING (public.is_admin());

-- PROPERTY DOCUMENTS
DROP POLICY IF EXISTS "Admins view all property documents" ON public.property_documents;
-- [HEADER] CREATE POLICY "Admins view all property documents" 
-- [GARBAGE]     ON public.property_documents FOR SELECT 
    USING (public.is_admin());

-- DOCUMENT FOLDERS
DROP POLICY IF EXISTS "Admins view all document folders" ON public.document_folders;
-- [HEADER] CREATE POLICY "Admins view all document folders" 
-- [GARBAGE]     ON public.document_folders FOR SELECT 
    USING (public.is_admin());

-- SHORT LINKS
DROP POLICY IF EXISTS "Admins view all short links" ON public.short_links;
-- [HEADER] CREATE POLICY "Admins view all short links" 
-- [GARBAGE]     ON public.short_links FOR SELECT 
    USING (public.is_admin());
...
DROP POLICY IF EXISTS "Admins full access to secure_documents" ON storage.objects;
-- [HEADER] CREATE POLICY "Admins full access to secure_documents"
-- [GARBAGE]     ON storage.objects FOR ALL
    USING (
-- [GARBAGE]         bucket_id = 'secure_documents' 
-- [GARBAGE]         AND public.is_admin()
    )
    WITH CHECK (
-- [GARBAGE]         bucket_id = 'secure_documents' 
-- [GARBAGE]         AND public.is_admin()
...
BEGIN
    ...
    UPDATE public.user_profiles
    SET role = 'admin',
-- [GARBAGE]         is_super_admin = true
-- [GARBAGE]     WHERE email = 'rubi@rentmate.co.il';
...
-- [GARBAGE]     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     title TEXT NOT NULL,
-- [GARBAGE]     description TEXT NOT NULL,
-- [GARBAGE]     category TEXT NOT NULL CHECK (category IN ('technical', 'billing', 'feature_request', 'bug', 'other')),
-- [GARBAGE]     priority TEXT NOT NULL DEFAULT 'medium' CHECK (priority IN ('low', 'medium', 'high', 'urgent')),
-- [GARBAGE]     status TEXT NOT NULL DEFAULT 'open' CHECK (status IN ('open', 'in_progress', 'waiting_user', 'resolved', 'closed')),
-- [GARBAGE]     created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     ticket_id UUID NOT NULL REFERENCES support_tickets(id) ON DELETE CASCADE,
-- [GARBAGE]     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    comment TEXT NOT NULL,
-- [GARBAGE]     is_admin BOOLEAN NOT NULL DEFAULT FALSE,
-- [GARBAGE]     created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- [HEADER] CREATE POLICY "Users can view own tickets"
-- [GARBAGE]     ON support_tickets FOR SELECT
    USING (auth.uid() = user_id);

-- Users can create tickets
DROP POLICY IF EXISTS "Users can create tickets" ON support_tickets;
-- [HEADER] CREATE POLICY "Users can create tickets"
-- [GARBAGE]     ON support_tickets FOR INSERT
    WITH CHECK (auth.uid() = user_id);

-- Users can update their own open tickets
DROP POLICY IF EXISTS "Users can update own open tickets" ON support_tickets;
-- [HEADER] CREATE POLICY "Users can update own open tickets"
-- [GARBAGE]     ON support_tickets FOR UPDATE
    USING (auth.uid() = user_id AND status = 'open');

-- Admins can view all tickets
DROP POLICY IF EXISTS "Admins can view all tickets" ON support_tickets;
-- [HEADER] CREATE POLICY "Admins can view all tickets"
-- [GARBAGE]     ON support_tickets FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
...
-- [HEADER] CREATE POLICY "Admins can update all tickets"
-- [GARBAGE]     ON support_tickets FOR UPDATE
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
...
-- [HEADER] CREATE POLICY "Users can view own ticket comments"
-- [GARBAGE]     ON ticket_comments FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM support_tickets
-- [GARBAGE]             WHERE id = ticket_comments.ticket_id AND user_id = auth.uid()
        )
    );

-- Users can add comments to their tickets
DROP POLICY IF EXISTS "Users can comment on own tickets" ON ticket_comments;
-- [HEADER] CREATE POLICY "Users can comment on own tickets"
-- [GARBAGE]     ON ticket_comments FOR INSERT
    WITH CHECK (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM support_tickets
-- [GARBAGE]             WHERE id = ticket_comments.ticket_id AND user_id = auth.uid()
...
-- [HEADER] CREATE POLICY "Admins can view all comments"
-- [GARBAGE]     ON ticket_comments FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
...
-- [HEADER] CREATE POLICY "Admins can comment on all tickets"
-- [GARBAGE]     ON ticket_comments FOR INSERT
    WITH CHECK (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
...
-- [GARBAGE]     BEFORE UPDATE ON support_tickets
    FOR EACH ROW
...
-- [GARBAGE]     AFTER INSERT ON support_tickets
    FOR EACH ROW
...
DECLARE
-- [GARBAGE]     r RECORD;
-- [GARBAGE]     extension_days_default int := 60;
-- [GARBAGE]     pref jsonb;
BEGIN
    -------------------------------------------------------
...
-- [GARBAGE]         FROM public.contracts c
-- [GARBAGE]         JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]         JOIN public.user_profiles up ON up.id = c.user_id
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.end_date = CURRENT_DATE + (COALESCE((up.notification_preferences->>'contract_expiry_days')::int, 30) || ' days')::INTERVAL
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
-- [GARBAGE]             AND metadata->>'contract_id' = r.id::text 
-- [GARBAGE]             AND metadata->>'event' = 'ending_soon'
-- [GARBAGE]             AND created_at > (CURRENT_DATE - INTERVAL '1 day')
        ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [GARBAGE]                 r.user_id,
                'warning',
                'Contract Ending Soon',
-- [GARBAGE]                 format('Contract for %s, %s ends in %s days.', r.city, r.address, COALESCE((r.notification_preferences->>'contract_expiry_days')::int, 30)),
-- [GARBAGE]                 json_build_object('contract_id', r.id, 'event', 'ending_soon')::jsonb
            );
        END IF;
    END LOOP;
...
-- [GARBAGE]         WHERE c.status = 'active'
-- [GARBAGE]         AND c.extension_option = TRUE
-- [GARBAGE]         AND c.end_date = CURRENT_DATE + (COALESCE((up.notification_preferences->>'extension_option_end_days')::int, 60) || ' days')::INTERVAL
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
...
-- [GARBAGE]                 r.user_id,
                'action',
                'Extension Deadline Approaching',
-- [GARBAGE]                 format('Extension option for %s, %s ends in %s days.', r.city, r.address, COALESCE((r.notification_preferences->>'extension_option_end_days')::int, 60)),
-- [GARBAGE]                 json_build_object('contract_id', r.id, 'event', 'extension_deadline')::jsonb
            );
        END IF;
    END LOOP;
...
-- [GARBAGE]         JOIN public.contracts c ON c.id = py.contract_id
-- [GARBAGE]         JOIN public.properties p ON p.id = c.property_id
-- [GARBAGE]         JOIN public.user_profiles up ON up.id = py.user_id
-- [GARBAGE]         WHERE py.status = 'pending'
-- [GARBAGE]         AND py.date = CURRENT_DATE + (COALESCE((up.notification_preferences->>'rent_due_days')::int, 0) || ' days')::INTERVAL
-- [GARBAGE]         AND (up.notification_preferences->>'rent_due_days')::int > 0
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
-- [GARBAGE]             AND metadata->>'payment_id' = r.id::text 
-- [GARBAGE]             AND metadata->>'event' = 'payment_warning'
-- [GARBAGE]             AND created_at > (CURRENT_DATE - INTERVAL '1 day')
        ) THEN
            INSERT INTO public.notifications (user_id, type, title, message, metadata)
            VALUES (
-- [GARBAGE]                 r.user_id,
                'info',
                'Payment Reminder',
-- [GARBAGE]                 format('Payment of ג‚×%s for %s, %s is due in %s days.', r.amount, r.city, r.address, (r.notification_preferences->>'rent_due_days')::int),
-- [GARBAGE]                 json_build_object('payment_id', r.id, 'event', 'payment_warning')::jsonb
            );
        END IF;
    END LOOP;
...
-- [GARBAGE]         WHERE py.status = 'pending'
-- [GARBAGE]         AND py.date = CURRENT_DATE
-- [GARBAGE]         AND COALESCE((up.notification_preferences->>'rent_due_today')::boolean, true) = true
    LOOP
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
-- [GARBAGE]             WHERE user_id = r.user_id 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [GARBAGE]         WHERE table_name = 'user_profiles' AND column_name = 'subscription_tier'
    ) THEN
...
RETURNS TRIGGER AS $$
BEGIN
-- [GARBAGE]     NEW.subscription_tier := NEW.plan_id;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- [GARBAGE]     BEFORE INSERT OR UPDATE OF plan_id ON user_profiles
    FOR EACH ROW
...
-- [GARBAGE]         ADD CONSTRAINT user_storage_usage_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
END $$;
...
-- [GARBAGE]     full_name TEXT,
-- [GARBAGE]     role TEXT,
-- [GARBAGE]     subscription_status TEXT,
-- [GARBAGE]     plan_id TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ,
-- [GARBAGE]     properties_count BIGINT,
-- [GARBAGE]     tenants_count BIGINT,
...
-- [GARBAGE]         up.email,
-- [GARBAGE]         up.full_name,
-- [GARBAGE]         up.role::TEXT,
-- [GARBAGE]         up.subscription_status::TEXT,
-- [GARBAGE]         up.plan_id,
-- [GARBAGE]         up.created_at,
-- [GARBAGE]         COALESCE(p.count, 0) as properties_count,
-- [GARBAGE]         COALESCE(t.count, 0) as tenants_count,
-- [GARBAGE]         COALESCE(c.count, 0) as contracts_count
-- [GARBAGE]     FROM user_profiles up
-- [GARBAGE]     LEFT JOIN (SELECT user_id, count(*) as count FROM properties GROUP BY user_id) p ON up.id = p.user_id
-- [GARBAGE]     LEFT JOIN (SELECT user_id, count(*) as count FROM tenants GROUP BY user_id) t ON up.id = t.user_id
-- [GARBAGE]     LEFT JOIN (SELECT user_id, count(*) as count FROM contracts GROUP BY user_id) c ON up.id = c.user_id
-- [GARBAGE]     WHERE up.deleted_at IS NULL
-- [GARBAGE]     ORDER BY up.created_at DESC;
END;
$$;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'properties') THEN
        ...
        ALTER TABLE public.properties
-- [GARBAGE]         ADD CONSTRAINT properties_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenants') THEN
        ...
        ALTER TABLE public.tenants
-- [GARBAGE]         ADD CONSTRAINT tenants_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'contracts') THEN
        ...
        ALTER TABLE public.contracts
-- [GARBAGE]         ADD CONSTRAINT contracts_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
...
-- [GARBAGE]         ADD CONSTRAINT admin_notifications_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
...
-- [GARBAGE]         ADD CONSTRAINT support_tickets_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
...
-- [GARBAGE]         ADD CONSTRAINT ticket_comments_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
...
-- [GARBAGE]         ADD CONSTRAINT property_documents_user_id_profiles_fkey 
-- [GARBAGE]         FOREIGN KEY (user_id) REFERENCES public.user_profiles(id) ON DELETE CASCADE;
    END IF;
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
-- [GARBAGE]     model TEXT NOT NULL,
-- [GARBAGE]     feature TEXT NOT NULL, -- 'chat' or 'contract-extraction'
-- [GARBAGE]     input_tokens INTEGER DEFAULT 0,
-- [GARBAGE]     output_tokens INTEGER DEFAULT 0,
-- [GARBAGE]     estimated_cost_usd NUMERIC(10, 6) DEFAULT 0,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [HEADER] CREATE POLICY "Admins can view all AI usage logs"
-- [GARBAGE]     ON public.ai_usage_logs FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
-- [GARBAGE]             WHERE id = auth.uid() AND role = 'admin'
...
-- [GARBAGE]     p_user_id UUID,
-- [GARBAGE]     p_model TEXT,
-- [GARBAGE]     p_feature TEXT,
-- [GARBAGE]     p_input_tokens INTEGER,
-- [GARBAGE]     p_output_tokens INTEGER
)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
-- [GARBAGE]     v_cost_input NUMERIC;
-- [GARBAGE]     v_cost_output NUMERIC;
-- [GARBAGE]     v_total_cost NUMERIC;
BEGIN
    -- Determine costs based on model
    -- Prices per 1M tokens
-- [HEADER]     IF p_model LIKE 'gpt-4o-mini%' THEN
-- [GARBAGE]         v_cost_input := 0.15;
-- [GARBAGE]         v_cost_output := 0.60;
-- [GARBAGE]     ELSIF p_model LIKE 'gpt-4o%' THEN
-- [GARBAGE]         v_cost_input := 2.50;
-- [GARBAGE]         v_cost_output := 10.00;
-- [GARBAGE]     ELSE
...
-- [GARBAGE]     v_total_cost := (p_input_tokens::NUMERIC / 1000000 * v_cost_input) + (p_output_tokens::NUMERIC / 1000000 * v_cost_output);
...
-- [GARBAGE]         output_tokens,
-- [GARBAGE]         estimated_cost_usd
    ) VALUES (
-- [GARBAGE]         p_user_id,
-- [GARBAGE]         p_model,
-- [GARBAGE]         p_feature,
-- [GARBAGE]         p_input_tokens,
-- [GARBAGE]         p_output_tokens,
-- [GARBAGE]         v_total_cost
    );
...
-- [GARBAGE]     ON CONFLICT (user_id) DO UPDATE
    SET message_count = public.ai_chat_usage.message_count + 1,
-- [GARBAGE]         tokens_used = public.ai_chat_usage.tokens_used + (p_input_tokens + p_output_tokens),
-- [GARBAGE]         updated_at = NOW();
END;
$$;
...
-- [GARBAGE]     total_revenue_amount NUMERIC;
-- [GARBAGE]     active_users_count INTEGER;
-- [GARBAGE]     total_ai_cost_usd NUMERIC;
BEGIN
    -- Check if the current user is an admin
    IF NOT EXISTS (
        SELECT 1 FROM user_profiles
...
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     title TEXT,
-- [GARBAGE]     messages JSONB DEFAULT '[]'::jsonb,
-- [GARBAGE]     total_cost_usd NUMERIC(10, 6) DEFAULT 0,
-- [GARBAGE]     metadata JSONB DEFAULT '{}'::jsonb,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [HEADER] CREATE POLICY "Users can view own AI conversations"
-- [GARBAGE]     ON public.ai_conversations FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete own AI conversations" ON public.ai_conversations;
-- [HEADER] CREATE POLICY "Users can delete own AI conversations"
-- [GARBAGE]     ON public.ai_conversations FOR DELETE
    USING (auth.uid() = user_id);

-- Admins can view everything
DROP POLICY IF EXISTS "Admins can view all AI conversations" ON public.ai_conversations;
-- [HEADER] CREATE POLICY "Admins can view all AI conversations"
-- [GARBAGE]     ON public.ai_conversations FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
...
-- [GARBAGE]     p_conversation_id UUID,
-- [GARBAGE]     p_new_messages JSONB,
-- [GARBAGE]     p_cost_usd NUMERIC DEFAULT 0,
-- [GARBAGE]     p_user_id UUID DEFAULT NULL
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
-- [GARBAGE]     v_conv_id UUID;
-- [GARBAGE]     v_final_user_id UUID;
BEGIN
    -- Determine user ID: prefer explicit, fallback to auth.uid()
-- [GARBAGE]     v_final_user_id := COALESCE(p_user_id, auth.uid());

    -- Update existing or insert new
    INSERT INTO public.ai_conversations (id, user_id, messages, total_cost_usd, updated_at)
    VALUES (
-- [GARBAGE]         p_conversation_id,
-- [GARBAGE]         v_final_user_id,
-- [GARBAGE]         p_new_messages,
-- [GARBAGE]         p_cost_usd,
-- [GARBAGE]         NOW()
    )
-- [GARBAGE]     ON CONFLICT (id) DO UPDATE
    SET messages = public.ai_conversations.messages || EXCLUDED.messages,
-- [GARBAGE]         total_cost_usd = public.ai_conversations.total_cost_usd + EXCLUDED.total_cost_usd,
-- [GARBAGE]         updated_at = NOW()
-- [GARBAGE]     RETURNING id INTO v_conv_id;
...
ALTER TABLE property_documents 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS invoice_number TEXT;

-- Create an index for faster duplicate checks
-- [HEADER] CREATE INDEX IF NOT EXISTS idx_property_documents_duplicate_check 
-- [GARBAGE] ON property_documents(vendor_name, document_date, invoice_number);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     admin_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
-- [GARBAGE]     status TEXT DEFAULT 'active' CHECK (status IN ('active', 'closed')),
-- [GARBAGE]     last_message_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     conversation_id UUID REFERENCES public.human_conversations(id) ON DELETE CASCADE,
-- [GARBAGE]     sender_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
-- [GARBAGE]     sender_role TEXT CHECK (sender_role IN ('user', 'admin')),
-- [GARBAGE]     content TEXT NOT NULL,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [HEADER] AS PERMISSIVE FOR ALL TO authenticated
...
-- [GARBAGE]         WHERE id = public.human_messages.conversation_id AND user_id = auth.uid()
    )
);
...
-- [GARBAGE]     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     admin_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
-- [GARBAGE]     status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'closed')),
-- [GARBAGE]     last_message_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.human_messages (
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     conversation_id UUID NOT NULL REFERENCES public.human_conversations(id) ON DELETE CASCADE,
-- [GARBAGE]     sender_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     role TEXT NOT NULL CHECK (role IN ('user', 'admin')),
-- [GARBAGE]     content TEXT NOT NULL,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [HEADER] CREATE POLICY "Admins can view all conversations"
-- [GARBAGE]     ON public.human_conversations
    FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM public.user_profiles
...
-- [HEADER] CREATE POLICY "Admins can insert conversations"
-- [GARBAGE]     ON public.human_conversations
    FOR INSERT
    WITH CHECK (
-- [GARBAGE]         EXISTS (
...
-- [HEADER] CREATE POLICY "Admins can update conversations"
-- [GARBAGE]     ON public.human_conversations
    FOR UPDATE
    USING (
-- [GARBAGE]         EXISTS (
...
-- [HEADER] CREATE POLICY "Users can view their own conversations"
-- [GARBAGE]     ON public.human_conversations
    FOR SELECT
    USING (auth.uid() = user_id);
...
-- [HEADER] CREATE POLICY "Admins can view all messages"
-- [GARBAGE]     ON public.human_messages
    FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM public.user_profiles
...
-- [HEADER] CREATE POLICY "Admins can insert messages"
-- [GARBAGE]     ON public.human_messages
    FOR INSERT
    WITH CHECK (
-- [GARBAGE]         EXISTS (
...
-- [HEADER] CREATE POLICY "Users can view messages in their conversations"
-- [GARBAGE]     ON public.human_messages
    FOR SELECT
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM public.human_conversations
-- [GARBAGE]             WHERE id = human_messages.conversation_id AND user_id = auth.uid()
        )
    );

DROP POLICY IF EXISTS "Users can insert messages in their active conversations" ON public.human_messages;
-- [HEADER] CREATE POLICY "Users can insert messages in their active conversations"
-- [GARBAGE]     ON public.human_messages
    FOR INSERT
    WITH CHECK (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM public.human_conversations
-- [GARBAGE]             WHERE id = human_messages.conversation_id 
-- [GARBAGE]             AND user_id = auth.uid()
-- [GARBAGE]             AND status = 'active'
        )
    );
...
-- [GARBAGE]         ADD COLUMN IF NOT EXISTS ai_data_consent BOOLEAN DEFAULT false;
    END IF;
END $$;
...
ALTER TABLE public.contracts 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS notice_period_days INTEGER,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS option_notice_days INTEGER;
...
-- [GARBAGE] ON CONFLICT (key) DO UPDATE SET 
-- [GARBAGE]   description = EXCLUDED.description;
...
-- [GARBAGE] FROM public.tenants t
-- [GARBAGE] WHERE c.tenant_id = t.id
-- [GARBAGE] AND (c.tenants IS NULL OR c.tenants = '[]'::jsonb);
...
-- [GARBAGE]     plan_id TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ,
-- [GARBAGE]     last_login TIMESTAMPTZ,
    
    -- Stats
-- [GARBAGE]     properties_count BIGINT,
-- [GARBAGE]     tenants_count BIGINT,
-- [GARBAGE]     contracts_count BIGINT,
-- [GARBAGE]     ai_sessions_count BIGINT,
-- [GARBAGE]     open_tickets_count BIGINT,
-- [GARBAGE]     storage_usage_mb NUMERIC
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- [GARBAGE]         COALESCE(c.count, 0) as contracts_count,
        ...
        -- AI Usage
-- [GARBAGE]         COALESCE(ai.count, 0) as ai_sessions_count,
        
        -- Support Status
-- [GARBAGE]         COALESCE(st.count, 0) as open_tickets_count,
        
        -- Storage Usage (Bytes to MB)
-- [GARBAGE]         ROUND(COALESCE(usu.total_bytes, 0) / (1024.0 * 1024.0), 2) as storage_usage_mb
...
-- [GARBAGE]     LEFT JOIN (SELECT user_id, count(*) as count FROM contracts GROUP BY user_id) c ON up.id = c.user_id
    -- AI Counts
-- [GARBAGE]     LEFT JOIN (SELECT user_id, count(*) as count FROM ai_conversations GROUP BY user_id) ai ON up.id = ai.user_id
    -- Open Support Tickets
-- [GARBAGE]     LEFT JOIN (SELECT user_id, count(*) as count FROM support_tickets WHERE status != 'resolved' GROUP BY user_id) st ON up.id = st.user_id
    -- Storage Usage
-- [GARBAGE]     LEFT JOIN user_storage_usage usu ON up.id = usu.user_id
...
-- [GARBAGE]     active_users_count INTEGER;
-- [GARBAGE]     total_ai_cost_usd NUMERIC;
-- [GARBAGE]     total_automated_actions INTEGER;
BEGIN
    -- Check if the current user is an admin
    IF NOT EXISTS (
        SELECT 1 FROM user_profiles
...
AS $$
DECLARE
-- [GARBAGE]     default_plan_id TEXT := 'free';
BEGIN
...
-- [GARBAGE]         NEW.email,
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)),
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'first_name', split_part(NEW.raw_user_meta_data->>'full_name', ' ', 1), 'User'),
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'last_name', 'User'),
        'user', -- Default role
        'active', -- Default status
-- [GARBAGE]         default_plan_id
    )
-- [GARBAGE]     ON CONFLICT (id) DO UPDATE SET
-- [GARBAGE]         email = EXCLUDED.email,
-- [GARBAGE]         full_name = COALESCE(EXCLUDED.full_name, user_profiles.full_name),
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     ticket_id UUID REFERENCES public.support_tickets(id) ON DELETE CASCADE,
-- [GARBAGE]     sentiment_score FLOAT, -- -1.0 to 1.0
-- [GARBAGE]     urgency_level TEXT CHECK (urgency_level IN ('low', 'medium', 'high', 'critical')),
-- [GARBAGE]     category TEXT,
-- [GARBAGE]     confidence_score FLOAT,
-- [GARBAGE]     ai_summary TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     name TEXT NOT NULL,
-- [GARBAGE]     trigger_type TEXT NOT NULL, -- 'lease_expiry', 'rent_overdue', 'ticket_created'
-- [GARBAGE]     condition JSONB, -- e.g. {"days_before": 60}
-- [GARBAGE]     action_type TEXT NOT NULL, -- 'email', 'notification', 'auto_reply'
-- [GARBAGE]     action_config JSONB, -- template_id, etc.
-- [GARBAGE]     is_enabled BOOLEAN DEFAULT true,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     rule_id UUID REFERENCES public.automation_rules(id),
-- [GARBAGE]     user_id UUID REFERENCES auth.users(id), -- Target user
-- [GARBAGE]     entity_id UUID, -- contract_id, ticket_id, etc.
-- [GARBAGE]     action_taken TEXT,
-- [GARBAGE]     status TEXT, -- 'success', 'failed'
-- [GARBAGE]     details JSONB,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [GARBAGE]     user_id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     lease_expiry_days INTEGER DEFAULT 100,
-- [GARBAGE]     extension_notice_days INTEGER DEFAULT 60,
-- [GARBAGE]     rent_overdue_days INTEGER DEFAULT 5,
-- [GARBAGE]     auto_reply_enabled BOOLEAN DEFAULT false,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
ALTER TABLE public.support_tickets 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS auto_reply_draft TEXT;
...
-- [HEADER] ALTER TABLE IF EXISTS public.user_automation_settings 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS email_notifications_enabled BOOLEAN DEFAULT true,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS sms_notifications_enabled BOOLEAN DEFAULT false,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS whatsapp_notifications_enabled BOOLEAN DEFAULT false,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS push_notifications_enabled BOOLEAN DEFAULT true;
...
RETURNS TRIGGER AS $$
DECLARE
-- [GARBAGE]   payload JSONB;
BEGIN
-- [GARBAGE]   payload := jsonb_build_object(
    'type', TG_OP,
    'table', TG_TABLE_NAME,
    'record', row_to_json(NEW),
    'old_record', CASE WHEN TG_OP = 'UPDATE' THEN row_to_json(OLD) ELSE NULL END
  ...
  PERFORM
-- [GARBAGE]     net.http_post(
-- [GARBAGE]       url := 'https://' || (SELECT value FROM system_settings WHERE key = 'supabase_project_ref') || '.supabase.co/functions/v1/on-event-trigger',
-- [GARBAGE]       headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer ' || (SELECT value FROM system_settings WHERE key = 'supabase_service_role_key')
      ),
-- [GARBAGE]       body := payload
    );
...
-- [GARBAGE] AFTER INSERT ON public.support_tickets
...
-- [GARBAGE] AFTER UPDATE ON public.payments
FOR EACH ROW EXECUTE FUNCTION public.handle_automated_engagement_webhook();
...
-- [GARBAGE] AFTER INSERT ON public.contracts
FOR EACH ROW EXECUTE FUNCTION public.handle_automated_engagement_webhook();
-- ============================================
...
-- [GARBAGE]     total_revenue_amount NUMERIC;
-- [GARBAGE]     active_users_count INTEGER;
-- [GARBAGE]     total_ai_cost NUMERIC;
-- [GARBAGE]     automated_actions_count INTEGER;
-- [GARBAGE]     stagnant_tickets_count INTEGER;
-- [GARBAGE]     avg_sentiment_score NUMERIC;
-- [GARBAGE]     last_automation_run TIMESTAMPTZ;
BEGIN
    -- Check if the current user is an admin
    IF NOT EXISTS (
        SELECT 1 FROM user_profiles
-- [GARBAGE]         WHERE id = auth.uid()
-- [GARBAGE]         AND role IN ('admin', 'super_admin')
    ) THEN
        RAISE EXCEPTION 'Access denied: Admin role required';
    END IF;
...
-- [GARBAGE]     id BIGSERIAL PRIMARY KEY,
-- [GARBAGE]     bucket_id TEXT NOT NULL,
-- [GARBAGE]     storage_path TEXT NOT NULL,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     processed_at TIMESTAMPTZ,
-- [GARBAGE]     error_log TEXT
);
...
-- [GARBAGE] AFTER DELETE ON public.property_documents
FOR EACH ROW
...
-- [GARBAGE]     p_user_id UUID,
-- [GARBAGE]     p_action TEXT,
-- [GARBAGE]     p_contract_id UUID DEFAULT NULL,
-- [GARBAGE]     p_details JSONB DEFAULT '{}'
)
RETURNS VOID AS $$
BEGIN
    INSERT INTO public.audit_logs (
-- [GARBAGE]         user_id,
-- [GARBAGE]         target_user_id,
-- [GARBAGE]         action,
-- [GARBAGE]         details,
-- [GARBAGE]         created_at
    )
    VALUES (
-- [GARBAGE]         p_user_id,
-- [GARBAGE]         p_user_id, -- In this context, target is usually the same user
-- [GARBAGE]         p_action,
-- [GARBAGE]         p_details || jsonb_build_object(
...
-- [GARBAGE]   date TEXT NOT NULL, -- Format: 'YYYY-MM'
-- [GARBAGE]   value DECIMAL(10, 4) NOT NULL,
-- [GARBAGE]   source TEXT DEFAULT 'cbs' CHECK (source IN ('cbs', 'exchange-api', 'manual', 'boi')),
-- [GARBAGE]   created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]   updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]   UNIQUE(index_type, date)
);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     index_type TEXT NOT NULL CHECK (index_type IN ('cpi', 'housing', 'construction', 'usd', 'eur')),
-- [GARBAGE]     base_period_start DATE NOT NULL,
-- [GARBAGE]     base_value NUMERIC NOT NULL DEFAULT 100.0,
-- [GARBAGE]     previous_base_period_start DATE,
-- [GARBAGE]     chain_factor NUMERIC,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]     UNIQUE(index_type, base_period_start)
);
...
-- [GARBAGE] ON CONFLICT (index_type, base_period_start) DO UPDATE 
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [GARBAGE]         WHERE table_name = 'user_profiles' AND column_name = 'phone'
    ) THEN
        ALTER TABLE public.user_profiles ADD COLUMN IF NOT EXISTS phone TEXT;
    END IF;
END $$;
...
BEGIN
    ...
    SET phone = au.phone
-- [GARBAGE]     FROM auth.users au
-- [GARBAGE]     WHERE up.id = au.id
-- [GARBAGE]     AND up.phone IS NULL
-- [GARBAGE]     AND au.phone IS NOT NULL;
EXCEPTION WHEN OTHERS THEN
...
-- [GARBAGE]         email = EXCLUDED.email,
-- [GARBAGE]         full_name = COALESCE(EXCLUDED.full_name, user_profiles.full_name),
-- [GARBAGE]         phone = COALESCE(EXCLUDED.phone, user_profiles.phone),
-- [GARBAGE]         updated_at = NOW();
...
-- [GARBAGE]     open_tickets_count BIGINT,
-- [GARBAGE]     storage_usage_mb NUMERIC,
-- [GARBAGE]     is_super_admin BOOLEAN
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- [GARBAGE]         up.phone,
-- [GARBAGE]         up.role::TEXT,
-- [GARBAGE]         COALESCE(up.subscription_status::TEXT, 'active'),
-- [GARBAGE]         up.plan_id,
-- [GARBAGE]         up.created_at,
-- [GARBAGE]         up.last_login,
        
        -- Asset Stats
-- [GARBAGE]         COALESCE(p.count, 0)::BIGINT as properties_count,
-- [GARBAGE]         COALESCE(t.count, 0)::BIGINT as tenants_count,
-- [GARBAGE]         COALESCE(c.count, 0)::BIGINT as contracts_count,
        
        -- Usage Stats
-- [GARBAGE]         COALESCE(ai.count, 0)::BIGINT as ai_sessions_count,
        
        -- Support Stats
-- [GARBAGE]         COALESCE(st.count, 0)::BIGINT as open_tickets_count,
        
        -- Storage Usage (Bytes to MB)
-- [GARBAGE]         ROUND(COALESCE(usu.total_bytes, 0) / (1024.0 * 1024.0), 2)::NUMERIC as storage_usage_mb,
        
        -- Permissions
-- [GARBAGE]         COALESCE(up.is_super_admin, false) as is_super_admin
...
-- [GARBAGE]     LEFT JOIN (SELECT user_id, count(*) as count FROM support_tickets WHERE status != 'resolved' GROUP BY user_id) st ON up.id = st.user_id
    -- Storage Usage
-- [GARBAGE]     LEFT JOIN (SELECT user_id, total_bytes FROM user_storage_usage) usu ON up.id = usu.user_id
...
-- [GARBAGE] ON CONFLICT (index_type, date) 
...
ALTER TABLE user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS google_refresh_token TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS google_drive_folder_id TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS google_drive_enabled BOOLEAN DEFAULT FALSE;
...
-- [GARBAGE]     ELSIF NEW.status = 'archived' THEN
...
RETURNS TRIGGER AS $$
DECLARE
-- [GARBAGE]     target_property_id uuid;
BEGIN
    -- Determine which property we are talking about
    -- TG_OP is the operation (INSERT, UPDATE, DELETE)
    IF (TG_OP = 'DELETE') THEN
-- [GARBAGE]         target_property_id := OLD.property_id;
-- [GARBAGE]     ELSE
-- [GARBAGE]         target_property_id := NEW.property_id;
    END IF;
    ...
    IF EXISTS (
        SELECT 1 FROM public.contracts 
-- [GARBAGE]         WHERE property_id = target_property_id 
-- [GARBAGE]         AND status = 'active'
    ) THEN
        UPDATE public.properties
        SET status = 'Occupied'
-- [GARBAGE]         WHERE id = target_property_id;
-- [GARBAGE]     ELSE
...
DROP TRIGGER IF EXISTS "trigger_update_property_status" ON public.contracts;
CREATE TRIGGER trigger_update_property_status
-- [GARBAGE] AFTER INSERT OR UPDATE OR DELETE ON public.contracts
FOR EACH ROW
...
-- [GARBAGE]     prop RECORD;
BEGIN
-- [HEADER]     FOR prop IN SELECT id FROM public.properties LOOP
        -- If an active contract exists, set to Occupied
        IF EXISTS (
            SELECT 1 FROM public.contracts 
-- [GARBAGE]             WHERE property_id = prop.id 
-- [GARBAGE]             AND status = 'active'
        ) THEN
            UPDATE public.properties
            SET status = 'Occupied'
-- [GARBAGE]             WHERE id = prop.id;
-- [GARBAGE]         ELSE
            -- Otherwise Vacant
            UPDATE public.properties
            SET status = 'Vacant'
...
BEGIN
    ...
    UPDATE public.contracts
    SET status = 'archived'
-- [GARBAGE]     WHERE status = 'active'
-- [GARBAGE]     AND end_date < CURRENT_DATE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     index_type TEXT NOT NULL CHECK (index_type IN ('cpi', 'housing', 'construction')),
-- [GARBAGE]     from_base TEXT NOT NULL,
-- [GARBAGE]     to_base TEXT NOT NULL,
-- [GARBAGE]     factor DECIMAL(10, 6) NOT NULL,
-- [GARBAGE]     effective_date DATE NOT NULL,
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
-- [GARBAGE]     UNIQUE(index_type, from_base, to_base)
);
...
-- [HEADER] CREATE POLICY "Allow authenticated users to read chaining factors"
-- [GARBAGE]     ON chaining_factors
    FOR SELECT
-- [GARBAGE]     TO authenticated
    USING (true);

-- Allow service role to manage chaining factors
-- [HEADER] CREATE POLICY "Allow service role to manage chaining factors"
-- [GARBAGE]     ON chaining_factors
    FOR ALL
-- [GARBAGE]     TO service_role
    USING (true);
...
-- [GARBAGE] ON CONFLICT (index_type, from_base, to_base) DO NOTHING;

-- Add index for faster lookups
-- [HEADER] CREATE INDEX IF NOT EXISTS idx_chaining_factors_lookup 
-- [GARBAGE]     ON chaining_factors(index_type, from_base, to_base);
...
BEGIN
    ...
    IF v_value IS NULL OR v_value = '' THEN
        BEGIN
-- [GARBAGE]             v_value := current_setting('app.settings.' || p_key, true);
        EXCEPTION WHEN OTHERS THEN
-- [GARBAGE]             v_value := NULL;
        END;
    END IF;
...
RETURNS TRIGGER AS $$
DECLARE
-- [GARBAGE]   v_project_ref TEXT;
-- [GARBAGE]   v_service_key TEXT;
-- [GARBAGE]   v_payload JSONB;
BEGIN
  -- Get Config
-- [GARBAGE]   v_project_ref := public.get_supabase_config('supabase_project_ref');
-- [GARBAGE]   v_service_key := public.get_supabase_config('supabase_service_role_key');

  -- If no config, log warning and exit (preventing 22P02 crashes)
-- [HEADER]   IF v_project_ref IS NULL OR v_service_key IS NULL THEN
    RAISE WARNING 'Skipping webhook: Supabase config missing (project_ref or service_key)';
    RETURN NEW;
  END IF;

  -- Build Payload safely using to_jsonb
-- [GARBAGE]   v_payload := jsonb_build_object(
    'type', TG_OP,
    'table', TG_TABLE_NAME,
    'record', to_jsonb(NEW),
  ...
  PERFORM
-- [GARBAGE]     net.http_post(
-- [GARBAGE]       url := 'https://' || v_project_ref || '.supabase.co/functions/v1/on-event-trigger',
-- [GARBAGE]       headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer ' || v_service_key
      ),
-- [GARBAGE]       body := v_payload
    );
...
-- [GARBAGE]     v_project_ref text;
-- [GARBAGE]     v_service_key text;
-- [GARBAGE]     v_target_email text;
-- [GARBAGE]     v_asset_alerts_enabled boolean;
BEGIN
    -- Get project config
-- [GARBAGE]     v_project_ref := public.get_supabase_config('supabase_project_ref');
-- [GARBAGE]     v_service_key := public.get_supabase_config('supabase_service_role_key');
...
-- [GARBAGE]         u.email, 
-- [GARBAGE]         COALESCE((up.notification_preferences->>'email_asset_alerts')::boolean, true)
-- [GARBAGE]     INTO v_target_email, v_asset_alerts_enabled
-- [GARBAGE]     FROM auth.users u
-- [GARBAGE]     LEFT JOIN public.user_profiles up ON up.id = u.id
-- [GARBAGE]     WHERE u.id = NEW.user_id;
...
-- [GARBAGE]     THEN
        PERFORM
-- [GARBAGE]           net.http_post(
-- [GARBAGE]             url := 'https://' || v_project_ref || '.supabase.co/functions/v1/send-notification-email',
-- [GARBAGE]             headers := jsonb_build_object(
                'Content-Type', 'application/json',
                'Authorization', 'Bearer ' || v_service_key
            ),
...
-- [HEADER] ALTER TABLE public.admin_notifications ADD CONSTRAINT admin_notifications_type_check 
-- [GARBAGE] CHECK (type IN ('upgrade_request', 'system_alert', 'support_ticket', 'user_signup', 'payment_success'));
...
ALTER TABLE public.properties 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS has_balcony BOOLEAN DEFAULT false,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS has_safe_room BOOLEAN DEFAULT false;
...
-- [HEADER] ALTER TABLE IF EXISTS public.contracts 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS pets_allowed BOOLEAN DEFAULT true,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS special_clauses TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS guarantees TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS guarantors_info TEXT;

-- Add elevator and accessibility columns to properties table
ALTER TABLE public.properties 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS has_elevator BOOLEAN DEFAULT false,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS is_accessible BOOLEAN DEFAULT false;
...
BEGIN
    ...
    SET status = CASE 
-- [GARBAGE]         WHEN EXISTS (
-- [HEADER]             SELECT 1 FROM public.contracts c
-- [GARBAGE]             WHERE c.property_id = p.id
-- [GARBAGE]             AND c.status = 'active'
-- [GARBAGE]             AND c.start_date <= CURRENT_DATE
-- [GARBAGE]             AND (c.end_date IS NULL OR c.end_date >= CURRENT_DATE)
        ) THEN 'Occupied'
-- [GARBAGE]         ELSE 'Vacant'
    END
-- [GARBAGE]     WHERE p.id IS NOT NULL; -- Added safe WHERE clause
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- [GARBAGE]         WHERE property_id = target_property_id 
-- [GARBAGE]         AND status = 'active'
-- [GARBAGE]         AND start_date <= CURRENT_DATE
-- [GARBAGE]         AND (end_date IS NULL OR end_date >= CURRENT_DATE)
    ) THEN
        UPDATE public.properties
        SET status = 'Occupied'
-- [GARBAGE]         WHERE id = target_property_id;
...
ALTER TABLE public.properties 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'Vacant' CHECK (status IN ('Occupied', 'Vacant'));

-- 2. Add 'updated_at' column if missing
ALTER TABLE public.properties 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW();
...
-- [GARBAGE]     BEFORE UPDATE ON public.contracts
    FOR EACH ROW
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS needs_painting BOOLEAN DEFAULT false,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS option_notice_days INTEGER,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS last_name TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS phone TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS plan_id TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS marketing_consent BOOLEAN DEFAULT FALSE,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS marketing_consent_at TIMESTAMPTZ,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS subscription_plan TEXT;
...
AS $$
DECLARE
-- [GARBAGE]     v_full_name TEXT;
-- [GARBAGE]     v_first_name TEXT;
-- [GARBAGE]     v_last_name TEXT;
-- [GARBAGE]     v_plan_id TEXT := 'free';
BEGIN
    -- Parse metadata
-- [GARBAGE]     v_full_name := COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1));
-- [GARBAGE]     v_first_name := COALESCE(NEW.raw_user_meta_data->>'first_name', split_part(v_full_name, ' ', 1), 'User');
-- [GARBAGE]     v_last_name := COALESCE(NEW.raw_user_meta_data->>'last_name', 'User');
...
-- INSERT: Only admins
-- [HEADER] CREATE POLICY "Admins can insert plans"
-- [GARBAGE]     ON subscription_plans FOR INSERT
    WITH CHECK (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
-- [GARBAGE]             WHERE id = auth.uid()
...
-- UPDATE: Only admins
-- [HEADER] CREATE POLICY "Admins can update plans"
-- [GARBAGE]     ON subscription_plans FOR UPDATE
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
-- [GARBAGE]             WHERE id = auth.uid()
...
-- DELETE: Only admins
-- [HEADER] CREATE POLICY "Admins can delete plans"
-- [GARBAGE]     ON subscription_plans FOR DELETE
    USING (
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM user_profiles
-- [GARBAGE]             WHERE id = auth.uid()
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [GARBAGE]         WHERE table_name = 'subscription_plans' AND column_name = 'max_tenants'
    ) THEN
...
SET 
-- [GARBAGE]     name = 'Beta Access (Unlimited)',
-- [GARBAGE]     max_properties = -1,
-- [GARBAGE]     max_contracts = -1,
-- [GARBAGE]     max_sessions = -1,
...
-- [GARBAGE]     features = jsonb_build_object(
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
-- [GARBAGE]         WHERE table_name = 'subscription_plans' AND column_name = 'max_storage_mb'
    ) THEN
...
ALTER TABLE subscription_plans 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;
...
AS $$
DECLARE
-- [GARBAGE]     selected_plan TEXT;
BEGIN
    -- Extract plan_id from metadata or default to 'free'
-- [GARBAGE]     selected_plan := COALESCE(NEW.raw_user_meta_data->>'plan_id', 'free');
...
-- [GARBAGE]         NEW.email,
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'full_name', split_part(NEW.email, '@', 1)),
-- [GARBAGE]         split_part(COALESCE(NEW.raw_user_meta_data->>'full_name', ''), ' ', 1),
-- [GARBAGE]         split_part(COALESCE(NEW.raw_user_meta_data->>'full_name', ''), ' ', 2),
        'user',
        'active',
-- [GARBAGE]         selected_plan
    )
-- [GARBAGE]     ON CONFLICT (id) DO UPDATE SET
-- [GARBAGE]         email = EXCLUDED.email,
-- [GARBAGE]         full_name = COALESCE(EXCLUDED.full_name, user_profiles.full_name),
-- [GARBAGE]         first_name = COALESCE(EXCLUDED.first_name, user_profiles.first_name),
-- [GARBAGE]         last_name = COALESCE(EXCLUDED.last_name, user_profiles.last_name),
-- [GARBAGE]         plan_id = COALESCE(selected_plan, user_profiles.plan_id),
-- [GARBAGE]         updated_at = NOW();
...
-- [GARBAGE]     region_name TEXT PRIMARY KEY,
-- [GARBAGE]     avg_rent NUMERIC NOT NULL,
-- [GARBAGE]     growth_1y NUMERIC DEFAULT 0,
-- [GARBAGE]     growth_2y NUMERIC DEFAULT 0,
-- [GARBAGE]     growth_5y NUMERIC DEFAULT 0,
-- [GARBAGE]     month_over_month NUMERIC DEFAULT 0,
-- [GARBAGE]     room_adjustments JSONB NOT NULL DEFAULT '{"2": 0.8, "3": 1.0, "4": 1.25, "5": 1.5}'::jsonb,
-- [GARBAGE]     type_adjustments JSONB NOT NULL DEFAULT '{"apartment": 1.0, "penthouse": 1.4, "house": 1.8}'::jsonb,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW(),
-- [GARBAGE]     updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [HEADER] CREATE POLICY "Allow public read access to rental market data"
-- [GARBAGE]     ON public.rental_market_data
    FOR SELECT
-- [GARBAGE]     TO public
    USING (true);
...
-- [GARBAGE] ON CONFLICT (region_name) DO UPDATE SET 
-- [GARBAGE]     avg_rent = EXCLUDED.avg_rent,
-- [GARBAGE]     growth_1y = EXCLUDED.growth_1y,
-- [GARBAGE]     growth_2y = EXCLUDED.growth_2y,
-- [GARBAGE]     growth_5y = EXCLUDED.growth_5y,
-- [GARBAGE]     month_over_month = EXCLUDED.month_over_month,
-- [GARBAGE]     room_adjustments = EXCLUDED.room_adjustments,
-- [GARBAGE]     type_adjustments = EXCLUDED.type_adjustments,
-- [GARBAGE]     updated_at = NOW();
-- Migration: enhance_subscription_marketing
-- Description: Adds marketing-focused columns to subscription_plans table.

ALTER TABLE subscription_plans
-- [GARBAGE] ADD COLUMN IF NOT EXISTS description TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS subtitle TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS badge_text TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS cta_text TEXT,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS sort_order INTEGER DEFAULT 0;

-- Set some reasonable defaults for existing plans to avoid empty fields
UPDATE subscription_plans 
SET 
-- [GARBAGE]     description = CASE 
-- [GARBAGE]         WHEN id = 'free' THEN 'Essential tracking for individual property owners.'
-- [GARBAGE]         WHEN id = 'solo' THEN 'Advanced optimization for serious landlords.'
-- [GARBAGE]         WHEN id = 'pro' THEN 'The ultimate yield maximizer for portfolio managers.'
-- [GARBAGE]         ELSE 'Manage your rental business professionally.'
-- [GARBAGE]     END,
-- [GARBAGE]     cta_text = CASE 
-- [GARBAGE]         WHEN price_monthly = 0 THEN 'Get Started'
-- [GARBAGE]         ELSE 'Start Free Trial'
-- [GARBAGE]     END,
-- [GARBAGE]     sort_order = CASE 
-- [GARBAGE]         WHEN id = 'free' THEN 10
-- [GARBAGE]         WHEN id = 'solo' THEN 20
-- [GARBAGE]         WHEN id = 'pro' THEN 30
-- [GARBAGE]         ELSE 100
    END
-- [GARBAGE] WHERE description IS NULL;
...
SELECT cron.schedule(
    ...
    SELECT
-- [GARBAGE]         net.http_post(
-- [GARBAGE]             url:=(SELECT value FROM system_settings WHERE key = 'api_url' LIMIT 1) || '/functions/v1/sync-rental-trends',
-- [GARBAGE]             headers:=jsonb_build_object(
                'Content-Type', 'application/json',
...
-- [GARBAGE]     FROM (
        SELECT 
-- [GARBAGE]             COALESCE(city, 'Unknown') as name,
-- [GARBAGE]             COUNT(*) as count
-- [GARBAGE]         FROM properties
-- [GARBAGE]         GROUP BY city
-- [GARBAGE]         ORDER BY count DESC
-- [GARBAGE]         LIMIT 10
    ) city_stats;
...
SELECT cron.schedule(
    ...
    SELECT
-- [GARBAGE]       net.http_post(
-- [GARBAGE]         url := 'https://' || public.get_supabase_config('supabase_project_ref') || '.supabase.co/functions/v1/send-daily-admin-summary',
-- [GARBAGE]         headers := jsonb_build_object(
          'Content-Type', 'application/json',
...
-- [GARBAGE] WHERE NOT EXISTS (SELECT 1 FROM public.system_settings WHERE key = 'supabase_project_ref');

INSERT INTO public.system_settings (key, value, description)
SELECT 'supabase_service_role_key', ('"' || current_setting('app.settings.service_role_key', true) || '"')::jsonb, 'Supabase Service Role Key'
-- [GARBAGE] WHERE NOT EXISTS (SELECT 1 FROM public.system_settings WHERE key = 'supabase_service_role_key')
-- [GARBAGE] AND current_setting('app.settings.service_role_key', true) IS NOT NULL;
...
-- [GARBAGE]         ELSE 'Vacant'
    END
-- [GARBAGE]     WHERE p.id IS NOT NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
EXCEPTION WHEN OTHERS THEN
-- [GARBAGE]     NULL; -- Skip if not scheduled
END $$;
...
VALUES 
    ...
    ('admin_email_daily_summary_enabled', 'true'::jsonb, 'Master toggle for daily admin summary email')
-- [GARBAGE] ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;
...
-- [GARBAGE] add column if not exists has_seen_welcome_v1 boolean default false;

-- Comment for documentation
-- Add seen_features tracking array
alter table public.user_preferences 
-- [GARBAGE] add column if not exists seen_features text[] default '{}';
...
-- [GARBAGE]     name = 'SOLO',
-- [GARBAGE]     max_properties = 1,
-- [GARBAGE]     price_monthly = 0,
-- [GARBAGE]     price_yearly = 0,
-- [GARBAGE]     features = '{"legal_library": true, "maintenance_tracker": true, "ai_assistant": false, "bill_analysis": false, "can_export": false, "cpi_autopilot": false}'::jsonb
-- [GARBAGE] WHERE id = 'free' OR id = 'solo';

-- 2. Update MATE (Pro)
UPDATE subscription_plans
SET 
-- [GARBAGE]     name = 'MATE',
-- [GARBAGE]     max_properties = 3,
-- [GARBAGE]     price_monthly = 0, -- Testing Stage: Free
-- [GARBAGE]     price_yearly = 0,
-- [GARBAGE]     features = '{"legal_library": true, "maintenance_tracker": true, "ai_assistant": true, "bill_analysis": true, "can_export": false, "cpi_autopilot": true, "whatsapp_bot": true}'::jsonb
-- [GARBAGE] WHERE id = 'pro' OR id = 'mate';

-- 3. Update MASTER (Enterprise)
UPDATE subscription_plans
SET 
-- [GARBAGE]     name = 'MASTER',
-- [GARBAGE]     max_properties = 10,
-- [GARBAGE]     price_monthly = 0, -- Testing Stage: Free
-- [GARBAGE]     price_yearly = 0,
-- [GARBAGE]     features = '{"legal_library": true, "maintenance_tracker": true, "ai_assistant": true, "bill_analysis": true, "can_export": true, "cpi_autopilot": true, "whatsapp_bot": true, "portfolio_visualizer": true}'::jsonb
-- [GARBAGE] WHERE id = 'enterprise' OR id = 'master';
...
BEGIN
    IF (OLD.role != NEW.role OR OLD.is_super_admin != NEW.is_super_admin) THEN
        ...
        IF NOT EXISTS (
            SELECT 1 FROM public.user_profiles 
-- [GARBAGE]             WHERE id = auth.uid() AND (role = 'admin' OR is_super_admin = true)
        ) THEN
...
-- [GARBAGE]     BEFORE UPDATE ON public.user_profiles
    FOR EACH ROW
...
SET public = false 
-- [GARBAGE] WHERE id = 'contracts';

-- 2. Harden 'property_images' bucket (if it exists)
UPDATE storage.buckets 
SET public = false 
-- [GARBAGE] WHERE id = 'property_images';

-- 3. Ensure 'secure_documents' is private
UPDATE storage.buckets 
SET public = false 
-- [GARBAGE] WHERE id = 'secure_documents';
...
-- [HEADER] CREATE POLICY "Users view own contracts"
-- [GARBAGE]     ON storage.objects
    FOR SELECT
    USING (
-- [GARBAGE]         bucket_id = 'contracts'
-- [GARBAGE]         AND
        (storage.foldername(name))[1] = auth.uid()::text
    );

DROP POLICY IF EXISTS "Users upload own contracts" ON storage.objects;
-- [HEADER] CREATE POLICY "Users upload own contracts"
-- [GARBAGE]     ON storage.objects
    FOR INSERT
    WITH CHECK (
-- [GARBAGE]         bucket_id = 'contracts'
...
-- [HEADER] CREATE POLICY "Users view own images"
-- [GARBAGE]     ON storage.objects
    FOR SELECT
    USING (
-- [GARBAGE]         bucket_id = 'property_images'
-- [GARBAGE]         AND
        (storage.foldername(name))[1] = auth.uid()::text
    );

DROP POLICY IF EXISTS "Users upload own images" ON storage.objects;
-- [HEADER] CREATE POLICY "Users upload own images"
-- [GARBAGE]     ON storage.objects
    FOR INSERT
    WITH CHECK (
-- [GARBAGE]         bucket_id = 'property_images'
...
CREATE POLICY "Users can view own whatsapp messages" ON public.whatsapp_messages
            FOR SELECT USING (
-- [GARBAGE]                 EXISTS (
-- [HEADER]                     SELECT 1 FROM public.whatsapp_conversations c
-- [GARBAGE]                     WHERE c.id = whatsapp_messages.conversation_id
-- [GARBAGE]                     AND (c.user_id = auth.uid() OR public.is_admin())
                )
            );
    END IF;
END $$;
...
UPDATE storage.buckets 
SET public = false 
-- [GARBAGE] WHERE id = 'feedback-screenshots';
...
-- [HEADER] CREATE POLICY "Users can upload own feedback screenshots"
-- [GARBAGE]     ON storage.objects FOR INSERT
    WITH CHECK (
-- [GARBAGE]         bucket_id = 'feedback-screenshots'
-- [GARBAGE]         AND
        (storage.foldername(name))[1] = auth.uid()::text
    );

-- [HEADER] CREATE POLICY "Users can view own feedback screenshots"
-- [GARBAGE]     ON storage.objects FOR SELECT
    USING (
-- [GARBAGE]         bucket_id = 'feedback-screenshots'
-- [GARBAGE]         AND
...
DROP POLICY IF EXISTS "Users view own contracts" ON storage.objects;
-- [HEADER] CREATE POLICY "Secure Access: Contracts"
-- [GARBAGE]     ON storage.objects FOR SELECT
    USING (bucket_id = 'contracts' AND (storage.foldername(name))[1] = auth.uid()::text);

DROP POLICY IF EXISTS "Users view own images" ON storage.objects;
-- [HEADER] CREATE POLICY "Secure Access: Property Images"
-- [GARBAGE]     ON storage.objects FOR SELECT
...
-- [GARBAGE] ADD COLUMN IF NOT EXISTS max_whatsapp_messages INTEGER DEFAULT 50;
...
ALTER TABLE public.user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS whatsapp_limit_override INTEGER DEFAULT NULL;

-- 3b. Also add to ai_usage_limits for UI consistency in Usage Dashboard
ALTER TABLE public.ai_usage_limits
-- [GARBAGE] ADD COLUMN IF NOT EXISTS monthly_whatsapp_limit INTEGER DEFAULT 50;
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
-- [GARBAGE]     conversation_id UUID REFERENCES public.whatsapp_conversations(id) ON DELETE SET NULL,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Policies
-- [HEADER] CREATE POLICY "Users can view their own whatsapp usage logs"
-- [GARBAGE]     ON public.whatsapp_usage_logs FOR SELECT
    USING (auth.uid() = user_id OR public.is_admin());

-- [HEADER] CREATE POLICY "Admins can manage all usage logs"
-- [GARBAGE]     ON public.whatsapp_usage_logs FOR ALL
    USING (public.is_admin());
...
-- [GARBAGE]     v_month_start := date_trunc('month', now());
    ...
    SELECT 
-- [GARBAGE]         COALESCE(up.whatsapp_limit_override, p.max_whatsapp_messages, 50) INTO v_limit
-- [GARBAGE]     FROM public.user_profiles up
-- [GARBAGE]     JOIN public.subscription_plans p ON up.plan_id = p.id
-- [GARBAGE]     WHERE up.id = p_user_id;
...
-- [GARBAGE]         v_limit := 50;
    ...
    SELECT COUNT(*)::INTEGER INTO v_current_usage
-- [GARBAGE]     FROM public.whatsapp_usage_logs
-- [GARBAGE]     WHERE user_id = p_user_id
-- [GARBAGE]       AND created_at >= v_month_start;
...
ALTER TABLE public.user_profiles 
-- [GARBAGE] ADD CONSTRAINT user_profiles_phone_key UNIQUE (phone);
...
DECLARE
-- [GARBAGE]     default_plan_id TEXT := 'free';
-- [GARBAGE]     v_phone TEXT;
BEGIN
    -- Extract phone from metadata or use NEW.phone (from auth schema if provided)
-- [GARBAGE]     v_phone := COALESCE(NEW.raw_user_meta_data->>'phone_number', NEW.phone);
...
-- [GARBAGE]         v_phone,
        'user', 
        'active', 
-- [GARBAGE]         COALESCE(NEW.raw_user_meta_data->>'plan_id', default_plan_id)
    )
-- [GARBAGE]     ON CONFLICT (id) DO UPDATE SET
-- [GARBAGE]         email = EXCLUDED.email,
-- [GARBAGE]         full_name = COALESCE(EXCLUDED.full_name, user_profiles.full_name),
...
-- [GARBAGE]         updated_at = NOW();
    ...
    RETURN NEW;
EXCEPTION 
-- [GARBAGE]     WHEN unique_violation THEN
...
FOR EACH ROW
-- [GARBAGE] WHEN (
-- [GARBAGE]     OLD.plan_id IS DISTINCT FROM NEW.plan_id OR
-- [GARBAGE]     OLD.email IS DISTINCT FROM NEW.email OR
-- [GARBAGE]     OLD.phone IS DISTINCT FROM NEW.phone
)
...
UPDATE storage.buckets 
SET public = false 
-- [GARBAGE] WHERE id = 'property-images';
...
-- [HEADER] CREATE POLICY "Manual uploads ownership"
-- [GARBAGE] ON storage.objects FOR ALL
-- [GARBAGE] TO authenticated
USING (
-- [GARBAGE]     bucket_id = 'property-images' AND
    (storage.foldername(name))[1] = (auth.uid())::text
)
WITH CHECK (
-- [GARBAGE]     bucket_id = 'property-images' AND
...
-- [HEADER] CREATE POLICY "Google imports ownership"
-- [GARBAGE] ON storage.objects FOR ALL
-- [GARBAGE] TO authenticated
USING (
-- [GARBAGE]     bucket_id = 'property-images' AND
...
-- [HEADER] CREATE POLICY "Allow public read access to index_data"
-- [GARBAGE] ON index_data
FOR SELECT
-- [GARBAGE] TO anon
USING (true);
...
-- [HEADER] CREATE POLICY "Allow public read access to index_bases"
-- [GARBAGE] ON index_bases
FOR SELECT
-- [GARBAGE] TO anon
USING (true);
...
-- [HEADER] CREATE POLICY "Allow authenticated users to read index_bases"
-- [GARBAGE] ON index_bases
FOR SELECT
-- [GARBAGE] TO authenticated
USING (true);
...
-- [GARBAGE]     stack TEXT,
-- [GARBAGE]     route TEXT,
-- [GARBAGE]     component_stack TEXT,
-- [GARBAGE]     metadata JSONB DEFAULT '{}'::jsonb,
-- [GARBAGE]     is_resolved BOOLEAN DEFAULT false,
-- [GARBAGE]     environment TEXT DEFAULT 'production'
);
...
-- [GARBAGE]         EXISTS (
            SELECT 1 FROM public.user_profiles
-- [GARBAGE]             WHERE user_profiles.id = auth.uid()
-- [GARBAGE]             AND user_profiles.role IN ('admin', 'super_admin')
        )
    );
...
-- [GARBAGE]     AFTER INSERT ON public.error_logs
    FOR EACH ROW
...
-- [GARBAGE] WHERE key IN ('supabase_project_ref', 'supabase_service_role_key')
-- [GARBAGE] AND value::text LIKE '"%"%';
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE,
-- [GARBAGE]     event_name TEXT NOT NULL,
-- [GARBAGE]     metadata JSONB DEFAULT '{}'::jsonb,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT now()
);
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
-- [GARBAGE]         WHERE tablename = 'analytics_events' AND policyname = 'Admins can read all analytics'
    ) THEN
-- [HEADER]         ;
...
-- [GARBAGE]                     WHERE id = auth.uid() AND role = 'admin'
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
-- [GARBAGE]         WHERE tablename = 'analytics_events' AND policyname = 'Users can log their own events'
    ) THEN
-- [HEADER]         ;
...
-- [GARBAGE]                     up.full_name,
-- [GARBAGE]                     up.email,
-- [GARBAGE]                     count(*) as event_count
-- [GARBAGE]                 FROM analytics_events ae
-- [GARBAGE]                 JOIN user_profiles up ON ae.user_id = up.id
-- [GARBAGE]                 WHERE ae.created_at > now() - (days_limit || ' days')::interval
-- [GARBAGE]                 GROUP BY ae.user_id, up.full_name, up.email
-- [GARBAGE]                 ORDER BY event_count DESC
-- [GARBAGE]                 LIMIT 10
            ) u
        ),
        'popular_features', (
            SELECT jsonb_agg(f) FROM (
                SELECT 
-- [GARBAGE]                     event_name,
-- [GARBAGE]                     count(*) as usage_count
-- [GARBAGE]                 FROM analytics_events
-- [GARBAGE]                 WHERE created_at > now() - (days_limit || ' days')::interval
-- [GARBAGE]                 GROUP BY event_name
-- [GARBAGE]                 ORDER BY usage_count DESC
            ) f
        ),
        'daily_trends', (
            SELECT jsonb_agg(t) FROM (
                SELECT 
-- [GARBAGE]                     date_trunc('day', created_at)::date as day,
-- [GARBAGE]                     count(*) as count
-- [GARBAGE]                 FROM analytics_events
-- [GARBAGE]                 WHERE created_at > now() - (days_limit || ' days')::interval
-- [GARBAGE]                 GROUP BY 1
-- [GARBAGE]                 ORDER BY 1 ASC
            ) t
        )
    ) INTO result;
...
ALTER TABLE public.user_profiles 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS security_status public.account_security_status DEFAULT 'active',
-- [GARBAGE] ADD COLUMN IF NOT EXISTS security_notes TEXT[],
-- [GARBAGE] ADD COLUMN IF NOT EXISTS flagged_at TIMESTAMPTZ,
-- [GARBAGE] ADD COLUMN IF NOT EXISTS last_security_check TIMESTAMPTZ;

-- 3. Create security_logs table
CREATE TABLE IF NOT EXISTS public.security_logs (
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     user_id UUID REFERENCES public.user_profiles(id) ON DELETE CASCADE,
-- [GARBAGE]     event_code TEXT NOT NULL, -- e.g. 'AUTH_VELOCITY', 'WHATSAPP_SPIKE', 'RESOURCE_SPIKE'
-- [GARBAGE]     severity TEXT CHECK (severity IN ('low', 'medium', 'high', 'critical')) DEFAULT 'low',
-- [GARBAGE]     details JSONB DEFAULT '{}'::jsonb,
-- [GARBAGE]     ip_address TEXT,
-- [GARBAGE]     user_agent TEXT,
-- [GARBAGE]     created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- [HEADER]     CREATE POLICY "Admins can view security logs"
-- [GARBAGE]         ON public.security_logs FOR SELECT
        USING (public.is_admin());
EXCEPTION WHEN OTHERS THEN
-- [HEADER]     CREATE POLICY "Admins can view security logs"
-- [GARBAGE]         ON public.security_logs FOR SELECT
...
-- [GARBAGE]     p_user_id UUID,
-- [GARBAGE]     p_event_code TEXT,
-- [GARBAGE]     p_severity TEXT,
-- [GARBAGE]     p_details JSONB DEFAULT '{}'::jsonb,
-- [GARBAGE]     p_ip TEXT DEFAULT NULL,
-- [GARBAGE]     p_ua TEXT DEFAULT NULL
)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
...
-- [GARBAGE]             flagged_at = NOW()
-- [GARBAGE]         WHERE id = p_user_id AND (security_status = 'active' OR security_status IS NULL);
    END IF;
END;
$$;
...
RETURNS TABLE (
-- [GARBAGE]     user_id UUID,
-- [GARBAGE]     event_code TEXT,
-- [GARBAGE]     severity TEXT,
-- [GARBAGE]     details JSONB
)
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
-- [GARBAGE]     v_user_id UUID;
-- [GARBAGE]     v_event_code TEXT;
-- [GARBAGE]     v_severity TEXT;
-- [GARBAGE]     v_details JSONB;
-- [GARBAGE]     v_hour_ago TIMESTAMPTZ := NOW() - INTERVAL '1 hour';
BEGIN
...
-- [GARBAGE]         FROM public.whatsapp_usage_logs w
-- [GARBAGE]         WHERE w.created_at >= v_hour_ago
-- [GARBAGE]         GROUP BY w.user_id
-- [GARBAGE]         HAVING count(*) > 30
    LOOP
        PERFORM public.log_security_event(v_user_id, 'WHATSAPP_SPIKE', 'medium', v_details);
-- [GARBAGE]         user_id := v_user_id;
-- [GARBAGE]         event_code := 'WHATSAPP_SPIKE';
-- [GARBAGE]         severity := 'medium';
-- [GARBAGE]         details := v_details;
        RETURN NEXT;
    END LOOP;
...
-- [GARBAGE]         FROM public.properties p
-- [GARBAGE]         WHERE p.created_at >= v_hour_ago
-- [GARBAGE]         GROUP BY p.user_id
-- [GARBAGE]         HAVING count(*) > 5
    LOOP
        PERFORM public.log_security_event(v_user_id, 'RESOURCE_SPIKE', 'high', v_details);
-- [GARBAGE]         user_id := v_user_id;
-- [GARBAGE]         event_code := 'RESOURCE_SPIKE';
-- [GARBAGE]         severity := 'high';
-- [GARBAGE]         details := v_details;
        RETURN NEXT;
    END LOOP;
...
-- [GARBAGE]         FROM public.security_logs s1
-- [GARBAGE]         JOIN public.security_logs s2 ON s1.ip_address = s2.ip_address AND s1.user_id != s2.user_id
-- [GARBAGE]         WHERE s1.created_at >= v_hour_ago AND s2.created_at >= v_hour_ago
-- [GARBAGE]         GROUP BY s1.user_id, s1.ip_address
-- [GARBAGE]         HAVING count(distinct s2.user_id) > 2
    LOOP
        PERFORM public.log_security_event(v_user_id, 'IP_COLLISION', 'medium', v_details);
-- [GARBAGE]         user_id := v_user_id;
-- [GARBAGE]         event_code := 'IP_COLLISION';
-- [GARBAGE]         severity := 'medium';
-- [GARBAGE]         details := v_details;
        RETURN NEXT;
    END LOOP;
...
-- [GARBAGE]     storage_usage_mb NUMERIC,
-- [GARBAGE]     is_super_admin BOOLEAN,
-- [GARBAGE]     security_status TEXT,
-- [GARBAGE]     flagged_at TIMESTAMPTZ,
-- [GARBAGE]     last_security_check TIMESTAMPTZ
) 
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- [GARBAGE]         COALESCE(up.is_super_admin, false) as is_super_admin,
        ...
        -- Security Fields
-- [GARBAGE]         up.security_status::TEXT,
-- [GARBAGE]         up.flagged_at,
-- [GARBAGE]         up.last_security_check
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_preferences' AND column_name = 'disclaimer_accepted') THEN
        ALTER TABLE user_preferences 
-- [GARBAGE]         ADD COLUMN IF NOT EXISTS disclaimer_accepted BOOLEAN DEFAULT false;
    END IF;
END $$;
-- ============================================
...
BEGIN
    -- Fetch config 
-- [GARBAGE]     v_ref := public.get_supabase_config('supabase_project_ref');
-- [GARBAGE]     v_key := public.get_supabase_config('supabase_service_role_key');
    
    -- Validate config
-- [HEADER]     IF v_ref IS NULL OR v_key IS NULL THEN
        RAISE WARNING 'Daily Admin Summary skipped: Missing config (ref=%, key_present=%)', v_ref, (v_key IS NOT NULL);
        RETURN;
    END IF;

    -- Construct URL
-- [GARBAGE]     v_url := 'https://' || v_ref || '.supabase.co/functions/v1/send-daily-admin-summary';
...
ALTER TABLE public.payments 
-- [GARBAGE] ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id);
...
-- Create RLS Policy
-- [HEADER] CREATE POLICY "Users can only see their own payments" 
-- [GARBAGE] ON public.payments 
FOR ALL 
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);
...
-- [GARBAGE]     id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
-- [GARBAGE]     created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
-- [GARBAGE]     function_name TEXT NOT NULL,
-- [GARBAGE]     level TEXT DEFAULT 'info',
-- [GARBAGE]     message TEXT NOT NULL,
-- [GARBAGE]     details JSONB
);
...
-- [HEADER] CREATE POLICY "Allow service role to insert debug logs"
-- [GARBAGE]     ON public.debug_logs
    FOR INSERT
-- [GARBAGE]     TO service_role
    WITH CHECK (true);

-- [HEADER] CREATE POLICY "Allow service role to select debug logs"
-- [GARBAGE]     ON public.debug_logs
    FOR SELECT
-- [GARBAGE]     TO service_role
    USING (true);
...
BEGIN
    ...
    FOR r IN 
        SELECT d.* 
-- [GARBAGE]         FROM cron.job_run_details d
-- [GARBAGE]         JOIN cron.job j ON j.jobid = d.jobid
-- [GARBAGE]         WHERE j.jobname = 'daily-admin-summary'
-- [GARBAGE]         AND d.start_time > (now() - interval '24 hours')
-- [GARBAGE]         ORDER BY d.start_time DESC 
    LOOP
        RAISE NOTICE 'RUN: ID=%, Status=%, Msg="%", Time=%', 
-- [GARBAGE]             r.runid, r.status, r.return_message, r.start_time;
    END LOOP;

    RAISE NOTICE '--- DEBUG LOGS (Today) ---';
    FOR r IN 
        SELECT created_at, message, details 
-- [GARBAGE]         FROM public.debug_logs 
-- [GARBAGE]         WHERE created_at > (now() - interval '24 hours')
-- [GARBAGE]         ORDER BY created_at DESC 
-- [GARBAGE]         LIMIT 10
    LOOP
...
-- [HEADER]     IF v_ref IS NULL OR v_key IS NULL THEN
    ...
    -- Construct URL
-- [GARBAGE]     v_url := 'https://' || v_ref || '.supabase.co/functions/v1/fetch-index-data';
...
DO $$
DECLARE
-- [GARBAGE]     job_names TEXT[] := ARRAY['index-update-day15', 'index-update-day16', 'index-update-day17', 'index-sync-primary', 'index-sync-retry-15', 'index-sync-retry-16'];
-- [GARBAGE]     jname TEXT;
BEGIN
    -- Unschedule legacy jobs safely
-- [GARBAGE]     FOREACH jname IN ARRAY job_names LOOP
